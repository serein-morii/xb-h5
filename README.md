<div class="batch-hint">全量 <b>106</b> 条账单，两种模式覆盖范围不同，请按需选择。</div># xb-h5｜喜八移动订单管理
...
> 写给实际用平台的人看的；下面的「最快发布方式」「本地开发」「生产构建」等是给开发/运维看的。

OTP 动态口令保险库的公开指南位于 `/otp/guide`，维护用文字版见：[OTP Vault 使用教程](docs/otp-vault-user-guide.md)。

**平台地址**：https://m.gooop.top

**测试账号**：请向管理员索取 UAT 测试账号（勿将生产密码写入公开文档）。

### 主要功能

- **订单管理**：录单、批量录单、发货、改地址、查询、删除
- **买家管理**：维护客户档案，生成专属下单短链接；可针对单个买家禁止下单 / 禁止查询订单
- **物流跟踪**：在途 / 签收 / 滞留订单监控，支持快递100、顺丰、EMS
- **物流用量**：店铺额度分配 + 手动 / 定时 / 查询三档开关
- **账单 / 价格 / 店铺**：成本核算、商品定价、店铺配置
- **短链管理**：自定义 `m.gooop.top/xxx` 跳转到公开工具页或外部网址，可设过期时间，可查访问次数和最近访问 IP

### 公开入口（无需登录）

| 地址 | 用途 |
| --- | --- |
| https://m.gooop.top/tools/order-search | 订单查询：手机号 + 验证码（不区分店铺） |
| https://m.gooop.top/tools/store-query | 专属查询：先选店铺，再输入手机号 + 验证码，只查该店铺的订单 |
| https://m.gooop.top/order | 订单查询：短码 + 验证码 |
| https://m.gooop.top/tools | 免登录工具箱：订单查询 / 专属查询 / 运费计算 / 运费对比 |
| https://m.gooop.top/{自定义路径} | 短链：登录后在「短链管理」里创建 |

### 注意事项

- 两个账号看到的是同一份数据，操作会互相影响
- 短链可以发给客户直接访问，无需登录；新建时已做本地路由冲突校验
- 公开接口有 IP 限速（30 秒 30 次），短链是 30 秒 60 次
...
- 店铺管理：店铺资料、通知配置和营业状态。
- 免登录工具箱：订单查询、专属查询、链接查询、运费对比和运费计算。
...
| `/tools` | 免登录工具箱 | 否 |
| `/tools/order-search` | 手机号和验证码查询订单（不区分店铺） | 否 |
| `/tools/store-query` | 专属查询：列出全部店铺，按店铺查单避免串单 | 否 |
| `/tools/store-query/{店铺编码}` | 进入某个店铺的专属查询页 | 否 |
