    ...
    <ResourceList
      ...
      renderActions={access.has("operations.online.forceLogout") ? (row, reload) => (
        <button className="danger-text" type="button" onClick={() => requestConfirm({
          ...
          danger: true,
          action: async () => {
            await apiRequest(`/monitor/online/${encodeURIComponent(textValue(row.tokenId, ""))}`, { method: "DELETE" });
...
function JobsPage({ notify, onBack, openLogs }: { notify: Notify; onBack: () => void; openLogs: () => void }) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      await apiRequest("/monitor/job", { method: editing === "new" ? "POST" : "PUT", body: form });
      ...
      renderActions={(row, reload) => (
        <>
          {access.has("operations.jobs.changeStatus") ? <button type="button" onClick={async () => {
            try {
              await apiRequest("/monitor/job/run", { method: "PUT", body: { jobId: row.jobId, jobGroup: row.jobGroup } });
          ...
          {access.has("operations.jobs.changeStatus") ? <button type="button" onClick={async () => {
            ...
            try {
              await apiRequest("/monitor/job/changeStatus", { method: "PUT", body: { jobId: row.jobId, status: next } });
          ...
          {access.has("operations.jobs.delete") ? <button className="danger-text" type="button" onClick={() => requestConfirm({
            ...
            danger: true,
            action: async () => {
              await apiRequest(`/monitor/job/${row.jobId}`, { method: "DELETE" });
...
function JobLogsPage({ notify, onBack }: { notify: Notify; onBack: () => void }) {
  ...
  return (
    <ResourceList
      ...
      renderActions={access.has("operations.jobs.delete") ? (row, reload) => <DeleteAction endpoint={`/monitor/jobLog/${row.jobLogId}`} label="调度日志" notify={notify} reload={reload} /> : undefined}
    />
  );
}
...
function OperLogsPage({ notify, onBack }: { notify: Notify; onBack: () => void }) {
  ...
  return (
    <ResourceList
      ...
      renderActions={access.has("operations.operLogs.delete") ? (row, reload) => <DeleteAction endpoint={`/monitor/operlog/${row.operId}`} label="操作日志" notify={notify} reload={reload} /> : undefined}
    />
  );
}
...
function LoginLogsPage({ notify, onBack }: { notify: Notify; onBack: () => void }) {
  ...
  return (
    <ResourceList
      ...
      renderActions={access.has("operations.loginLogs.delete") ? (row, reload) => <DeleteAction endpoint={`/monitor/logininfor/${row.infoId}`} label="登录日志" notify={notify} reload={reload} /> : undefined}
    />
  );
}
...
function ServerPage({ notify, onBack }: { notify: Notify; onBack: () => void }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const result = await apiRequest<DataRow>("/monitor/server");
...
function CachePage({ notify, onBack }: { notify: Notify; onBack: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [cacheResult, namesResult] = await Promise.all([
        apiRequest<DataRow>("/monitor/cache"),
        apiRequest<DataRow>("/monitor/cache/getNames"),
      ]);
  ...
  async function selectName(name: string) {
    ...
    setValue(null);
    try {
      const result = await apiRequest<DataRow>(`/monitor/cache/getKeys/${encodeURIComponent(name)}`);
  ...
  async function selectKey(key: string) {
    setActiveKey(key);
    try {
      const result = await apiRequest<DataRow>(`/monitor/cache/getValue/${encodeURIComponent(activeName)}/${encodeURIComponent(key)}`);
  ...
  return (
    <div className="module-page opsc-page">
      ...
      <div className="opsc-cache-browser">
        ...
        <article className="opsc-cache-column"><div className="opsc-cache-title"><h2>缓存键</h2>{activeName ? <button type="button" onClick={() => void clear(`/monitor/cache/clearCacheName/${encodeURIComponent(activeName)}`, "清理该缓存空间", () => { setKeys([]); setValue(null); })}><Trash2 size={14} /></button> : null}</div>{keys.map((key) => <button className={`opsc-cache-item ${activeKey === key ? "opsc-cache-item-active" : ""}`} key={key} type="button" onClick={() => void selectKey(key)}><code>{key}</code></button>)}</article>
        <article className="opsc-cache-column"><div className="opsc-cache-title"><h2>缓存内容</h2>{activeKey ? <button type="button" onClick={() => void clear(`/monitor/cache/clearCacheKey/${encodeURIComponent(activeKey)}`, "删除该缓存键", () => { setKeys((current) => current.filter((key) => key !== activeKey)); setActiveKey(""); setValue(null); })}><Trash2 size={14} /></button> : null}</div><pre className="opsc-json">{value === null ? "选择缓存键查看内容" : JSON.stringify(value, null, 2)}</pre></article>
      </div>
...
async function downloadGeneratedCode(tableNames: string) {
  const response = await fetch(`${API_BASE}/tool/gen/batchGenCode${toQuery({ tables: tableNames })}`, {
...
function GeneratorPage({ notify, onBack }: { notify: Notify; onBack: () => void }) {
  ...
  async function preview(row: DataRow) {
    try {
      const result = await apiRequest<DataRow>(`/tool/gen/preview/${row.tableId}`);
  ...
  async function loadDbTables() {
    setDbLoading(true);
    try {
      const result = await apiRequest<DataRow>("/tool/gen/db/list", { query: { pageNum: 1, pageSize: 100, tableName: dbKeyword || undefined } });
  ...
  async function importTables(reload: () => Promise<void>) {
    ...
    try {
      await apiRequest("/tool/gen/importTable", { method: "POST", query: { tables: [...dbSelected].join(",") } });
  ...
  async function openEdit(row: DataRow) {
    setEditLoading(true);
    try {
      const result = await apiRequest<DataRow>(`/tool/gen/${row.tableId}`);
  ...
  async function saveEdit() {
    ...
    setSaving(true);
    try {
      await apiRequest("/tool/gen", { method: "PUT", body: { ...editing.info, columns: editing.columns, params: editing.info.params } });
        ...
        <>
          ...
          {access.has("operations.codegen.edit") ? <button type="button" onClick={() => {
            ...
            requestConfirm({
              ...
              danger: true,
              action: async () => {
                await apiRequest(`/tool/gen/synchDb/${encodeURIComponent(tableName)}`);
          ...
          {access.has("operations.codegen.delete") ? <button className="danger-text" type="button" onClick={() => requestConfirm({
            ...
            danger: true,
            action: async () => {
              await apiRequest(`/tool/gen/${row.tableId}`, { method: "DELETE" });
