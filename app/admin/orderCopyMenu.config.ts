...
function buildRowContext(row: Record<string, unknown>): Record<string, string> {
  ...
  const orderLink = `${origin}/tools/order#${encodeURIComponent(signId)}`;
  const purchaserLink = `${origin}/tools/order#${encodeURIComponent(`v-${signId}`)}`;
  return {
...
export async function fetchOrderCopyMenu(
  ...
  try {
    const result = await request<Record<string, unknown>>("/system/order-copy-menu/config");
