...
import {
	...
	listVaultRecipients, revokeVaultShare, saveVaultCredential, clearOtpToken, getOtpToken, otpApiRequest, setOtpToken, type VaultCredential, type VaultRecipient, type VaultShare,
} from "./vaultApi";
import OtpAuthScreen from "./OtpAuthScreen";
import "./otp-vault.css";

type Modal = "credential" | "scanner" | "detail" | "import" | "share" | "shareDetail" | "created" | null;
type VaultView = "all" | "favorite" | "shares" | "settings";
type VaultPrefs = { masked: boolean; compact: boolean; grouped: boolean };
...
function parseGoogleMigration(url: URL): Partial<typeof emptyCredential>[] {
  ...
  readProtoFields(bytes, (field, wire, value) => { if (field === 1 && wire === 2 && value instanceof Uint8Array) entries.push(value); });
...
function decodeQrWithCanvas(source: CanvasImageSource) {
...
function OtpVaultContent({ onLogout }: { onLogout: () => void }) {
  ...
  const [prefs, setPrefs] = useState<VaultPrefs>(() => { try { return { masked: false, compact: false, grouped: true, ...JSON.parse(localStorage.getItem("handy-vault-prefs") || "{}") }; } catch { return { masked: false, compact: false, grouped: true }; } });
  ...
  const filtered = useMemo(() => {
    ...
    const result = credentials.filter((item) => (!value || `${item.issuer} ${item.accountName}`.toLowerCase().includes(value)) && (!issuer || item.issuer === issuer) && (view !== "favorite" || item.favorite));
    ...
    return result;
  }, [credentials, issuer, query, sort, view]);
  ...
  const applyScannedCredential = async (raw: string) => {
    const parsed = parseOtpPayload(raw);
    if (parsed.length === 1) {
      setForm((current) => ({ ...current, ...parsed[0] }));
    ...
    setBusy(true);
    try {
      await Promise.all(parsed.map((item) => saveVaultCredential(null, { ...emptyCredential, ...item })));
      stopScanner(); setScanText(""); setScanError(""); setModal(null);
      notify(`已导入 ${parsed.length} 条验证器数据`);
      await load(true);
    } finally {
      setBusy(false);
    }
  ...
  const openScanner = async () => {
    stopScanner(); setScanText(""); setScanError(""); setModal("scanner");
    ...
    try {
      ...
      const scan = async () => {
        ...
        try {
          ...
          const raw = result[0]?.rawValue || decodeQrWithCanvas(scanVideoRef.current);
          if (raw) return void applyScannedCredential(raw).catch((error) => setScanError(error instanceof Error ? error.message : "二维码内容无效"));
        } catch { /* keep scanning */ }
  ...
  const scanImage = async (file?: File) => {
    ...
    try {
      ...
      const raw = result[0]?.rawValue || decodeQrWithCanvas(bitmap);
      bitmap.close();
      if (!raw) throw new Error("图片中没有识别到二维码");
      await applyScannedCredential(raw);
  ...
  async function removeCredential(id: number) {
    if (!window.confirm("删除后不会影响已经创建的授权快照，确定删除？")) return;
    try { await deleteVaultCredential(id); notify("凭据已删除"); await load(true); } catch (error) { notify(error instanceof Error ? error.message : "删除失败", true); }
  }
  ...
  const displayAccount = (account: string) => prefs.masked ? account.replace(/^(.{1,2}).*(@.*)?$/, "$1••••$2") : account;
  ...
  return <div className="vault-page">
    <section className="vault-head">
      ...
      <div className="vault-head-actions"><button type="button" className="vault-ghost" onClick={() => { setLegacyText(""); setModal("import"); }} aria-label="导入文件"><FileUp size={14} /><span>导入</span></button><button type="button" className="vault-primary" onClick={() => openCredential()} aria-label="添加凭据"><Plus size={15} /><span>添加</span></button><button type="button" className="vault-ghost vault-logout" onClick={onLogout} aria-label="退出登录"><LogOut size={14} /><span>退出</span></button></div>
    </section>
    ...
    {view === "settings" ? <section className="vault-settings"><header className="vault-settings-hero"><span><Settings2 size={20} /></span><div><small>PERSONAL PREFERENCES</small><h2>按你的习惯显示</h2><p>这些偏好只保存在当前设备，不会修改或上传任何凭据信息。</p></div></header><div className="vault-settings-group"><header><div><b>界面显示</b><small>验证码与账号列表</small></div><span>3 项</span></header>{([['masked', EyeOff, '隐藏账号', '在列表中遮住账号主体', 'violet'], ['compact', LayoutGrid, '紧凑卡片', '缩小留白，一屏看到更多内容', 'blue'], ['grouped', Layers3, '按系统分组', '将同一系统的凭据排列在一起', 'green']] as const).map(([key, Icon, title, detail, tone]) => <label className="vault-setting-row" key={key}><span className={`vault-setting-icon is-${tone}`}><Icon size={17} /></span><span className="vault-setting-copy"><b>{title}</b><small>{detail}</small></span><input type="checkbox" checked={prefs[key]} onChange={(event) => setPrefs({ ...prefs, [key]: event.target.checked })} /><i /></label>)}</div><footer className="vault-settings-note"><ShieldCheck size={15} /><span><b>本地偏好</b><small>清除浏览器数据后会恢复默认显示。</small></span></footer></section> : null}
    ...
    {modal === "detail" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className="vault-modal small vault-detail">
      <header><div><small>CREDENTIAL DETAIL</small><h2>凭据详情</h2></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header>
      {detailLoading || !liveDetail ? <div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在安全读取…</div> : <>
        <div className="vault-detail-identity"><span className="vault-service-mark">{liveDetail.issuer.slice(0, 2).toUpperCase()}</span><div><b>{liveDetail.issuer}</b><small>{liveDetail.shared ? `${liveDetail.sharedBy} 分享 · ${liveDetail.shareExpireTime ? formatRemaining(liveDetail.shareExpireTime, now) : "临时有效"}` : `${liveDetail.sensitivityLevel === "HIGH" ? "高敏感" : liveDetail.sensitivityLevel === "SAFE" ? "敏感" : "普通"}标记`}</small></div></div>
        <div className="vault-detail-values">
        ...
        {liveDetail.loginUrl ? <a className="vault-detail-link" href={liveDetail.loginUrl} target="_blank" rel="noreferrer"><ExternalLink size={14} /><span>{liveDetail.loginUrl}</span></a> : null}
        {liveDetail.note ? <p className="vault-detail-note">{liveDetail.note}</p> : null}
        <footer><button type="button" className="vault-ghost" onClick={closeModal}>关闭</button>{!liveDetail.shared ? <><button type="button" className="vault-ghost" onClick={() => openShare(liveDetail.id)}><Share2 size={15} />分享</button><button type="button" className="vault-primary" onClick={() => openCredential(liveDetail)}>编辑凭据</button></> : null}</footer>
      </>}
    </section></div> : null}
    ...
    {modal === "import" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><form className="vault-modal share vault-share-form vault-import-form" onSubmit={submitImport}><header><div><small>IMPORT</small><h2>导入文件</h2><p>选择文件后会加密保存到当前账号</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header><div className="vault-share-scroll"><section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>选择文件</h3></div></div><p className="vault-section-help">支持包含 OTP 地址的文本文件</p><label className="vault-file"><FileUp size={20} /><span>{legacyText ? "文件已读取" : "选择文件"}</span><input type="file" accept=".txt,text/plain" onChange={async (event) => { const file = event.target.files?.[0]; if (file) setLegacyText(await file.text()); }} /></label></section></div><footer><span>{legacyText ? "文件已读取，可以开始导入" : "请选择要导入的文件"}</span><div><button type="button" className="vault-ghost" onClick={closeModal}>取消</button><button className="vault-primary" disabled={busy || !legacyText}>{busy ? "导入中" : "确认导入"}</button></div></footer></form></div> : null}

    {modal === "shareDetail" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className="vault-modal small vault-share-detail">
      <header><div><small>AUTHORIZATION DETAIL</small><h2>临时授权详情</h2></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header>
      {shareDetailLoading || !shareDetail ? <div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在读取授权配置…</div> : <>
        <div className="vault-share-detail-hero"><span className={`vault-status is-${shareDetail.status.toLowerCase()}`}>{shareDetail.status === "ACTIVE" ? "有效" : shareDetail.status === "EXPIRED" ? "已过期" : shareDetail.status === "LIMIT_REACHED" ? "次数已用完" : "已撤销"}</span><div><small>剩余有效时间</small><b>{shareDetail.status === "ACTIVE" ? formatRemaining(shareDetail.expireTime, now) : "授权已结束"}</b></div></div>
        <div className="vault-share-detail-grid"><section><span>授权内容</span><b>{shareDetail.itemCount} 个凭据</b></section><section><span>{shareDetail.shareMode === "DIRECT" ? "接收人" : "访问次数"}</span><b>{shareDetail.shareMode === "DIRECT" ? shareDetail.recipientUsername : `${shareDetail.accessCount}${shareDetail.maxAccessCount ? ` / ${shareDetail.maxAccessCount}` : " / 不限"}`}</b></section><section><span>访问方式</span><b>{shareDetail.shareMode === "DIRECT" ? "指定用户" : shareDetail.accessCodeEnabled ? "访问码验证" : "链接直达"}</b></section><section><span>复制权限</span><b>{shareDetail.allowCopy ? "允许复制" : "仅查看"}</b></section></div>
        <section className="vault-share-detail-section"><span>允许查看</span><div className="vault-permission-tags">{([["showAccount", "账号"], ["showPassword", "密码"], ["showOtp", "OTP"], ["showLoginUrl", "登录地址"], ["showNote", "备注"]] as const).filter(([key]) => shareDetail[key]).map(([, label]) => <b key={label}>{label}</b>)}</div></section>
        {shareDetail.sharePath ? <label><span>授权链接</span><div className="vault-copy-row"><input readOnly value={`${location.origin}${shareDetail.sharePath}`} /><button type="button" onClick={() => void copy(`${location.origin}${shareDetail.sharePath}`, "授权链接已复制")} aria-label="复制授权链接"><Copy size={15} /></button></div></label> : null}
        {shareDetail.shareMode === "DIRECT" ? <p className="vault-modal-note">该授权只会显示在 {shareDetail.recipientUsername} 的 OTP Vault 中，到期或撤销后自动消失。</p> : shareDetail.accessCodeEnabled ? <label><span>访问码</span>{shareDetail.accessCode ? <button type="button" className="vault-access-code vault-access-code-button" onClick={() => void copy(shareDetail.accessCode || "", "访问码已复制")}>{shareDetail.accessCode}<Copy size={15} /></button> : <p className="vault-modal-note">这是升级前创建的授权，当时只保存了不可逆哈希，无法还原访问码。请重新创建授权。</p>}</label> : <p className="vault-modal-note">此授权未启用访问码，获得链接即可访问。</p>}
        {shareDetail.shareMode === "LINK" ? <section className="vault-access-records"><header><div><span>访问记录</span><small>最近 50 条</small></div><b>{shareDetail.accessRecords?.length || 0}</b></header>{shareDetail.accessRecords?.length ? <div>{shareDetail.accessRecords.map((record, index) => <article key={`${record.createTime}-${index}`} className={record.success ? "is-success" : "is-failed"}><span>{record.success ? <ShieldCheck size={15} /> : <ShieldAlert size={15} />}</span><div><b>{accessActionLabel(record.action, record.success)}</b><small>{record.ipAddress || "未知 IP"} · {deviceLabel(record.userAgent)}</small>{record.detail ? <em>{record.detail}</em> : null}</div><time>{new Date(normalizeDateTime(record.createTime)).toLocaleString("zh-CN", { hour12: false })}</time></article>)}</div> : <p>还没有访问记录</p>}</section> : null}
        <footer><button type="button" className="vault-ghost" onClick={closeModal}>关闭</button>{shareDetail.status === "ACTIVE" ? <button type="button" className="vault-danger" onClick={async () => { await revokeVaultShare(shareDetail.id); notify("授权已撤销"); setModal(null); await load(true); }}>撤销授权</button> : null}</footer>
      </>}
    </section></div> : null}
