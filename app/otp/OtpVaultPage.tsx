...
function OtpVaultContent({ onLogout }: { onLogout: () => void }) {
  ...
  return <div className="vault-page">
    ...
    {view === "settings" ? <section className="vault-settings"><header className="vault-settings-hero"><span><Settings2 size={20} /></span><div><small>PERSONAL PREFERENCES</small><h2>按你的习惯显示</h2><p>这些偏好会跟随当前账号保存。</p></div></header><div className="vault-settings-group"><header><div><b>界面显示</b><small>验证码与账号列表</small></div><span>5 项</span></header>{([['masked', EyeOff, '隐藏账号', '在列表中遮住账号主体', 'violet'], ['compact', LayoutGrid, '紧凑卡片', '缩小留白，一屏看到更多内容', 'blue'], ['grouped', Layers3, '按系统分组', '将同一系统的凭据排列在一起', 'green'], ['showShared', User, '显示共享', '在全部列表中展示别人分享给我的凭据', 'blue'], ['autoRefresh', Clock3, '自动刷新', '定时同步授权状态和新增共享', 'green']] as const).map(([key, Icon, title, detail, tone]) => <label className="vault-setting-row" key={key}><span className={`vault-setting-icon is-${tone}`}><Icon size={17} /></span><span className="vault-setting-copy"><b>{title}</b><small>{detail}</small></span><input type="checkbox" checked={prefs[key]} onChange={(event) => updatePrefs({ ...prefs, [key]: event.target.checked })} /><i /></label>)}</div><footer className="vault-settings-note"><ShieldCheck size={15} /><span><b>账号偏好</b><small>换设备登录后会自动同步。</small></span></footer></section> : null}
    ...
    {modal === "detail" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className="vault-modal share vault-share-form vault-detail">
      <header><div><small>CREDENTIAL DETAIL</small><h2>凭据详情</h2><p>{liveDetail ? `${liveDetail.issuer} · ${displayAccount(liveDetail.accountName)}` : "正在安全读取"}</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header>
