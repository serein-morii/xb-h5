...
export default function AdminOrderEntry({ username, notify }: { username: string; notify: (message: string, type?: "success" | "error" | "info") => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiRequest<{ data?: StoreRow[] }>(`${API_PATHS.stores.root}/options`, { query: { createBy: "", name: "" } }),
      apiRequest<{ data?: { products?: Option[]; sizes?: Option[] } }>(`${API_PATHS.content.search}/order-options`, { auth: false }),
      apiRequest<{ data?: Row[] }>(`${API_PATHS.logistics.shipments}/companies`),
  ...
  async function createPurchaser() {
    ...
    try {
      const result = await apiRequest<{ data?: Purchaser }>(API_PATHS.customers.purchasers, { method: "POST", body: { name: createForm.name.trim(), phone: createForm.phone, storeCode: createForm.storeCode } });
  ...
  async function parseAddress(text = pasteText) {
    ...
    try {
      const result = await apiRequest<{ data?: Row[] }>(`${API_PATHS.content.search}/addr`, { auth: false, query: { addr: text.trim() } });
  ...
  async function detectExpress(expCode = form.expCode) {
    ...
    try {
      const result = await apiRequest<{ data?: Row }>(`${API_PATHS.logistics.shipments}/companies/match`, { query: { expCode: expCode.trim() } });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await apiRequest(API_PATHS.orders.root, { method: "POST", body });
      let latest: Row = { purchaser: selectedPurchaser.name, store: selectedPurchaser.storeName };
      try {
        const result = await apiRequest<{ data?: Row[] }>(`${API_PATHS.content.search}/purchaser/orders`, { auth: false, query: { id: selectedPurchaser.shortId } });
