...
export async function fetchAccessManifest(): Promise<AccessManifest> {
  try {
    const result = await apiRequest<{ data?: AccessManifest }>(`${API_PATHS.identity.access}/manifest`);
    if (result.data && Array.isArray(result.data.capabilities)) return result.data;
  } catch (error) {
    if (!(error instanceof Error)) throw error;
  }
  return fromLegacyGetInfo(await apiRequest<DataRow>(API_PATHS.auth.getInfo));
}
