...
export function OrderEditor({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiRequest<{ data?: DataRow[] }>(`${API_PATHS.stores.root}/options`, { query: { createBy: "", name: "" } }),
      apiRequest<{ data?: ProductCatalogRow[] }>(API_PATHS.catalog.products),
  ...
  async function createAndSelectPurchaser() {
    ...
    try {
      const result = await apiRequest<{ data?: DataRow }>(API_PATHS.customers.purchasers, { method: "POST", body: { name: purchaserForm.name.trim(), phone: purchaserForm.phone, storeCode: purchaserForm.storeCode } });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await apiRequest(API_PATHS.orders.root, { method: form.id ? "PUT" : "POST", body: { ...form, orderNum: Number(form.orderNum || 1), salePrice } });
...
export function ShippingEditor({
  ...
  async function detectExpress(codeToCheck?: string) {
    ...
    setDetecting(true);
    try {
      const result = await apiRequest<{ data?: DataRow }>(`${API_PATHS.logistics.shipments}/companies/match`, { query: { expCode: code } });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await apiRequest(API_PATHS.orders.root, { method: "PUT", body: updated });
...
export function OrdersPage({ notify, onNavigate }: { notify: (message: string, type?: "success" | "error" | "info") => void; onNavigate?: (key: MenuKey) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [result, stats] = await Promise.all([
        apiRequest<DataRow>(API_PATHS.orders.root, { query: filters }),
        apiRequest<{ data?: DataRow }>(`${API_PATHS.orders.root}/stats`),
      ]);
  ...
  async function refreshLoadedRange(expectedDelta = 0) {
    ...
    try {
      while (pageNum <= 200) {
        ...
        const result = await apiRequest<DataRow>(API_PATHS.orders.root, {
      ...
      const stats = await apiRequest<{ data?: DataRow }>(`${API_PATHS.orders.root}/stats`);
      const statsData = (stats.data && typeof stats.data === "object" ? stats.data : {}) as DataRow;
      setCounts({
        pending: Number(statsData.pending || 0),
        shipping: Number(statsData.waiting || 0),
  ...
  async function getDetail(row: DataRow) {
    try {
      const result = await apiRequest<DataRow>(`${API_PATHS.orders.root}/${row.id}`);
  ...
  async function openShipping(row?: DataRow) {
    ...
    try { const result = await apiRequest<DataRow>(`${API_PATHS.orders.root}/${target.id}`); setShipping(result.data || target); }
  ...
  function action(path: string, actionIds: string, success: string) {
    return async () => {
      try { await apiRequest(`${API_PATHS.orders.root}/${path}/${actionIds}`, { method: "PATCH" }); notify(success, "success"); setSelected(new Set()); await refreshLoadedRange(); }
  ...
  async function runBatchSequential(path: string, label: string, idArr: string[]) {
    ...
    for (const id of idArr) {
      try {
        await apiRequest(`${API_PATHS.orders.root}/${path}/${id}`, { method: "PATCH" });
        success += 1;
      } catch (error) {
        failed += 1;
  ...
  function requestBatch(path: string, label: string, row?: DataRow) {
    ...
    const sequential = ["send", "finish", "to-send"].includes(path);
  ...
  function requestDelete(row?: DataRow) {
    ...
    setConfirm({ title: "删除订单", message: `删除后无法恢复，确认删除 ${row ? 1 : selected.size} 个订单？`, danger: true, action: async () => { await apiRequest(`${API_PATHS.orders.root}/${target}`, { method: "DELETE" }); notify("删除成功", "success"); setSelected(new Set()); await refreshLoadedRange(row ? -1 : -selected.size); } });
  }
  ...
  async function submitBatchSalePrice(event: FormEvent) {
    ...
    try {
      await apiRequest(`${API_PATHS.orders.root}/sale-price`, { method: "PATCH", body: { ids: targetIds, salePrice } });
  ...
  async function markPay(kind: "paid" | "unpaid", row?: DataRow) {
    ...
    for (const id of idList) {
      try {
        await apiRequest(`${API_PATHS.orders.root}/${path}`, { method: "PATCH", body: { ids: [id] } });
        success += 1;
      } catch (error) {
        failed += 1;
        if (!firstErrorMsg) firstErrorMsg = error instanceof Error ? error.message : "操作失败";
  ...
  async function refreshLogisticsAll() {
    ...
    for (const target of targets) {
      ...
      if (!orderCode) {
        ...
        try {
          await apiRequest(`${API_PATHS.logistics.shipments}/refresh`, {
            method: "PATCH",
            body: [orderCode],
            timeoutMs: 55_000,
          });
  ...
  async function loadAllOrders() {
    ...
    try {
      while (pageNum <= maxPages) {
        const result = await apiRequest<DataRow>(API_PATHS.orders.root, { query: { ...filters, pageNum, pageSize } });
        const pageRows = Array.isArray(result.rows) ? result.rows : [];
        serverTotal = Number(result.total || 0);
        accumulated.push(...pageRows);
