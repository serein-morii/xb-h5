...
const MODULES: Record<Exclude<ModuleKey, "dictData">, ModuleConfig> = {
  users: {
    ...
    icon: Users,
    listPath: API_PATHS.identity.users,
    resourcePath: API_PATHS.identity.users,
    idKey: "userId",
    nameKey: "userName",
  ...
  roles: {
    ...
    icon: ShieldCheck,
    listPath: API_PATHS.identity.roles,
    resourcePath: API_PATHS.identity.roles,
    idKey: "roleId",
    nameKey: "roleName",
  ...
  depts: {
    ...
    icon: Building2,
    listPath: API_PATHS.identity.departments,
    resourcePath: API_PATHS.identity.departments,
    idKey: "deptId",
    nameKey: "deptName",
  ...
  posts: {
    ...
    listPath: API_PATHS.identity.positions,
    resourcePath: API_PATHS.identity.positions,
    idKey: "postId",
    nameKey: "postName",
  ...
  menus: {
    ...
    icon: MenuIcon,
    listPath: API_PATHS.identity.menus,
    resourcePath: API_PATHS.identity.menus,
    idKey: "menuId",
    nameKey: "menuName",
  ...
  dictTypes: {
    ...
    icon: BookKey,
    listPath: API_PATHS.administration.dictionaryTypes,
    resourcePath: API_PATHS.administration.dictionaryTypes,
    idKey: "dictId",
    nameKey: "dictName",
    defaults: { status: "0" },
    search: [
    ...
    cachePath: `${API_PATHS.administration.dictionaryTypes}/cache`,
  },
  configs: {
    key: "configs",
    title: "参数设置",
    ...
    icon: Settings2,
    listPath: API_PATHS.administration.config,
    resourcePath: API_PATHS.administration.config,
    idKey: "configId",
    nameKey: "configName",
    ...
    cachePath: `${API_PATHS.administration.config}/cache`,
  },
  notices: {
    key: "notices",
    title: "通知公告",
    ...
    icon: Bell,
    listPath: API_PATHS.administration.notices,
    resourcePath: API_PATHS.administration.notices,
    idKey: "noticeId",
    nameKey: "noticeTitle",
...
function dictDataConfig(dictType: string): ModuleConfig {
  return {
    ...
    icon: ListTree,
    listPath: API_PATHS.administration.dictionaryEntries,
    resourcePath: API_PATHS.administration.dictionaryEntries,
    idKey: "dictCode",
    nameKey: "dictLabel",
...
function ResourceModule({
  ...
  async function loadUserMeta(userId?: unknown) {
    const suffix = userId === undefined ? "/form-options" : `/${userId}`;
    try {
      const result = await apiRequest<DataRow>(`${API_PATHS.identity.users}${suffix}`);
  ...
  async function changeStatus(row: DataRow) {
    const path = config.key === "users" ? `${API_PATHS.identity.users}/status` : `${API_PATHS.identity.roles}/status`;
  ...
  async function openAuthRoles(row: DataRow) {
    setAuthUser(row);
    try {
      const result = await apiRequest<DataRow>(`${API_PATHS.identity.users}/${row.userId}/roles`);
  ...
  async function saveAuthRoles() {
    if (!authUser) return;
    try {
      await apiRequest(`${API_PATHS.identity.users}/roles`, {
        method: "PUT",
  ...
  async function openMenuPermissions(row: DataRow) {
    setMenuRole(row);
    try {
      const result = await apiRequest<DataRow>(`${API_PATHS.identity.menus}/roles/${row.roleId}/tree`);
  ...
  async function saveMenuPermissions() {
    if (!menuRole) return;
    try {
      await apiRequest(API_PATHS.identity.roles, { method: "PUT", body: { ...menuRole, menuIds: [...menuIds] } });
  ...
  async function openDataPermissions(row: DataRow) {
    ...
    try {
      const result = await apiRequest<DataRow>(`${API_PATHS.identity.departments}/roles/${row.roleId}/tree`);
  ...
  async function saveDataPermissions() {
    if (!dataRole) return;
    try {
      await apiRequest(`${API_PATHS.identity.roles}/data-scope`, {
        method: "PUT",
        body: {
  ...
  async function openRoleUsers(row: DataRow) {
    ...
    try {
      const result = await apiRequest<DataRow>(`${API_PATHS.identity.roles}/users/allocated`, {
  ...
  async function loadAvailableRoleUsers() {
    ...
    setRoleUsersLoading(true);
    try {
      const result = await apiRequest<DataRow>(`${API_PATHS.identity.roles}/users/unallocated`, {
  ...
  async function cancelRoleUser(row: DataRow) {
    if (!roleUsersRole) return;
    try {
      await apiRequest(`${API_PATHS.identity.roles}/users/cancel`, {
        method: "PUT",
  ...
  async function saveRoleUsers() {
    ...
    try {
      await apiRequest(`${API_PATHS.identity.roles}/users/select-all`, {
        method: "PUT",
  ...
  async function exportUsers() {
    try {
      await downloadFile(`${API_PATHS.identity.users}/export`, filters, `users_${Date.now()}.xlsx`);
  ...
  async function downloadUserTemplate() {
    try {
      await downloadFile(`${API_PATHS.identity.users}/import-template`, {}, `user_template_${Date.now()}.xlsx`);
  ...
  async function importUsers(file: File) {
    try {
      const result = await uploadFile(`${API_PATHS.identity.users}/import`, file, { updateSupport: false }) as DataRow;
  ...
  return (
    <div className="module-page sysm-center">
      ...
      {resetUser && access.has("system.users.resetPassword") ? (
        ...
        <div className="form-footer"><button className="button button-ghost" type="button" onClick={() => setResetUser(null)}>取消</button><button className="button button-primary" type="button" onClick={async () => { if (!newPassword) return notify("请输入新密码", "info"); try { await apiRequest(`${API_PATHS.identity.users}/password/reset`, { method: "PUT", body: { userId: resetUser.userId, password: newPassword } }); notify("密码已重置", "success"); setResetUser(null); } catch (error) { notify(error instanceof Error ? error.message : "重置失败", "error"); } }}>确认重置</button></div></Sheet>
      ) : null}
