...
export default function CustomerAuthPage({ mode }: { mode: Mode }) {
  ...
  useEffect(() => {
    ...
    setError("");
    apiRequest<{ data?: Context }>(`${API_PATHS.customers.root}/register-context`, { auth: false, query: { shortId } })
      .then((result) => {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === "login") {
        ...
        const result = await apiRequest<{ data?: AuthData }>(`${API_PATHS.customers.root}/auth/register`, {
          auth: false,
          method: "POST",
        ...
        await apiRequest(`${API_PATHS.customers.root}/auth/reset`, {
          auth: false,
          method: "POST",
          body: { email, code, password },
        });
        clearCustomerToken();
        window.location.href = shortId ? `${APP_ROUTES.customerLogin}?shortId=${shortId}&reset=success` : `${APP_ROUTES.customerLogin}?reset=success`;
      }
    } catch (cause) {
  ...
  function finishLogin(data?: AuthData) {
    ...
    window.location.href = `${APP_ROUTES.toolOrderDetail}/${data.purchaserShortId}`;
  }
  ...
  return (
    <main className="customer-auth-page">
      <a className="customer-auth-brand" href={APP_ROUTES.home}><b>炎陵黄桃</b><span>返回主页</span></a>
          ...
          {mode === "register" && registerPreview?.confirmRequired ? (
            <section className="customer-register-confirm">
              ...
              {registerPreview.registered && registerPreview.matchType === "phone"
                ? <a href={APP_ROUTES.customerLogin}>去登录</a>
