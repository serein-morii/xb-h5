...
export default function PublicOrder({ embedded = false }: { embedded?: boolean }) {
  ...
  async function startOnlinePay(payMethod: "wx" | "alipay", refresh = false) {
    ...
    if (!orderCode || !signId || payBusyMethod) return;
    setLastPayMethod(payMethod);
    setPayBusyMethod(payMethod); setOnlinePayError("");
    try {
  ...
  return <div className="tool-page signed-order-tool">
    ...
    {payOpen && order ? <div className="purchaser-help-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setPayOpen(false); }}>
      <section className="purchaser-sheet purchaser-online-pay-sheet">
        ...
        <div className="purchaser-online-pay-channels" role="radiogroup" aria-label="支付方式">
          {wxEnabled ? <button type="button" className="pay-channel is-wx" disabled={payBusyMethod !== null} onClick={() => void startOnlinePay("wx")}>
            <span className="pay-channel-icon">{payBusyMethod === "wx" ? <LoaderCircle className="spin" size={20} /> : <Wallet size={21} />}</span>
            <span className="pay-channel-copy"><b>微信支付</b><small>拉起微信完成付款</small></span>
            <ChevronRight size={17} />
          </button> : null}
          {alipayEnabled ? <button type="button" className="pay-channel is-alipay" disabled={payBusyMethod !== null} onClick={() => void startOnlinePay("alipay")}>
