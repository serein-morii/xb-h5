  ...
  return <>
    ...
    <section className="tool-order-results soft-list">{visible.map((order) => {
      ...
      return <article key={order.id} className="soft-card">
        ...
        {order.signId || (isPending && (onEdit || onDelete)) || onView || (onPay && isPayable) ? <>
          {order.signId ? <div className="tool-order-actions"><button type="button" className="tool-edit tool-pay-entry" onClick={() => void copyToClipboard(`${window.location.origin}/tools/order#${encodeURIComponent(String(order.signId))}`).then((ok) => { if (ok) { setCopied(true); window.setTimeout(() => setCopied(false), 1800); } })}><Copy size={12} /><span>复制付款链接</span></button></div> : null}
