...
export default function BatchOrderEntry() {
  ...
  useEffect(() => {
    apiRequest<{ data?: Row[] }>(`${API_PATHS.stores.root}/options`, { query: { createBy: "", name: "" } })
      .then((r) => {
    ...
    apiRequest<{ data?: { products?: DictOption[]; sizes?: DictOption[] } }>(`${API_PATHS.content.search}/order-options`, { auth: false })
      .then((r) => {
  ...
  async function runPreview(parsed: Row[]) {
    ...
    try {
      const res = await apiRequest<{ data?: PreviewResponse }>(`${API_PATHS.orders.batches}/preview`, {
        method: "POST",
  ...
  async function loadPurchasers(): Promise<PurchaserOption[]> {
    ...
    try {
      const res = await apiRequest<{ data?: any[] }>(API_PATHS.customers.purchasers, { query: { storeCode } });
  ...
  async function confirmSubmit() {
    ...
    try {
      ...
      const result = await apiRequest<{ data?: BatchResponse }>(`${API_PATHS.orders.batches}/submit`, {
        method: "POST",
