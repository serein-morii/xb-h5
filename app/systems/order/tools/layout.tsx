  ...
  return <div className="public-tools-shell">
    ...
    {!isDedicatedOrderLink ? <aside className={`tools-login-float${actionsOpen ? " is-open" : ""}`}>
      {actionsOpen ? <div className="tools-float-actions">
        {showBackToTools ? <a className="tools-back-trigger" href={APP_ROUTES.tools} aria-label="返回工具箱" title="返回工具箱"><ArrowLeft size={17} /></a> : null}
        <a className="tools-login-action" href={APP_ROUTES.manage} aria-label="登录管理后台" title="登录管理后台"><LogIn size={17} /></a>
      </div> : null}
