...
export default function OrderLinkGenerator({ embedded = false }: { embedded?: boolean }) {
  ...
  async function selectPurchaser(purchaser: Purchaser) {
    ...
    if (!purchaser.storeId) {
      const result = await apiRequest<{ data?: Purchaser }>(`${API_PATHS.customers.purchasers}/${purchaser.id}/store`, { method: "PUT", body: { storeCode } });
  ...
  async function createPurchaser() {
    ...
    const result = await apiRequest<{ data?: Purchaser }>(API_PATHS.customers.purchasers, { method: "POST", body: { name: name.trim(), phone: phone.trim(), storeCode } });
  ...
  async function searchPurchaser(event: FormEvent) {
    ...
    try {
      const result = await apiRequest<{ data?: Candidate[] }>(`${API_PATHS.customers.purchasers}/match`, { query: { name: name.trim() } });
  ...
  if (!authenticated && !embedded) return <div className="tool-page"><section className="tool-hero"><span><Link2 size={25} /></span><div><small>PRIVATE LINK CREATOR</small><h1>生成链接</h1><p>该功能会检索买家档案，需要先登录管理后台。</p></div></section><section className="order-link-login"><User size={28} /><h2>请先登录</h2><p>登录后才能匹配买家、查看历史订单并生成专属链接。</p><a href={APP_ROUTES.manage}>前往管理登录</a></section></div>;
