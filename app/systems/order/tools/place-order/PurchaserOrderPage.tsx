...
export default function PurchaserOrderPage() {
  ...
  const refreshCustomerToken = useCallback(async (purchaserId: string) => {
    ...
    clearCustomerToken();
    const result = await apiRequest<{ data?: LinkContext & { token?: string; registered?: boolean; phoneRequired?: boolean } }>(`${API_PATHS.customers.root}/auth/short-id`, {
      auth: false,
      method: "POST",
  ...
  const loadCustomerProfile = useCallback(async () => {
    ...
    try {
      const result = await customerApiRequest<{ data?: CustomerProfile }>(`${API_PATHS.customers.root}/auth/profile`, {}, linkKey.purchaserId);
  ...
  async function toggleCustomerQuickLogin() {
    ...
    try {
      const result = await customerApiRequest<{ data?: CustomerProfile }>(`${API_PATHS.customers.root}/auth/quick-login`, { method: "PUT", body: { enabled } }, linkKey.purchaserId);
      ...
      if (enabled === 0) {
        ...
        clearCustomerToken();
        window.setTimeout(() => { window.location.href = `${APP_ROUTES.customerLogin}?shortId=${linkKey.purchaserId}`; }, 700);
  ...
  async function sendCustomerProfileCode(action: "current" | "new-email") {
    ...
    setProfileBusy(true); setProfileError("");
    try {
      await customerApiRequest(`${API_PATHS.customers.root}/auth/profile-code`, { method: "POST", body: { action, ...(action === "new-email" ? { newEmail: emailDraft.email.trim() } : {}) } }, linkKey.purchaserId);
  ...
  async function saveCustomerPhone() {
    ...
    setProfileBusy(true); setProfileError("");
    try {
      const result = await customerApiRequest<{ data?: CustomerProfile }>(`${API_PATHS.customers.root}/auth/profile`, { method: "PUT", body: { action: "phone", phone: phoneDraft.phone, currentCode: phoneDraft.code } }, linkKey.purchaserId);
  ...
  async function saveCustomerEmail() {
    ...
    setProfileBusy(true); setProfileError("");
    try {
      const result = await customerApiRequest<{ data?: CustomerProfile }>(`${API_PATHS.customers.root}/auth/profile`, { method: "PUT", body: { action: "email", newEmail: emailDraft.email.trim(), currentCode: emailDraft.currentCode, newCode: emailDraft.newCode } }, linkKey.purchaserId);
  ...
  async function ensureMineRegisterConfirmation() {
    ...
    try {
      const result = await apiRequest<{ data?: RegisterPreview }>(`${API_PATHS.customers.root}/auth/register-preview`, {
        auth: false,
        method: "POST",
  ...
  async function sendMineRegisterCode() {
    ...
    setProfileError("");
    try {
      await apiRequest(`${API_PATHS.customers.root}/auth/code`, {
        auth: false,
        method: "POST",
  ...
  async function submitMineRegister(event: FormEvent) {
    ...
    try {
      const result = await apiRequest<{ data?: CustomerAuthData }>(`${API_PATHS.customers.root}/auth/register`, {
        auth: false,
        method: "POST",
        body: {
  ...
  async function toggleOnboardingQuickLogin() {
    ...
    setProfileError("");
    try {
      const result = await customerApiRequest<{ data?: CustomerProfile }>(`${API_PATHS.customers.root}/auth/quick-login`, {
        method: "PUT",
        body: { enabled },
      }, linkKey.purchaserId);
  ...
  function logoutCustomer() {
    ...
    window.setTimeout(() => {
      window.location.href = `${APP_ROUTES.customerLogin}?shortId=${linkKey.purchaserId}`;
    }, 350);
  }
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      ...
      try {
        sessionResult = await apiRequest<{ data?: SessionData }>(`${API_PATHS.customers.root}/auth/short-id`, {
          auth: false,
      ...
      const [contextResult, optionsResult, catalogResult] = await Promise.all([
        apiRequest<{ data?: LinkContext }>(`${API_PATHS.content.search}/order-link`, { auth: false, query: parsed }),
        apiRequest<{ data?: { products?: Option[]; sizes?: Option[] } }>(`${API_PATHS.content.search}/order-options`, { auth: false }),
        apiRequest<{ data?: CatalogProduct[] }>(`${API_PATHS.customers.root}/products`, { auth: false }).catch((cause) => {
  ...
  async function loadAddresses(token: string) {
    ...
    setAddressBookError("");
    try {
      const result = await customerApiRequest<{ data?: PurchaserAddressRecord[] }>(`${API_PATHS.content.search}/purchaser/addresses`, {
  ...
  async function submitAddressAuth() {
    ...
    setAddressBookError("");
    try {
      const result = await customerApiRequest<{ data?: { token?: string } }>(`${API_PATHS.content.search}/purchaser/address-session`, {
        method: "POST",
        body: {
  ...
  async function saveAddressRecord() {
    ...
    try {
      ...
      await customerApiRequest(`${API_PATHS.content.search}/purchaser/address${isEdit ? `/${addressDraft.id}` : ""}`, {
  ...
  async function deleteAddressRecord() {
    ...
    setAddressBookError("");
    try {
      await customerApiRequest(`${API_PATHS.content.search}/purchaser/address/${addressDeleteTarget.id}`, {
        method: "DELETE",
  ...
  async function parseAddress() {
    ...
    try {
      const result = await apiRequest<{ data?: Row[] }>(`${API_PATHS.content.search}/addr`, { auth: false, query: { addr: pasteText.trim() } });
  ...
  async function parseAddressIntoDraft() {
    ...
    setAddressBookError("");
    try {
      const result = await apiRequest<{ data?: Row[] }>(`${API_PATHS.content.search}/addr`, { auth: false, query: { addr: addressDraftPasteText.trim() } });
      const parsed = Array.isArray(result.data) ? result.data[0] : null;
      if (!parsed) throw new Error("没有识别到有效地址，请手动填写");
      const fullAddress = String(parsed.allAddress || [parsed.province, parsed.city, parsed.area, parsed.detail, parsed.address].filter(Boolean).join(""));
  ...
  async function submitOrder() {
    ...
    try {
      ...
      const result = await customerApiRequest<{ data?: Row }>(`${API_PATHS.content.search}/order`, { method: "POST", body }, linkKey.purchaserId);
  ...
  async function submitEdit() {
    ...
    try {
      ...
      await customerApiRequest(`${API_PATHS.content.search}/order/${editingOrder.id}`, { method: "PUT", query: { purchaserShortId: linkKey.purchaserId }, body }, linkKey.purchaserId);
  ...
  async function submitDelete() {
    ...
    try {
      await customerApiRequest(`${API_PATHS.content.search}/order/${confirmingDelete.id}`, { method: "DELETE", query: { purchaserShortId: linkKey.purchaserId } }, linkKey.purchaserId);
        ...
        {mineRegisterPreview?.confirmRequired ? (
          <section className="purchaser-mine-register-confirm">
            ...
            {mineRegisterPreview.registered && mineRegisterPreview.matchType === "phone"
              ? <button type="button" onClick={() => { window.location.href = `${APP_ROUTES.customerLogin}?shortId=${linkKey.purchaserId}`; }}>去登录</button>
        ...
        <section className={`purchaser-success-modal${successNeedsPayment ? " payment-flow" : ""}`} role="alertdialog" aria-modal="true" aria-labelledby="purchaser-success-title">
          ...
          {successNeedsPayment ? (
            <div className="purchaser-payment-panel">
              ...
              {Number(linkContext.payEnabled) === 1 ? (
