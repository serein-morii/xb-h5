...
export default function PurchaserManager({ embedded = false }: { embedded?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [purchaserResult, storeResult] = await Promise.all([
        apiRequest<{ data?: Purchaser[] }>(API_PATHS.customers.purchasers),
        apiRequest<{ data?: StoreRow[] }>(`${API_PATHS.stores.root}/options`),
      ]);
  ...
  async function bind(item: Purchaser) {
    ...
    try {
      await apiRequest(`${API_PATHS.customers.purchasers}/${item.id}/store`, { method: "PUT", body: { storeCode } });
  ...
  async function unbind(item: Purchaser) {
    setBusyId(item.id); setError("");
    try {
      await apiRequest(`${API_PATHS.customers.purchasers}/${item.id}/store/unbind`, { method: "PATCH" });
  ...
  async function saveCostPriceConfig() {
    ...
    try {
      await apiRequest(`${API_PATHS.customers.purchasers}/${costPriceTarget.id}/cost-price`, {
        method: "PUT",
        body: {
  ...
  async function saveCodeConfig() {
    ...
    try {
      await apiRequest(`${API_PATHS.customers.purchasers}/${codeTarget.id}/order-code`, {
        method: "PUT",
  ...
  async function createPurchaser() {
    ...
    try {
      await apiRequest(API_PATHS.customers.purchasers, { method: "POST", body: { name: createForm.name.trim(), phone: createForm.phone, storeCode: createForm.storeCode } });
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await apiRequest(`${API_PATHS.customers.purchasers}/${editTarget.id}`, {
        method: "PUT",
        body: {
  ...
  async function submitDelete() {
    ...
    try {
      await apiRequest(`${API_PATHS.customers.purchasers}/${confirmingDelete.id}`, { method: "DELETE" });
  ...
  async function toggleBlock(item: Purchaser, field: "blockOrder" | "blockQuery", next: 0 | 1) {
    ...
    try {
      ...
      await apiRequest(`${API_PATHS.customers.purchasers}/${item.id}/block`, { method: "PUT", body });
    } catch (cause) {
  ...
  async function toggleAddressVerify(item: Purchaser, next: 0 | 1) {
    ...
    setBlockBusyId(item.id); setError("");
    try {
      await apiRequest(`${API_PATHS.customers.purchasers}/${item.id}/address-verify`, { method: "PUT", body: { addressVerifyEnabled: next } });
    } catch (cause) {
  ...
  async function toggleQueryExpRefresh(item: Purchaser, next: 0 | 1) {
    ...
    try {
      ...
      await apiRequest(`${API_PATHS.customers.purchasers}/${item.id}/query-exp-refresh`, {
        method: "PUT",
  ...
  async function setBlockDisplay(item: Purchaser, next: BlockDisplay) {
    ...
    setBlockBusyId(item.id); setError("");
    try {
      await apiRequest(`${API_PATHS.customers.purchasers}/${item.id}/block`, { method: "PUT", body: { blockDisplayType: next } });
    } catch (cause) {
  ...
  async function updateCustomerSetting(item: Purchaser, field: "accountRequired" | "paymentRequired", next: 0 | 1) {
    ...
    setBlockBusyId(item.id); setError("");
    try {
      const result = await apiRequest<{ data?: Purchaser }>(`${API_PATHS.customers.purchasers}/${item.id}/customer-settings`, { method: "PUT", body: { accountRequired, paymentRequired } });
  ...
  if (!authenticated && !embedded) return <div className="tool-page"><section className="tool-hero"><span><Users size={25} /></span><div><small>PURCHASER MANAGEMENT</small><h1>买家管理</h1><p>查看、绑定或解绑买家的所属店铺。</p></div></section><section className="order-link-login"><User size={28} /><h2>请先登录</h2><p>店铺绑定属于管理操作，登录后才可以查看和修改。</p><a href={APP_ROUTES.manage}>前往管理登录</a></section></div>;
