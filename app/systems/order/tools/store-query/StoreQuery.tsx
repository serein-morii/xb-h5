...
function resolveStoreCodeFromPath(): string {
  ...
  const prefix = `${APP_ROUTES.toolStoreQuery}/`;
...
export default function StoreQuery({ storeCode: storeCodeProp, onResolvedName }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const result = await apiRequest<{ data?: PublicOrderRecord[] }>(API_PATHS.content.search, {
        auth: false,
        method: "POST",
        body: {
  ...
  return <div className="tool-page order-search-tool store-query-tool">
    <a className="store-query-back" href={APP_ROUTES.toolStoreQuery}>
      <ArrowLeft size={16} />
      <span>返回店铺列表</span>
    </a>
