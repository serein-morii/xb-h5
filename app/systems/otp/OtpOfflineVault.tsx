...
import { decryptVaultBackup, generateOfflineCode, readOfflineVault, tryUnlockOfflineVault } from "./vaultCrypto";
...
export default function OtpOfflineVault({ onExit }: { onExit: () => void }) {
  ...
  useEffect(() => {
    void tryUnlockOfflineVault().then((backup) => { if (backup?.items?.length) setItems(backup.items); }).catch(() => undefined);
  }, []);
  useEffect(() => {
