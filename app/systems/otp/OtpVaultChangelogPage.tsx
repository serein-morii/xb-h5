...
const changelog = [
  {
    ...
    items: [
      ...
      "新增密码生成器，可调整长度和字符类型，并即时显示密码强度",
      "安全中心新增密码安全体检，在浏览器本地检查弱密码、重复密码和未启用动态口令的账号",
      "批量导入支持浏览器与 1Password 密码 CSV、未加密 Bitwarden JSON，以及原有 OTP 格式，会先预览新增、重复和冲突",
      "读取和准备大批量导入时显示进度，不再长时间无反馈",
      "开启零知识时导入只上传客户端密文，明文不会离开浏览器",
      "通知增加系统通知，可与邮件、Bark 一起按事件勾选；通知页会核对本机与服务器订阅状态",
      "补充 iPhone 主屏幕安装和桌面浏览器授权说明，密钥轮换后会提示重新订阅",
      "更换通知邮箱需要验证码；注销账号会清掉推送订阅",
      "凭据筛选可按动态口令、登录密码或安全笔记过滤，卡片会显示标签",
    ],
  },
  {
    date: "2026-09-14",
