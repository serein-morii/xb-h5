...
const changelog = [
  {
    ...
    title: "短信 / 邮箱验证码自动接收",
    items: [
      "新增接收通道：iPhone 快捷指令、邮件自动化转发和通用 Webhook，可把收到的验证码自动推进保险库",
      "来源组按发送方、正文关键词和收件账号匹配，验证码会自动贴到对应凭据卡片",
      "卡片显示来源图标和剩余时间横条，最新验证码带「新」标记，点按即可复制",
      "历史验证码加密保留 30 天，在凭据详情里分页查看",
      "凭据详情改版：接收验证码与验证器同款大号展示，来源移到「下一组」位置，访问地址并入登录信息",
      "接收通道支持随时重命名",
      "分享页只分享一条凭据时自动展开账号密码，大块卡片在右下角显示剩余时间",
      "倒计时统一按 X分X秒 显示，等待验证码的占位文字适当缩小",
      "修复创建来源组时页面整页刷新回启动画面的问题",
    ],
  },
  {
    date: "2026-09-15",
