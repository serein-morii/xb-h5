    ...
    <article className="otp-guide-doc">
      ...
      <section id="use" className="otp-guide-chapter">
        ...
        <div className="otp-guide-prose">
          ...
          <h3>自动接收短信 / 邮箱验证码</h3>
          <p>在添加凭据时打开「接收短信、邮件或 Webhook 验证码」，保存后在凭据详情点「来源」完成两步配置：先创建接收通道（iPhone 快捷指令、邮件自动化转发或通用 Webhook），再新建来源组选择通道并设置匹配规则。</p>
          <ul>
            <li>手机收到验证码后会自动贴到命中的凭据卡片，带「新」标记，点按即可复制。</li>
            <li>卡片用来源图标和剩余时间横条表示接收状态，历史验证码加密保留 30 天。</li>
            <li>通道创建一次可复用；配置步骤见通道详情里的「教程」。</li>
          </ul>
        </div>
      </section>
