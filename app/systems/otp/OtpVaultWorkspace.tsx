...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const renderCredential = (item: VaultCredential) => {
    ...
    return <article className={`vault-card${prefs.compact ? " is-compact" : ""}${item.shared ? " is-shared" : ""}`} key={item.id} role="button" tabIndex={0} onClick={() => void openDetail(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openDetail(item); } }}>
      ...
      <div className="vault-progress"><i style={{ width: `${progress}%` }} /></div><div className="vault-card-foot"><span>{item.shared ? `来自 ${item.sharedBy}` : `${item.otpType || "TOTP"} · ${item.algorithm} · ${item.digits} 位`}</span>{nextOtpLabel && !item.shared && item.otpType !== "HOTP" && !item.requiresStepUp ? <span className="vault-card-next"><small>下一组</small><b>{concealed ? "••• •••" : nextOtpLabel}</b><button type="button" className="vault-next-copy" onClick={(event) => { event.stopPropagation(); void copy(item.nextOtp || "", "下一组验证码已复制"); }} aria-label="复制下一组验证码"><Copy size={11} /></button></span> : null}<span>{item.shared && item.shareExpireTime ? formatRemaining(item.shareExpireTime, now) : item.requiresStepUp ? "已锁定" : item.otpType === "HOTP" ? `计数 ${item.hotpCounter || 0}` : item.currentOtp ? `${left}s` : "无 OTP"}</span></div>
      ...
      {detailLoading || !liveDetail ? <div className="vault-share-scroll"><div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在安全读取…</div></div> : <>
        <div className="vault-share-scroll">
          ...
          <section className="vault-share-section vault-detail-section"><div className="vault-section-title"><div><span>01</span><h3>登录信息</h3></div><small>{liveDetail.shared ? `${liveDetail.sharedBy} 分享 · ${liveDetail.shareExpireTime ? formatRemaining(liveDetail.shareExpireTime, now) : "临时有效"}` : "敏感信息仅在需要时显示"}</small></div><div className="vault-detail-values">
            ...
            {liveDetail.currentOtp ? <section className="is-otp"><span className="vault-otp-label">{liveDetail.otpType === "HOTP" ? `HOTP 验证码 · 计数 ${liveDetail.hotpCounter || 0}` : "动态验证码"}{liveDetail.otpType !== "HOTP" && liveDetail.otpValidUntil ? <em>{detailOtpLeft}s</em> : null}</span><div><b>{liveDetail.currentOtp.replace(/(.{3})/, "$1 ")}</b>{!liveDetail.shared || liveDetail.allowCopy ? <button type="button" onClick={() => void copyOtp(liveDetail)} aria-label="复制验证码"><Copy size={15} /></button> : null}{!liveDetail.shared && liveDetail.otpType === "HOTP" ? <button type="button" disabled={busy} onClick={() => void advanceHotp(liveDetail.id)}>下一个</button> : null}</div>{liveDetail.nextOtp && !liveDetail.shared && liveDetail.otpType !== "HOTP" ? <div className="vault-detail-next"><span>下一组</span><b>{liveDetail.nextOtp.replace(/(.{3})/, "$1 ")}</b><small>{detailOtpLeft}s 后启用</small><button type="button" className="vault-next-copy" onClick={() => void copy(liveDetail.nextOtp || "", "下一组验证码已复制")} aria-label="复制下一组验证码"><Copy size={12} /></button></div> : null}{liveDetail.otpType !== "HOTP" && liveDetail.otpValidUntil ? <div className="vault-progress"><i style={{ width: `${detailOtpProgress}%` }} /></div> : null}</section> : null}
          </div></section>
