...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const manageShareSave = async (saveId: number, action: "kick" | "ban") => {
    ...
    try {
      ...
      notify(action === "kick" ? "已踢掉该转存" : "已禁止该用户转存");
    } catch (error) {
    ...
    {modal === "shareDetail" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className="vault-modal share vault-share-form vault-share-detail">
      ...
      {shareDetailLoading || !shareDetail ? <div className="vault-share-scroll"><div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在读取授权配置…</div></div> : <>
        <div className="vault-share-scroll">
          ...
          {shareDetail.shareMode === "LINK" ? <section className="vault-share-section"><div className="vault-section-title"><div><span>03</span><h3>转存操作列表</h3></div><button type="button">{shareDetail.saveRecords?.length || 0} 条</button></div><p className="vault-section-help">踢掉后该用户无法再转存；禁止后每次都会提示“该链接已被分享者禁止转存”。</p><div className="vault-access-records vault-save-records">{shareDetail.saveRecords?.length ? <div>{shareDetail.saveRecords.map((record) => <article key={record.id} className={record.status === "ACTIVE" ? "is-success" : "is-failed"}><span>{record.status === "BANNED" ? <Ban size={15} /> : record.status === "KICKED" ? <UserMinus size={15} /> : <User size={15} />}</span><div><b>{record.nickname || record.username || "未知用户"}</b><small>{record.username ? `@${record.username}` : "转存用户"} · {saveStatusLabel(record.status)}</small></div><div className="vault-save-actions">{record.status === "ACTIVE" ? <button type="button" disabled={busy} onClick={() => void manageShareSave(record.id, "kick")}><UserMinus size={13} />踢掉</button> : null}{record.status !== "BANNED" ? <button type="button" disabled={busy} onClick={() => void manageShareSave(record.id, "ban")}><Ban size={13} />禁止</button> : null}</div></article>)}</div> : <p>还没有人转存</p>}</div></section> : null}
