...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const openShareDetail = async (share: VaultShare) => {
	...
	async function submitTextShare(event: FormEvent) {
		...
		if (!textContent) return notify("请输入要分享的文本", true);
		if (textContent.length > 50000) return notify("文本最多 50000 个字符", true);
		setBusy(true);
		try {
			const payload = { ...textShareForm, shareType: "TEXT", shareMode: "LINK", textContent, name: textShareForm.name.trim() || "临时文本分享", durationSeconds: Math.min(LONG_TERM_SECONDS, Math.max(86400, textShareForm.durationSeconds || DEFAULT_SHARE_SECONDS)), maxAccessCount: textShareForm.maxAccessCount ? Number(textShareForm.maxAccessCount) : null };
			...
			await load(true);
		} catch (error) { notify(error instanceof Error ? error.message : textShareEditingId ? "更新文本分享失败" : "创建文本分享失败", true); }
		finally { setBusy(false); }
	}
  ...
  return <div className="vault-page" onPointerDown={handlePagePointerDown}>
    ...
    {view === "text" ? <section className="vault-panel vault-text-workspace vault-view-enter" key="text">
      <header className="vault-panel-head">
        <div><span className="vault-panel-title-row"><h2>文本分享</h2></span><p>安全发送文字、Markdown、代码或 HTML，过期自动失效，也可以随时撤销</p></div>
      ...
      <div className="vault-text-overview">
        ...
        <div className="vault-text-overview-stats" aria-label="文本分享概览"><span><b>{activeTextShareCount}</b><small>有效分享</small></span><span><b>{textShares.length}</b><small>全部分享</small></span><span><b>{textShareAccessCount}</b><small>累计查看</small></span></div>
        <button type="button" className="vault-text-overview-create" onClick={openTextShare}>新建文本分享</button>
      </div>
      {textShares.length ? filteredTextShares.length ? <div className="vault-text-share-list">{filteredTextShares.map((share) => <article className={`vault-text-card is-${share.status.toLowerCase()}`} key={share.id}>
        <header className="vault-text-card-head">
          ...
          <button type="button" className="vault-text-card-title" onClick={() => void openShareDetail(share)}><b>{share.name?.trim() || "临时文本分享"}</b><small>{textShareFormatLabel(share.textFormat)}</small></button>
      ...
      </article>)}</div> : <div className="vault-empty vault-text-empty"><Search size={20} /><b>没有匹配的文本分享</b><p>换个关键词试试，名称和正文都可以搜索。</p><button type="button" className="vault-ghost" onClick={() => setTextQuery("")}>清空搜索</button></div> : <div className="vault-empty vault-text-empty"><FileText size={20} /><b>第一段分享，从这里开始</b><p>粘贴文字或 Markdown，设置有效期后即可生成安全链接。</p><button type="button" className="vault-primary" onClick={openTextShare}><Plus size={15} />新建文本分享</button></div>}
    </section> : null}
    ...
    {view === "text" ? <button type="button" className="vault-create-share" onClick={openTextShare} aria-label="新建文本分享"><FileText size={14} /><span>新建分享</span></button> : null}
...
function formatShareText(share: { name?: string; shareType?: string; sharePath?: string; shareUrl?: string; accessCode?: string; expireTime: string }) {
  const title = share.name?.trim() || (share.shareType === "TEXT" ? "临时文本分享" : "临时凭据授权");
