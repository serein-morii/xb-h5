...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const openShareDetail = async (share: VaultShare) => {
	...
	const openTextShare = () => {
		...
		setShareDetail(null);
		setTextShareForm({ name: "", textContent: "", textFormat: "TEXT", durationSeconds: DEFAULT_SHARE_SECONDS, accessCodeEnabled: true, accessCodeMode: "AUTO", accessCode: "", allowCopy: true, oneTime: false, maxAccessCount: "" });
		setModal("textShare");
	};
  ...
  return <div className="vault-page" onPointerDown={handlePagePointerDown}>
    ...
    <nav className="vault-tabs" aria-label="密钥管理导航">
      {([['all', KeyRound, '凭据'], ['text', FileText, '文本'], ['shares', Link2, '授权'], ['settings', Settings2, '我的']] as const).map(([key, Icon, label]) => <button type="button" className={view === key ? "is-active" : ""} onClick={() => changeView(key)} key={key}><Icon size={15} />{label}</button>)}
    </nav>
    ...
    <nav className="vault-mobile-nav" aria-label="密钥管理导航">
      {([['all', KeyRound, '凭据'], ['text', FileText, '文本'], ['shares', Link2, '授权'], ['settings', Settings2, '我的']] as const).map(([key, Icon, label]) => <button type="button" className={view === key ? "is-active" : ""} onClick={() => changeView(key)} key={key}><Icon size={18} /><span>{label}</span></button>)}
    </nav>
