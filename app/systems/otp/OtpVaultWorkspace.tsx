...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const [legacyText, setLegacyText] = useState("");
	...
	const [textShareForm, setTextShareForm] = useState({ name: "", textContent: "", textFormat: "TEXT" as "TEXT" | "MARKDOWN", durationSeconds: DEFAULT_SHARE_SECONDS, accessCodeEnabled: true, accessCodeMode: "AUTO", accessCode: "", allowCopy: true, oneTime: false, maxAccessCount: "" });
  ...
  return <div className="vault-page" onPointerDown={handlePagePointerDown}>
    ...
    {view === "text" ? <section className="vault-panel vault-text-workspace vault-view-enter" key="text">
      <header className="vault-panel-head">
        <div><span className="vault-panel-title-row"><h2>文本分享</h2></span><p>安全发送文字或 Markdown，过期自动失效，也可以随时撤销</p></div>
      ...
      <div className="vault-text-overview">
        ...
        <button type="button" className="vault-text-overview-create" onClick={openTextShare}><Plus size={15} /><span>新建文本分享</span></button>
      </div>
      {textShares.length ? filteredTextShares.length ? <div className="vault-text-share-list">{filteredTextShares.map((share) => <article className={`vault-text-card is-${share.status.toLowerCase()}`} key={share.id}>
        <header className="vault-text-card-head">
          ...
          <button type="button" className="vault-text-card-title" onClick={() => void openShareDetail(share)}><b>{share.name?.trim() || "临时文本分享"}</b><small>{share.textFormat === "MARKDOWN" ? "Markdown 文档" : "普通文本"}</small></button>
    ...
    {modal === "textShare" ? <div className="vault-modal-mask"><form className="vault-modal share vault-share-form vault-text-share-form" onSubmit={submitTextShare}>
      ...
      <div className="vault-share-scroll">
        <section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>文本内容</h3></div><span className="vault-count-pill">{textShareForm.textContent.length} / 50000</span></div><div className="vault-form-grid vault-single-field"><label className="wide"><span>分享名称</span><input maxLength={40} value={textShareForm.name} onChange={(event) => setTextShareForm({ ...textShareForm, name: event.target.value })} placeholder="例如 临时说明" /></label></div><p className="vault-section-help">名称会出现在列表、对方打开的页面和复制文案第一行</p><div className="vault-share-mode" role="group" aria-label="文本格式"><button type="button" className={textShareForm.textFormat === "TEXT" ? "is-active" : ""} onClick={() => setTextShareForm({ ...textShareForm, textFormat: "TEXT" })}><FileText size={16} /><span><b>普通文本</b><small>按原始换行显示</small></span></button><button type="button" className={textShareForm.textFormat === "MARKDOWN" ? "is-active" : ""} onClick={() => setTextShareForm({ ...textShareForm, textFormat: "MARKDOWN" })}><MessageSquareText size={16} /><span><b>Markdown</b><small>支持标题、列表与代码</small></span></button></div><label className="vault-text-editor"><span>要分享的内容</span><textarea required maxLength={50000} value={textShareForm.textContent} onChange={(event) => setTextShareForm({ ...textShareForm, textContent: event.target.value })} placeholder={textShareForm.textFormat === "MARKDOWN" ? "# 标题\n\n输入 Markdown 内容…" : "输入要临时分享的文本…"} /></label>{textShareForm.textContent.trim() ? <div className="vault-text-live-preview"><header><Eye size={13} /><b>实时预览</b><em>{textShareForm.textFormat === "MARKDOWN" ? "Markdown" : "普通文本"}</em></header><article className={`vault-md-body ${textShareForm.textFormat === "MARKDOWN" ? "is-markdown" : "is-text"}`} dangerouslySetInnerHTML={{ __html: renderRichText(textShareForm.textContent, textShareForm.textFormat === "MARKDOWN" ? "markdown" : "text") }} /></div> : null}</section>
