...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const [bindingTab, setBindingTab] = useState<"list" | "edit" | "channels">("list");
  ...
  const groups = useMemo(
    () => groupCredentials(filtered, credentialTab, prefs.grouped),
    [credentialTab, filtered, prefs.grouped],
  );
  ...
  return <div className="vault-page" onPointerDown={handlePagePointerDown}>
    ...
    {modal === "inboundChannels" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className="vault-modal share vault-share-form vault-inbound-modal">
      ...
      <div className="vault-share-scroll">
        <div className="vault-modal-tabs" role="tablist"><button type="button" className={channelTab === "channels" ? "is-active" : ""} onClick={() => setChannelTab("channels")}>接收通道 {inboundChannels.length ? `· ${inboundChannels.length}` : ""}</button><button type="button" className={channelTab === "unmatched" ? "is-active" : ""} onClick={() => setChannelTab("unmatched")}>待归类验证码{recentDynamicCodes.some((item) => !item.credentialId) ? ` · ${recentDynamicCodes.filter((item) => !item.credentialId).length}` : ""}</button></div>
        {channelTab === "channels" ? <>
          {channelFormOpen || !inboundChannels.length ? <form className="vault-share-section vault-channel-create" onSubmit={submitInboundChannel}><div className="vault-section-title"><div><span>01</span><h3>新建通道</h3></div>{inboundChannels.length ? <button type="button" onClick={() => setChannelFormOpen(false)}>收起</button> : null}</div><div className="vault-form-grid"><label><span>通道名称</span><input required maxLength={40} value={channelForm.name} onChange={(event) => setChannelForm({ ...channelForm, name: event.target.value })} placeholder="例如：我的 iPhone" /></label><label><span>通道类型</span><select value={channelForm.channelType} onChange={(event) => setChannelForm({ ...channelForm, channelType: event.target.value })}><option value="IPHONE">iPhone 快捷指令</option><option value="EMAIL">邮件自动化转发</option><option value="GENERIC">通用 Webhook</option></select></label><label><span>鉴权方式</span><select value={channelForm.authMode} onChange={(event) => setChannelForm({ ...channelForm, authMode: event.target.value as VaultInboundAuthMode })}><option value="TOKEN">请求头 Token 校验</option><option value="OPEN">URL 即凭证（飞书风格）</option></select></label></div><button className="vault-primary" disabled={busy}><Plus size={14} />创建接收通道</button></form> : <div className="vault-channel-toolbar"><small>点通道卡片展开地址和密钥</small><button type="button" className="vault-channel-add" onClick={() => setChannelFormOpen(true)}><Plus size={13} />新建通道</button></div>}
          <section className="vault-share-section"><div className="vault-section-title"><div><span>{channelFormOpen || !inboundChannels.length ? "02" : "01"}</span><h3>已配置通道</h3></div><small>{inboundChannels.length} 个</small></div><div className="vault-channel-list">{inboundChannels.length ? inboundChannels.map((channel) => <article className={`${channel.enabled ? "" : "is-disabled"}${expandedChannelId === channel.id ? " is-open" : ""}`} key={channel.id}><header onClick={() => setExpandedChannelId(expandedChannelId === channel.id ? null : channel.id)}><span>{channel.channelType === "IPHONE" ? <MessageSquareText size={16} /> : channel.channelType === "EMAIL" ? <Mail size={16} /> : <Webhook size={16} />}</span><div><b>{channel.name}</b><small>{channelTypeLabel(channel.channelType)}{channel.lastReceivedTime ? ` · 最近接收 ${dynamicCodeAge(channel.lastReceivedTime, now)}` : ""}</small></div><button type="button" className={channel.enabled ? "is-on" : ""} onClick={(event) => { event.stopPropagation(); void toggleInboundChannel(channel); }}>{channel.enabled ? "接收中" : "已暂停"}</button><ChevronDown size={15} className="vault-channel-chevron" /></header>{expandedChannelId === channel.id ? <><label><span>Webhook 地址</span><div><input readOnly value={webhookUrlOf(channel)} /><button type="button" onClick={() => void copy(webhookUrlOf(channel), "Webhook 地址已复制")} aria-label="复制 Webhook 地址"><Copy size={14} /></button></div></label>{channel.authMode === "OPEN" ? <label><span>鉴权方式</span><div className="vault-channel-hint">URL 本身即凭证，请求无需携带请求头，请勿外泄链接</div></label> : <label><span>请求头 X-Otp-Webhook-Token</span><div><input readOnly value={channel.webhookToken} /><button type="button" onClick={() => void copy(channel.webhookToken, "Webhook Token 已复制")} aria-label="复制 Webhook Token"><Copy size={14} /></button></div></label>}<footer><span className="vault-channel-actions"><button type="button" onClick={() => void switchChannelAuthMode(channel)}>{channel.authMode === "OPEN" ? "改用 Token 校验" : "改为免请求头"}</button><button type="button" onClick={() => void rotateInboundChannel(channel)}>重新生成密钥</button><button type="button" onClick={() => { setTutorialChannel(channel); setModal("inboundTutorial"); }}><BookOpen size={13} />配置教程</button><button type="button" className="is-danger" onClick={() => setPendingChannelDelete(channel)}><Trash2 size={13} />删除</button></span></footer></> : null}</article>) : <div className="vault-inline-empty"><Webhook size={18} />还没有接收通道</div>}</div></section>
        </> : <section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>待归类验证码</h3></div><small>{recentDynamicCodes.filter((item) => !item.credentialId).length} 条</small></div><div className="vault-unmatched-codes">{recentDynamicCodes.filter((item) => !item.credentialId).map((item) => <article key={item.id}><span>{sourceLabel(item.sourceType)} · {item.sender || item.channelName}</span><b>{item.code.replace(/(.{3})(?=.)/, "$1 ")}</b><small>{dynamicCodeAge(item.receivedTime, now)}</small><button type="button" onClick={() => void copy(item.code, "验证码已复制")}><Copy size={13} /></button></article>)}{recentDynamicCodes.every((item) => item.credentialId) ? <div className="vault-inline-empty"><Check size={17} />当前没有待归类验证码</div> : null}</div><p className="vault-section-help">匹配到「验证码来源」规则的验证码会自动贴到对应账号卡片，其余会暂存在这里。</p></section>}
      </div>
      <footer><span>鉴权方式可随时切换；验证码自动从正文中提取</span><div><button type="button" className="vault-primary" onClick={closeModal}>完成</button></div></footer>
    </section></div> : null}
    ...
    {modal === "codeBindings" && bindingTarget ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><form className="vault-modal share vault-share-form vault-binding-modal" onSubmit={submitCodeBinding}>
      ...
      <div className="vault-share-scroll">
        <div className="vault-modal-tabs" role="tablist"><button type="button" className={bindingTab === "list" ? "is-active" : ""} onClick={() => { setBindingTab("list"); setEditingBindingId(null); setBindingForm({ ...emptyBindingForm, channelId: inboundChannels[0]?.id || 0 }); }}>匹配规则 {codeBindings.length ? `· ${codeBindings.length}` : ""}</button><button type="button" className={bindingTab === "edit" ? "is-active" : ""} onClick={() => setBindingTab("edit")}>{editingBindingId ? "编辑规则" : "添加规则"}</button><button type="button" className={bindingTab === "channels" ? "is-active" : ""} onClick={() => setBindingTab("channels")}>接收通道 {inboundChannels.length ? `· ${inboundChannels.length}` : ""}</button></div>
        {bindingTab === "list" ? <section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>已绑定来源</h3></div></div><div className="vault-binding-list">{codeBindings.length ? codeBindings.map((binding) => <article className={binding.enabled ? "" : "is-disabled"} key={binding.id}><button type="button" onClick={() => editCodeBinding(binding)}><span>{binding.sourceType === "SMS" ? <MessageSquareText size={15} /> : binding.sourceType === "EMAIL" ? <Mail size={15} /> : <Webhook size={15} />}</span><div><b>{sourceLabel(binding.sourceType)} · {binding.channelName}</b><small>{[binding.senderPattern && `发送方 ${binding.senderPattern}`, binding.keywordPattern && `关键词 ${binding.keywordPattern}`, binding.recipientHint && `账号提示 ${binding.recipientHint}`].filter(Boolean).join(" · ") || "接收该通道的全部验证码"}</small></div><em>{binding.enabled ? "启用" : "停用"}</em></button>{binding.enabled ? <button type="button" onClick={() => void stopCodeBinding(binding)}>停用</button> : null}</article>) : <div className="vault-inline-empty"><Radio size={18} />尚未绑定验证码来源</div>}</div><p className="vault-section-help">点一条规则可以修改匹配条件；命中规则的验证码会自动贴到「{bindingTarget.issuer}」卡片上。</p></section> : bindingTab === "edit" ? <section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>{editingBindingId ? "编辑匹配规则" : "添加匹配规则"}</h3></div></div>{inboundChannels.length ? <div className="vault-form-grid"><label><span>接收通道</span><select required value={bindingForm.channelId} onChange={(event) => setBindingForm({ ...bindingForm, channelId: Number(event.target.value) })}>{inboundChannels.map((channel) => <option value={channel.id} key={channel.id}>{channel.name}{channel.enabled ? "" : "（已暂停）"}</option>)}</select></label><label><span>验证码来源</span><select value={bindingForm.sourceType} onChange={(event) => setBindingForm({ ...bindingForm, sourceType: event.target.value as DynamicCodeSource })}><option value="SMS">短信</option><option value="EMAIL">邮箱</option><option value="WEBHOOK">Webhook</option></select></label><label><span>发送方包含</span><input maxLength={120} value={bindingForm.senderPattern} onChange={(event) => setBindingForm({ ...bindingForm, senderPattern: event.target.value })} placeholder="例如 1069、GitHub" /></label><label><span>正文关键词</span><input maxLength={160} value={bindingForm.keywordPattern} onChange={(event) => setBindingForm({ ...bindingForm, keywordPattern: event.target.value })} placeholder="例如 验证码，逗号分隔" /></label><label><span>收件账号提示</span><input maxLength={160} value={bindingForm.recipientHint} onChange={(event) => setBindingForm({ ...bindingForm, recipientHint: event.target.value })} placeholder="邮箱或手机号尾号，可选" /></label><label><span>预计有效时间</span><select value={bindingForm.expireSeconds} onChange={(event) => setBindingForm({ ...bindingForm, expireSeconds: Number(event.target.value) })}><option value={300}>5 分钟</option><option value={600}>10 分钟</option><option value={900}>15 分钟</option><option value={1800}>30 分钟</option></select></label></div> : <button type="button" className="vault-scan-entry" onClick={() => setBindingTab("channels")}><Webhook size={17} /><span><b>先创建接收通道</b><small>创建后再回来为当前凭据配置匹配规则</small></span></button>}</section> : <section className="vault-share-section"><div className="vault-section-title"><div><span>03</span><h3>接收通道</h3></div>{inboundChannels.length ? <button type="button" onClick={() => setChannelFormOpen(true)}>新建</button> : null}</div>{channelFormOpen || !inboundChannels.length ? <form className="vault-channel-create" onSubmit={async (event) => { await submitInboundChannel(event); setBindingTab("channels"); }}><div className="vault-form-grid"><label><span>通道名称</span><input required maxLength={40} value={channelForm.name} onChange={(event) => setChannelForm({ ...channelForm, name: event.target.value })} placeholder="例如：我的 iPhone" /></label><label><span>通道类型</span><select value={channelForm.channelType} onChange={(event) => setChannelForm({ ...channelForm, channelType: event.target.value })}><option value="IPHONE">iPhone 快捷指令</option><option value="EMAIL">邮件自动化转发</option><option value="GENERIC">通用 Webhook</option></select></label><label><span>鉴权方式</span><select value={channelForm.authMode} onChange={(event) => setChannelForm({ ...channelForm, authMode: event.target.value as VaultInboundAuthMode })}><option value="TOKEN">请求头 Token 校验</option><option value="OPEN">URL 即凭证（飞书风格）</option></select></label></div><div className="vault-channel-form-actions">{inboundChannels.length ? <button type="button" className="vault-ghost" onClick={() => setChannelFormOpen(false)}>取消</button> : null}<button className="vault-primary" disabled={busy}><Plus size={14} />创建接收通道</button></div></form> : null}<div className="vault-channel-list">{inboundChannels.length ? inboundChannels.map((channel) => <article className={`${channel.enabled ? "" : "is-disabled"}${expandedChannelId === channel.id ? " is-open" : ""}`} key={channel.id}><header onClick={() => setExpandedChannelId(expandedChannelId === channel.id ? null : channel.id)}><span>{channel.channelType === "IPHONE" ? <MessageSquareText size={16} /> : channel.channelType === "EMAIL" ? <Mail size={16} /> : <Webhook size={16} />}</span><div><b>{channel.name}</b><small>{channelTypeLabel(channel.channelType)}{channel.lastReceivedTime ? ` · 最近接收 ${dynamicCodeAge(channel.lastReceivedTime, now)}` : ""}</small></div><button type="button" className={channel.enabled ? "is-on" : ""} onClick={(event) => { event.stopPropagation(); void toggleInboundChannel(channel); }}>{channel.enabled ? "接收中" : "已暂停"}</button><ChevronDown size={15} className="vault-channel-chevron" /></header>{expandedChannelId === channel.id ? <><label><span>Webhook 地址</span><div><input readOnly value={webhookUrlOf(channel)} /><button type="button" onClick={() => void copy(webhookUrlOf(channel), "Webhook 地址已复制")} aria-label="复制 Webhook 地址"><Copy size={14} /></button></div></label>{channel.authMode === "OPEN" ? <label><span>鉴权方式</span><div className="vault-channel-hint">URL 本身即凭证，请求无需携带请求头，请勿外泄链接</div></label> : <label><span>请求头 X-Otp-Webhook-Token</span><div><input readOnly value={channel.webhookToken} /><button type="button" onClick={() => void copy(channel.webhookToken, "Webhook Token 已复制")} aria-label="复制 Webhook Token"><Copy size={14} /></button></div></label>}<footer><span className="vault-channel-actions"><button type="button" onClick={() => void switchChannelAuthMode(channel)}>{channel.authMode === "OPEN" ? "改用 Token 校验" : "改为免请求头"}</button><button type="button" onClick={() => void rotateInboundChannel(channel)}>重新生成密钥</button><button type="button" onClick={() => { setTutorialChannel(channel); setModal("inboundTutorial"); }}><BookOpen size={13} />配置教程</button><button type="button" className="is-danger" onClick={() => setPendingChannelDelete(channel)}><Trash2 size={13} />删除</button></span></footer></> : null}</article>) : <div className="vault-inline-empty"><Webhook size={18} />还没有接收通道</div>}</div></section>}
      </div>
      <footer><span>{bindingTab === "channels" ? "通道给手机推码用，规则决定贴到哪个账号" : "发送方、正文关键词和账号提示同时填写时需要全部匹配"}</span><div>{bindingTab === "edit" && editingBindingId ? <button type="button" className="vault-ghost" onClick={() => { setEditingBindingId(null); setBindingForm({ ...emptyBindingForm, channelId: inboundChannels[0]?.id || 0 }); setBindingTab("list"); }}>取消编辑</button> : null}<button type="button" className="vault-ghost" onClick={closeModal}>关闭</button>{bindingTab === "edit" ? <button className="vault-primary" disabled={busy || !inboundChannels.length}><Check size={14} />{editingBindingId ? "保存规则" : "添加来源"}</button> : null}</div></footer>
    </form></div> : null}
