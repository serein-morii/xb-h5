...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const [legacyText, setLegacyText] = useState("");
	...
	const [shareForm, setShareForm] = useState({ name: "", shareMode: "LINK", recipientUsername: "", durationSeconds: 86400, accessCodeEnabled: true, accessCodeMode: "AUTO", accessCode: "", showAccount: true, showPassword: true, showOtp: true, showLoginUrl: true, showNote: true, allowCopy: true, forbidSave: false, oneTime: false, maxAccessCount: "" });
  ...
  const filtered = useMemo(() => {
    ...
    const result = credentials.filter((item) => (!value || `${item.issuer} ${item.accountName} ${item.note || ""}`.toLowerCase().includes(value)) && (!issuer || item.issuer === issuer) && matchesCredentialTab(item, credentialTab, prefs.showShared));
    result.sort((a, b) => {
    ...
    return result;
  }, [credentials, credentialTab, issuer, lastUsed, prefs.listSort, prefs.showShared, query]);
  ...
  const openShare = (credentialId?: number) => { setSelected(credentialId ? [credentialId] : []); setSharePickerOpen(!credentialId); setDurationDraft(null); setShareForm((current) => ({ ...current, name: "", durationSeconds: 86400 })); setModal("share"); };
  ...
  async function submitShare(event: FormEvent) {
    ...
    setBusy(true);
    try {
      const result = await createVaultShare({ ...shareForm, durationSeconds: Math.min(LONG_TERM_SECONDS, Math.max(86400, shareForm.durationSeconds || 86400)), credentialIds: selected, maxAccessCount: shareForm.maxAccessCount ? Number(shareForm.maxAccessCount) : null });
  ...
  const renderCredential = (item: VaultCredential) => {
    ...
    return <article className={`vault-card${prefs.compact ? " is-compact" : ""}${item.shared ? " is-shared" : ""}`} key={item.id} role="button" tabIndex={0} onClick={() => void openDetail(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openDetail(item); } }}>
      ...
      <div className="vault-progress"><i style={{ width: `${progress}%` }} /></div><div className="vault-card-foot"><span>{item.shared ? `来自 ${item.sharedBy}` : `${item.otpType || "TOTP"} · ${item.algorithm} · ${item.digits} 位`}</span>{nextOtpLabel && item.otpType !== "HOTP" && !item.requiresStepUp ? <span className="vault-card-next"><small>下一组</small><b>{concealed ? "••• •••" : nextOtpLabel}</b><button type="button" className="vault-next-copy" onClick={(event) => { event.stopPropagation(); void copy(item.nextOtp || "", "下一组验证码已复制"); }} aria-label="复制下一组验证码"><Copy size={11} /></button></span> : null}<span>{item.requiresStepUp ? "已锁定" : item.otpType === "HOTP" ? `计数 ${item.hotpCounter || 0}` : item.currentOtp ? `${left}s` : shareValidity || (item.shared ? "" : "无 OTP")}</span></div>
      <div className="vault-card-actions"><button type="button" onClick={(event) => { event.stopPropagation(); void openDetail(item); }} aria-label="查看"><Eye size={13} /><span>查看</span></button><button type="button" className={`vault-compact-favorite${item.favorite ? " is-favorite" : ""}`} onClick={(event) => { event.stopPropagation(); void toggleFavorite(item); }} aria-label={item.favorite ? "取消收藏" : "收藏"}><Star size={13} fill={item.favorite ? "currentColor" : "none"} /><span>收藏</span></button>{!item.shared ? <><button type="button" onClick={(event) => { event.stopPropagation(); openShare(item.id); }} aria-label="分享"><Share2 size={13} /><span>分享</span></button><button type="button" onClick={(event) => { event.stopPropagation(); openCredential(item); }} aria-label="编辑"><Pencil size={13} /><span>编辑</span></button><button type="button" onClick={(event) => { event.stopPropagation(); setPendingDelete(item); setModal("deleteConfirm"); }} aria-label="删除"><Trash2 size={13} /><span>删除</span></button></> : null}</div>
    </article>;
  };
  ...
  return <div className="vault-page">
    ...
    {view === "all" ? <section className="vault-panel">
      ...
      {loading ? <div className="vault-empty"><LoaderCircle className="spin" size={24} />正在加载安全数据…</div> : filtered.length ? <div className="vault-groups">{groups.map(([name, items]) => <section className="vault-group" key={name || "all"}>{name ? <header><b>{name}</b><span>{items.length}</span></header> : null}<div className={`vault-grid${prefs.compact ? " is-compact" : ""}`}>{items.map(renderCredential)}</div></section>)}</div> : <div className="vault-empty">{credentialTab === "received" ? <Inbox size={20} /> : <KeyRound size={20} />}<b>{credentialTab === "favorite" ? "还没有收藏凭据" : credentialTab === "received" ? "还没有收到的凭据" : "没有找到凭据"}</b><p>{credentialTab === "favorite" ? "点击凭据右上角的星标即可收藏。" : credentialTab === "received" ? "指定给你的授权，或从分享链接转存的内容会出现在这里。" : "可以添加一项，或导入文件。"}</p></div>}
    </section> : null}
