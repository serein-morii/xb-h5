...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  return <div className="vault-page">
    ...
    {view === "settings" ? <section className="vault-settings"><header className="vault-panel-head"><div><h2>我的</h2><p>账号资料和显示偏好会跟随当前账号</p></div></header>
      <div className="vault-settings-group vault-account-settings"><header><div className="vault-account-profile"><span className="vault-account-avatar">{(accountNick || accountName || "?").trim().slice(0, 1).toUpperCase()}</span><div><b>{accountNick || accountName || "未设置用户名"}</b><small>账号与登录</small></div></div></header>
