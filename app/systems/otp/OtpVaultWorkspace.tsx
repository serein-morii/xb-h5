...
export default function OtpVaultWorkspace({ onLogout, accountName, accountEmail, onAccountNameChange }: { onLogout: () => void; accountName: string; accountEmail: string; onAccountNameChange: (name: string) => void }) {
  ...
  const [legacyText, setLegacyText] = useState("");
	...
	const [shareForm, setShareForm] = useState({ name: "", shareMode: "LINK", recipientUsername: "", durationSeconds: 86400, accessCodeEnabled: true, accessCodeMode: "AUTO", accessCode: "", showAccount: true, showPassword: false, showOtp: true, showLoginUrl: false, showNote: false, allowCopy: true, oneTime: false, maxAccessCount: "" });
  ...
  const copyShareInfo = async (share: VaultShare) => {
    ...
    if (!text) return notify("该授权没有可复制的链接", true);
  ...
  return <div className="vault-page">
    ...
    {modal === "detail" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className="vault-modal share vault-share-form vault-detail">
      ...
      {detailLoading || !liveDetail ? <div className="vault-share-scroll"><div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在安全读取…</div></div> : <>
        <div className="vault-share-scroll">
          <section className="vault-detail-overview"><span className="vault-service-mark">{liveDetail.issuer.slice(0, 2).toUpperCase()}</span><div><small>{liveDetail.shared ? "SHARED CREDENTIAL" : "PRIVATE CREDENTIAL"}</small><b>{liveDetail.issuer}</b><p className={accountClass}>{liveDetail.accountName}</p></div><em>{liveDetail.shared ? "共享" : liveDetail.sensitivityLevel === "CRITICAL" ? "严格保护" : liveDetail.sensitivityLevel === "SENSITIVE" ? "再次验证" : "常规保护"}</em></section>
	...
	{modal === "shareEdit" && shareDetail ? <div className="vault-modal-mask"><form className="vault-modal share vault-share-form" onSubmit={submitShareEdit}>
		...
		<div className="vault-share-scroll">
			...
			<section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>名称与剩余有效期</h3></div></div><label><span>授权名称</span><input maxLength={40} value={shareForm.name} onChange={(event) => setShareForm({ ...shareForm, name: event.target.value })} placeholder="例如 王五的授权，不填则为临时凭据授权" /></label><p className="vault-section-help">保存后从当前时间重新计算，原链接不会变化</p><div className="vault-duration"><div className="vault-duration-head"><span><Clock3 size={15} /></span><div><small>从现在起</small><b>{shareForm.durationSeconds / 86400} 天</b></div></div><div className="vault-duration-presets">{[1, 3, 7, 30].map((days) => <button type="button" className={shareForm.durationSeconds === days * 86400 ? "is-active" : ""} onClick={() => setShareForm({ ...shareForm, durationSeconds: days * 86400 })} key={days}>{days} 天</button>)}</div><label><span>自定义</span><input type="number" min={1} max={365} value={shareForm.durationSeconds / 86400} onChange={(event) => setShareForm({ ...shareForm, durationSeconds: Number(event.target.value) * 86400 })} /><em>天</em></label></div></section>
    ...
    {modal === "account" ? <div className="vault-modal-mask"><section className="vault-modal share vault-share-form vault-account-modal">
