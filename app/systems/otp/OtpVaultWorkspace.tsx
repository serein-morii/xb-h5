...
type Modal = "credential" | "scanner" | "detail" | "importChoice" | "import" | "share" | "shareDetail" | "shareEdit" | "shareCreateConfirm" | "deleteConfirm" | "revokeConfirm" | "shareDeleteConfirm" | "saveActionConfirm" | "releaseConfirm" | "logoutConfirm" | "deleteAccountConfirm" | "duplicateConfirm" | "created" | "username" | "nickname" | "email" | "password" | "syncShares" | null;
type VaultView = "all" | "shares" | "settings";
type SettingsSection = "security" | "account" | "appearance" | "about" | null;
...
const defaultPrefs: VaultPrefs = { masked: false, compact: true, grouped: true, showShared: true, autoRefresh: true, autoLockMinutes: 5, stepUpEnabled: false, securityAlerts: true, theme: "system", concealOtp: false, listSort: "name", defaultFavorites: false, screenLockSet: false, screenLockPasskeyEnabled: false, autoScreenLockMinutes: 0 };
...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    const result = credentials.filter((item) => (
      (!value || `${item.issuer} ${item.accountName} ${item.note || ""} ${item.sharedBy || ""} ${item.shareName || ""}`.toLowerCase().includes(value))
    ...
    return result;
  }, [credentials, credentialTab, issuer, lastUsed, prefs.listSort, prefs.showShared, query, sharedByFilter]);
  ...
  const clearListFilters = () => { setIssuer(""); setSharedByFilter(""); };
  ...
  async function toggleFavorite(item: VaultCredential) {
    try {
      ...
      else await saveVaultCredential(item.id, { issuer: item.issuer, accountName: item.accountName, favorite: !item.favorite, loginUrl: item.loginUrl, note: item.note, sensitivityLevel: item.sensitivityLevel });
      await load(true);
    }
  ...
  return <div className="vault-page">
    ...
    {view === "settings" ? <section className="vault-settings vault-view-enter" key="settings"><header className="vault-panel-head"><div>{settingsSection ? <button type="button" className="vault-settings-back" onClick={() => setSettingsSection(null)}><ArrowLeft size={15} />返回</button> : null}{settingsSection !== "security" ? <><h2>{settingsSection === "account" ? "账号" : settingsSection === "appearance" ? "外观和显示" : settingsSection === "about" ? "关于" : "设置"}</h2><p>{settingsSection === "account" ? "用户名、账号、邮箱和登录" : settingsSection === "appearance" ? "主题、卡片和列表偏好" : settingsSection === "about" ? "" : "安全、账号和显示偏好会跟随当前账号"}</p></> : null}</div></header>
      {!settingsSection ? <div className="vault-settings-group vault-settings-hub">
      ...
      {settingsSection === "security" ? <section className="vault-security-page"><VaultSecurityCenter prefs={prefs} updatePrefs={updatePrefs} zeroKnowledgeKey={zeroKnowledgeKey} onZeroKnowledgeKey={setZeroKnowledgeKey} onEditScreenLock={() => setScreenLockSetup(true)} /></section> : null}
      ...
      {settingsSection === "appearance" ? <div className="vault-settings-group vault-settings-appearance">
      ...
      {settingsSection === "about" ? <div className="vault-settings-group vault-settings-about">
      ...
      {detailLoading || !liveDetail ? <div className="vault-share-scroll"><div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在安全读取…</div></div> : <>
        <div className="vault-share-scroll">
          ...
          <section className="vault-share-section vault-detail-section"><div className="vault-section-title"><div><span>01</span><h3>登录信息</h3></div><small>{liveDetail.shared ? `${liveDetail.sharedBy} 分享 · ${liveDetail.shareExpireTime ? formatRemaining(liveDetail.shareExpireTime, now) : "临时有效"}` : "敏感信息仅在需要时显示"}</small></div><div className="vault-detail-values">
            ...
            {liveDetail.currentOtp ? <section className="is-otp"><span className="vault-otp-label">{liveDetail.otpType === "HOTP" ? `HOTP 验证码 · 计数 ${liveDetail.hotpCounter || 0}` : "动态验证码"}{liveDetail.otpType !== "HOTP" && liveDetail.otpValidUntil ? <em>{detailOtpLeft}s</em> : null}</span><div><b>{liveDetail.currentOtp.replace(/(.{3})/, "$1 ")}</b>{!liveDetail.shared || liveDetail.allowCopy ? <button type="button" onClick={() => void copyOtp(liveDetail)} aria-label="复制验证码"><Copy size={15} /></button> : null}{!liveDetail.shared && liveDetail.otpType === "HOTP" ? <button type="button" disabled={busy} onClick={() => void advanceHotp(liveDetail.id)}>下一个</button> : null}</div>{liveDetail.nextOtp && liveDetail.otpType !== "HOTP" ? <div className="vault-detail-next"><span>下一组</span><b>{liveDetail.nextOtp.replace(/(.{3})/, "$1 ")}</b><small>{detailOtpLeft}s 后启用</small><button type="button" className="vault-next-copy" onClick={() => void copy(liveDetail.nextOtp || "", "下一组验证码已复制")} aria-label="复制下一组验证码"><Copy size={12} /></button></div> : null}{liveDetail.otpType !== "HOTP" && liveDetail.otpValidUntil ? <div className="vault-progress"><i style={{ width: `${detailOtpProgress}%` }} /></div> : null}</section> : null}
          </div></section>
