...
import VaultScreenLock from "./VaultScreenLock";
...
import { DEFAULT_SHARE_SECONDS, PENDING_SAVE_KEY, SHARED_BY_SELF, SHARE_ITEM_LIMIT, clipboardReadBlocked, defaultShareName, groupCredentials, groupReceivedBySource, listSharedByOptions, matchesCredentialKind, matchesCredentialTab, matchesSharedByFilter, normalizeTextShareFormat, parseShareClipboard, receivedShareSourceLabel, rememberShareAccessCode, selectShareItems, shareDetailCredentials, sharedByFilterLabel, sharerDisplay, shouldOfferClipboardShare, splitCredentialTags, textShareFormatHint, textShareFormatLabel, textSharePlaceholder, textShareRenderType, textShareSearchLabel, toggleShareSelection, type CredentialKindFilter, type CredentialTab, type ShareTab, type TextShareFormat } from "./otpVaultShare";
...
type NotifyChannel = "email" | "bark" | "push";
function parseNotificationRules(raw?: string) {
  try {
    const parsed = JSON.parse(raw || "{}") as Record<string, { email?: boolean; bark?: boolean; push?: boolean }>;
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return {};
    const next: Record<string, { email: boolean; bark: boolean; push: boolean }> = {};
    for (const [key, value] of Object.entries(parsed)) {
      if (!value || typeof value !== "object") continue;
      next[key] = { email: value.email !== false, bark: value.bark !== false, push: value.push !== false };
    }
    return next;
  } catch { return {}; }
}
type VaultView = "all" | "text" | "shares" | "settings";
const VAULT_NAV = [
  ["all", KeyRound, "凭据"],
  ["shares", Link2, "授权"],
  ["text", FileText, "笔记"],
...
type SettingsSection = "security" | "account" | "appearance" | "notifications" | "about" | null;
...
const DURATION_PRESETS = [
  ...
  { days: 7, label: "7 天" },
  { days: 30, label: "30 天" },
  { days: LONG_TERM_DAYS, label: "长期" },
];
function isLongTerm(seconds: number) {
  return seconds >= 10 * 365 * 86400;
}
function durationHeadline(seconds: number) {
  return isLongTerm(seconds) ? "长期" : `${Math.max(1, Math.round(seconds / 86400))} 天`;
}
...
function writeLastScreenActive(at = Date.now()) {
...
type ImportPreviewItem = { issuer: string; accountName: string; status: "NEW" | "DUPLICATE" | "CONFLICT" };

function readImportFile(file: File, onProgress: (percent: number) => void) {
...
async function hydrateClientCredential(item: VaultCredential, key: CryptoKey | null) {
  ...
  if (item.clientOtpSecretCiphertext) {
    ...
    next.currentOtp = await generateOfflineCode({ ...item, otpSecret }, now);
    if (item.otpType !== "HOTP") next.nextOtp = await generateOfflineCode({ ...item, otpSecret }, now + period * 1000);
    next.otpValidUntil = item.otpType === "HOTP" ? 0 : (Math.floor(now / 1000 / period) + 1) * period * 1000;
  }
  return next;
}
...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const textShares = useMemo(() => shares.filter((share) => share.shareType === "TEXT"), [shares]);
  const credentialShares = useMemo(() => shares.filter((share) => share.shareType !== "TEXT"), [shares]);
  ...
  const clipboardArmed = useRef(false);
  ...
  const [textFolders, setTextFolders] = useState<VaultTextFolder[]>([]);
  const [textFolderFilter, setTextFolderFilter] = useState<number | "all" | "none">("all");
  const [folderModal, setFolderModal] = useState<{ mode: "create" | "rename"; id?: number; name: string } | null>(null);
  const [folderDeleteTarget, setFolderDeleteTarget] = useState<VaultTextFolder | null>(null);
  const folderedTextShares = useMemo(() => (
    textFolderFilter === "all" ? textShares
      : textFolderFilter === "none" ? textShares.filter((share) => !share.textFolderId)
      : textShares.filter((share) => share.textFolderId === textFolderFilter)
  ), [textFolderFilter, textShares]);
  const filteredTextShares = useMemo(() => {
    const value = textQuery.trim().toLowerCase();
    if (!value) return folderedTextShares;
    return folderedTextShares.filter((share) => `${share.name || ""} ${share.textContent || ""} ${textShareSearchLabel(share.textFormat)} ${share.accessCodeEnabled ? "访问码" : "免密码"}`.toLowerCase().includes(value));
  }, [folderedTextShares, textQuery]);
  const activeTextShareCount = useMemo(() => textShares.filter((share) => share.status === "ACTIVE").length, [textShares]);
  const textShareAccessCount = useMemo(() => textShares.reduce((total, share) => total + (share.accessCount || 0), 0), [textShares]);
  ...
  const [legacyText, setLegacyText] = useState("");
	const [importItems, setImportItems] = useState<VaultTransferItem[]>([]);
	...
	const [importProgress, setImportProgress] = useState<{ label: string; percent: number } | null>(null);
	const [tagDraft, setTagDraft] = useState("");
	const [selected, setSelected] = useState<number[]>([]);
	const [sharePickerOpen, setSharePickerOpen] = useState(false);
	const [shareForm, setShareForm] = useState({ name: "", shareMode: "LINK", recipientUsername: "", durationSeconds: DEFAULT_SHARE_SECONDS, accessCodeEnabled: true, accessCodeMode: "AUTO", accessCode: "", showAccount: true, showPassword: true, showOtp: true, showLoginUrl: true, showNote: true, allowCopy: true, forbidSave: false, oneTime: false, maxAccessCount: "" });
	const [textShareForm, setTextShareForm] = useState({ name: "", textContent: "", textFormat: "TEXT" as TextShareFormat, durationSeconds: DEFAULT_SHARE_SECONDS, accessCodeEnabled: true, accessCodeMode: "AUTO", accessCode: "", allowCopy: true, oneTime: false, maxAccessCount: "", textFolderId: null as number | null });
	const [textShareEditingId, setTextShareEditingId] = useState<number | null>(null);
	...
	const [created, setCreated] = useState<{ name?: string; shareType?: "CREDENTIAL" | "TEXT"; shareMode: "LINK" | "DIRECT"; recipientUsername?: string; shareUrl?: string; accessCode?: string; autoFillAllowed: boolean; expireTime: string } | null>(null);
  ...
  // 凭据页内「全部 / 收藏 / 我收到的」；初始值跟随「我的」里的 defaultFavorites 偏好
  const [credentialTab, setCredentialTab] = useState<CredentialTab>("all");
  ...
  const applyDurationDays = (days: number) => {
    ...
    setShareForm((current) => ({ ...current, durationSeconds: next * 86400 }));
  };
  const onDurationDraft = (raw: string) => {
    const value = raw.replace(/[^\d]/g, "");
    setDurationDraft(value);
    if (value === "") return;
    const days = Math.min(365, Math.max(1, Number(value)));
    setShareForm((current) => ({ ...current, durationSeconds: days * 86400 }));
  };
  const commitDurationDraft = () => {
    if (durationDraft === "") applyDurationDays(Math.round(shareForm.durationSeconds / 86400) || 1);
    else setDurationDraft(null);
  };
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      listVaultTextFolders().then((folderResult) => setTextFolders(folderResult.data || [])).catch(() => setTextFolders([]));
  ...
  const scanClipboard = useCallback(async () => {
    ...
    try {
      await considerClipboardText(await readClipboardText());
    } catch (error) {
      if (clipboardReadBlocked(error)) clipboardArmed.current = true;
    }
  ...
  useEffect(() => {
    ...
    const onPointerDown = () => {
      if (!clipboardArmed.current) return;
      clipboardArmed.current = false;
      void scanClipboard();
    };
  ...
  useEffect(() => {
    if (notifyEmailCountdown <= 0) return;
    const timer = window.setTimeout(() => setNotifyEmailCountdown((value) => Math.max(0, value - 1)), 1000);
    return () => window.clearTimeout(timer);
  }, [notifyEmailCountdown]);
  useEffect(() => {
    ...
    setPushStatusError("");
    void Promise.all([
      getVaultPushPublicKey(),
      "serviceWorker" in navigator
        ? navigator.serviceWorker.getRegistration().then((registration) => registration?.pushManager.getSubscription())
        : Promise.resolve(null),
    ]).then(async ([keyResult, subscription]) => {
      if (cancelled) return;
	  setPushServerEnabled(Boolean(keyResult.data.enabled && keyResult.data.publicKey));
      if (!keyResult.data.enabled || !keyResult.data.publicKey) {
      ...
      if (!webPushKeyMatchesSubscription(subscription, keyResult.data.publicKey)) {
        await deleteVaultPushSubscription(subscription.endpoint).catch(() => undefined);
        await subscription.unsubscribe().catch(() => false);
        if (!cancelled) {
          setPushEnabled(false);
          setPushSynced(false);
          setPushStatusError("推送密钥已更新，请重新开启这台设备");
        }
        return;
      }
      setPushEnabled(true);
      await saveVaultPushSubscription(webPushSubscriptionBody(subscription));
      if (!cancelled) setPushSynced(true);
    }).catch((error) => {
      if (!cancelled) {
        setPushSynced(false);
        setPushStatusError(error instanceof Error ? error.message : "订阅状态同步失败");
      }
    }).finally(() => { if (!cancelled) setPushBusy(false); });
  ...
  useEffect(() => { void load(); const sent = Date.now(); void getVaultPreferences().then((result) => {
    ...
    void getVaultScreenLockState().then((state) => {
      ...
      const locked = Boolean(remote.screenLockSet) && (Boolean(state.data?.locked) || idleLocked);
      sessionStorage.setItem(SCREEN_LOCK_KEY, locked ? "locked" : "unlocked");
      setScreenLocked(locked);
      if (idleLocked && !state.data?.locked) void setVaultScreenLockState(true).catch(() => undefined);
    }).catch(() => {
      const idleLocked = shouldAutoLockByIdle(remote);
      const locked = Boolean(remote.screenLockSet) && (sessionStorage.getItem(SCREEN_LOCK_KEY) === "locked" || idleLocked);
      setScreenLocked(locked);
      if (idleLocked) void setVaultScreenLockState(true).catch(() => undefined);
    });
	...
	useEffect(() => {
		...
		return () => { window.clearTimeout(timer); events.forEach((name) => window.removeEventListener(name, reset)); document.removeEventListener("visibilitychange", visibility); };
		  ...
		  useEffect(() => {
		    ...
		    let lastActive = readLastScreenActive() || writeLastScreenActive();
		    let timer = 0;
		    ...
		    const markActive = () => {
		      lastActive = writeLastScreenActive();
		      window.clearTimeout(timer);
		      timer = window.setTimeout(lockIfIdle, idleMs);
		    };
		    const persistActive = () => writeLastScreenActive(lastActive);
		    const onVisible = () => {
		      if (document.hidden) {
		        persistActive();
		        return;
		      }
		      lastActive = readLastScreenActive() || lastActive;
		      if (Date.now() - lastActive >= idleMs) lockScreen();
		      else {
		        window.clearTimeout(timer);
		    ...
		    return () => {
		      ...
		      window.removeEventListener("pagehide", persistActive);
		    };
	...
	const updatePrefs = async (next: VaultPrefs) => {
			...
			: pushBusy
				...
				: pushEnabled
					? pushSynced
						...
						: "当前设备已授权，但尚未同步到服务器"
					: iosNeedsHomeScreen
						? "iPhone / iPad 需先用 Safari 添加到主屏幕，再从主屏幕打开"
		...
		async function toggleBrowserPush() {
			...
			try {
				...
				setPushStatusError("");
				if (pushEnabled) {
					const existing = await navigator.serviceWorker.getRegistration().then((reg) => reg?.pushManager.getSubscription());
					if (existing) {
						await deleteVaultPushSubscription(existing.endpoint);
				...
				if (!key.data.publicKey) throw new Error("服务器尚未配置 Web Push 公钥");
				setPushServerEnabled(Boolean(key.data.enabled));
				await (navigator.serviceWorker.getRegistration() || navigator.serviceWorker.register("/otp-sw.js", { updateViaCache: "none" }));
				const registration = await navigator.serviceWorker.ready;
				const permission = await Notification.requestPermission();
				if (permission !== "granted") throw new Error("浏览器未允许通知");
				const bytes = decodeWebPushPublicKey(key.data.publicKey);
  ...
  const filtered = useMemo(() => {
    ...
    const result = credentials.filter((item) => (
      (!value || `${item.issuer} ${item.accountName} ${item.note || ""} ${item.tags || ""} ${item.sharedBy || ""} ${item.shareName || ""}`.toLowerCase().includes(value))
  ...
  const importCounts = useMemo(() => ({
    added: importPreview.filter((item) => item.status === "NEW").length,
  ...
  useEffect(() => {
    if (modal !== "share" && modal !== "shareCreateConfirm") return;
    if (shareNameTouched) return;
    const issuers = ownCredentials.filter((item) => selected.includes(item.id)).map((item) => item.issuer);
    const name = defaultShareName(accountNick || accountName, issuers);
    setShareForm((current) => current.name === name ? current : { ...current, name });
  }, [accountName, accountNick, modal, ownCredentials, selected, shareNameTouched]);
  const groups = useMemo(
    () => groupCredentials(filtered, credentialTab, prefs.grouped, true, prefs.groupBy || "system"),
    [credentialTab, filtered, prefs.grouped, prefs.groupBy],
  );
  ...
  const setCredentialTabSafe = (tab: CredentialTab) => {
    ...
    if (tab === "received" && sharedByFilter === SHARED_BY_SELF) setSharedByFilter("");
  };
  const toggleCollapsed = (key: string) => {
    setCollapsedGroups((prev) => ({ ...prev, [key]: !prev[key] }));
  };
  const isCollapsed = (key: string) => Boolean(collapsedGroups[key]);
  ...
  const submitInboundChannel = async () => {
    if (!channelForm.name.trim()) { notify("请填写通道名称", true); return null; }
    setBusy(true);
    try {
      const created = (await createVaultInboundChannel(channelForm)).data;
      await loadInboundChannelData();
      setChannelForm({ ...emptyChannelForm });
      setChannelFormOpen(false);
      notify("接收通道已创建");
      return created;
    } catch (error) { notify(error instanceof Error ? error.message : "接收通道创建失败", true); return null; }
    finally { setBusy(false); }
  };
  ...
  const removeInboundChannel = async (channel: VaultInboundChannel) => {
    setBusy(true);
    try {
      ...
      notify("接收通道已删除");
    } catch (error) { notify(error instanceof Error ? error.message : "接收通道删除失败", true); }
    finally { setBusy(false); }
  };
  ...
  const addCredentialTags = (raw = tagDraft) => {
    ...
    const current = splitCredentialTags(form.tags);
    const next = [...new Set([...current, ...additions])].slice(0, 8);
    if (current.length + additions.length > 8) notify("最多添加 8 个标签", true);
    setForm((value) => ({ ...value, tags: next.join(" ") }));
    setTagDraft("");
  };
  const removeCredentialTag = (tag: string) => {
    setForm((value) => ({ ...value, tags: splitCredentialTags(value.tags).filter((item) => item !== tag).join(" ") }));
  };
  ...
  const openDetail = async (item: VaultCredential) => {
    ...
    setDetailCodeHistory([]); setDetailCodeHistoryTotal(0); setDetailCodeHistoryPage(0);
	    try {
      const hydrated = await hydrateClientCredential((await getVaultCredential(item.id)).data, zeroKnowledgeKey);
      setDetail(hydrated);
      if (hydrated.dynamicCodeEnabled || hydrated.dynamicSources?.length || hydrated.dynamicCode) void loadDetailCodeHistory(item.id, 1, true);
    }
    catch (error) { setModal(null); notify(error instanceof Error ? error.message : "详情加载失败", true); }
    finally { setDetailLoading(false); }
  };
  const loadDetailCodeHistory = async (credentialId: number, page: number, replace = false) => {
    setDetailCodeHistoryLoading(true);
    try {
      const result = await listVaultCredentialDynamicCodes(credentialId, page);
      setDetailCodeHistory((current) => replace ? result.data.rows : [...current, ...result.data.rows]);
      setDetailCodeHistoryTotal(result.data.total);
  ...
  const openReceivedShareDetail = (share: VaultShare) => {
    setShareDetail(share);
    setShareDetailLoading(false);
    setModal("shareDetail");
  };
  const cancelRevoke = () => {
    if (busy) return;
  ...
  async function toggleFavorite(item: VaultCredential) {
    try {
      if (item.shared) await favoriteSharedCredential(Math.abs(item.id), !item.favorite);
      else await saveVaultCredential(item.id, { issuer: item.issuer, accountName: item.accountName, favorite: !item.favorite, loginUrl: item.loginUrl, note: item.note, tags: item.tags, sensitivityLevel: item.sensitivityLevel });
      await load(true);
    }
    ...
    try {
			const current = credentials.find((item) => item.id === id);
				const result = current?.zeroKnowledge
					? await saveVaultCredential(id, { issuer: current.issuer, accountName: current.accountName, hotpCounter: (current.hotpCounter || 0) + 1, favorite: current.favorite, loginUrl: current.loginUrl, note: current.note, tags: current.tags, sensitivityLevel: current.sensitivityLevel })
  ...
  async function submitImport(event: FormEvent) {
    ...
    try {
      if (!importItems.length) {
        setImportProgress({ label: "正在解析文件", percent: 5 });
        const parsed = parseVaultImportText(legacyText);
        const items: VaultTransferItem[] = [];
        for (let index = 0; index < parsed.length; index += 1) {
          const item = parsed[index];
          const body = await protectCredential({ ...emptyCredential, ...item });
          items.push({
            issuer: String(body.issuer || item.issuer),
            accountName: String(body.accountName || item.accountName),
            password: typeof body.password === "string" ? body.password : undefined,
            otpSecret: typeof body.otpSecret === "string" ? body.otpSecret : undefined,
            clientPasswordCiphertext: typeof body.clientPasswordCiphertext === "string" ? body.clientPasswordCiphertext : undefined,
            clientOtpSecretCiphertext: typeof body.clientOtpSecretCiphertext === "string" ? body.clientOtpSecretCiphertext : undefined,
            otpType: item.otpType,
            hotpCounter: item.hotpCounter,
        ...
        setImportProgress({ label: "正在检查重复和冲突", percent: 90 });
        const preview = await previewVaultImport(items);
        setImportItems(items);
        setImportPreview(preview.data.items || []);
        setImportProgress(null);
        return;
      }
      setImportProgress({ label: "正在写入保险库", percent: 92 });
      const result = await commitVaultImport(importItems, importReplaceExisting);
      const created = result.data.created || 0;
      const updated = result.data.updated || 0;
      const skipped = result.data.skipped || 0;
      notify(`已导入 ${created + updated} 项${skipped ? `，跳过 ${skipped} 项重复或冲突凭据` : ""}`);
      setModal(null); resetImport(); await load(true); syncOfflineCopy();
    } catch (error) { notify(error instanceof Error ? error.message : "导入失败", true); }
    finally { setBusy(false); setImportProgress(null); }
  }
  const applyShareSelection = (id: number, checked: boolean) => {
    if (!checked) { setSelected(selected.filter((value) => value !== id)); return; }
    const result = toggleShareSelection(selected, id);
    if (result.limited) notify(`单次最多选择 ${SHARE_ITEM_LIMIT} 个凭据`, true);
    setSelected(result.selected);
  };
  const toggleSelectAllShareItems = () => {
    const ids = ownCredentials.map((item) => item.id);
    if (selected.length && selected.length === Math.min(ids.length, SHARE_ITEM_LIMIT)) setSelected([]);
    else {
      const next = selectShareItems(ids);
      if (ids.length > SHARE_ITEM_LIMIT) notify(`单次最多选择 ${SHARE_ITEM_LIMIT} 个凭据`, true);
      setSelected(next);
    }
  };
  const openShare = (credentialId?: number) => {
    const nextSelected = credentialId ? [credentialId] : [];
    const issuers = ownCredentials.filter((item) => nextSelected.includes(item.id)).map((item) => item.issuer);
    setSelected(nextSelected);
    setSharePickerOpen(!credentialId);
    setDurationDraft(null);
    setShareNameTouched(false);
    setShareForm((current) => ({ ...current, name: defaultShareName(accountNick || accountName, issuers), durationSeconds: DEFAULT_SHARE_SECONDS }));
    setModal("share");
  };
  const openShareDetail = async (share: VaultShare) => {
	...
	const openShareEdit = async (share: VaultShare) => {
		...
		try {
			...
			const days = Math.max(1, Math.ceil((new Date(normalizeDateTime(detail.expireTime)).getTime() - Date.now()) / 86400000));
				const durationDays = days >= 10 * 365 ? LONG_TERM_DAYS : days;
			if (detail.shareType === "TEXT") {
				setShareDetail(detail);
				setTextShareEditingId(detail.id);
				setTextShareForm({ name: detail.name || "", textContent: detail.textContent || "", textFormat: normalizeTextShareFormat(detail.textFormat), durationSeconds: durationDays * 86400, accessCodeEnabled: detail.accessCodeEnabled, accessCodeMode: "CUSTOM", accessCode: detail.accessCode || "", allowCopy: detail.allowCopy, oneTime: detail.oneTime, maxAccessCount: detail.maxAccessCount ? String(detail.maxAccessCount) : "", textFolderId: detail.textFolderId ?? null });
				setModal("textShare");
				return;
			}
			setShareDetail(detail); setSelected(detail.credentialIds || []); setSharePickerOpen(false);
			setShareForm((current) => ({ ...current, name: detail.name || "", shareMode: detail.shareMode, recipientUsername: detail.recipientUsername || "", durationSeconds: durationDays * 86400, accessCodeEnabled: detail.accessCodeEnabled, accessCodeMode: "CUSTOM", accessCode: detail.accessCode || "", showAccount: detail.showAccount, showPassword: detail.showPassword, showOtp: detail.showOtp, showLoginUrl: detail.showLoginUrl, showNote: detail.showNote, allowCopy: detail.allowCopy, forbidSave: Boolean(detail.forbidSave), oneTime: detail.oneTime, maxAccessCount: detail.maxAccessCount ? String(detail.maxAccessCount) : "" }));
			setDurationDraft(null);
				setModal("shareEdit");
	...
	const openTextShare = () => {
		setTextShareEditingId(null);
		setShareDetail(null);
		setTextShareForm({ name: "", textContent: "", textFormat: "TEXT", durationSeconds: DEFAULT_SHARE_SECONDS, accessCodeEnabled: false, accessCodeMode: "AUTO", accessCode: "", allowCopy: true, oneTime: false, maxAccessCount: "", textFolderId: typeof textFolderFilter === "number" ? textFolderFilter : null });
		setModal("textShare");
	};
	async function submitTextShare(event: FormEvent) {
		event.preventDefault();
		const textContent = textShareForm.textContent.trim();
		if (!textContent) return notify("请输入笔记内容", true);
		if (textContent.length > 50000) return notify("笔记最多 50000 个字符", true);
		setBusy(true);
		try {
			const payload = { ...textShareForm, textFolderId: textShareForm.textFolderId ?? null, shareType: "TEXT", shareMode: "LINK", textContent, name: textShareForm.name.trim() || "临时笔记", durationSeconds: Math.min(LONG_TERM_SECONDS, Math.max(86400, textShareForm.durationSeconds || DEFAULT_SHARE_SECONDS)), maxAccessCount: textShareForm.maxAccessCount ? Number(textShareForm.maxAccessCount) : null };
			if (textShareEditingId) {
				const result = await updateVaultShare(textShareEditingId, payload);
				setShareDetail(result.data);
				setModal("shareDetail");
				notify("笔记已更新");
			} else {
				const result = await createVaultShare(payload);
				setCreated(result.data);
				setModal("created");
			}
			await load(true);
		} catch (error) { notify(error instanceof Error ? error.message : textShareEditingId ? "更新笔记失败" : "创建笔记失败", true); }
		finally { setBusy(false); }
	}
  const requestShareCreate = (event: FormEvent) => {
    event.preventDefault();
    if (!selected.length) return notify("至少选择一个凭据", true);
    if (selected.length > SHARE_ITEM_LIMIT) return notify(`单次最多授权 ${SHARE_ITEM_LIMIT} 个凭据`, true);
    if (shareForm.shareMode === "DIRECT" && !shareForm.recipientUsername.trim()) return notify("请输入接收人账号", true);
    setModal("shareCreateConfirm");
  };
  async function confirmShareCreate() {
    ...
    try {
      const issuers = ownCredentials.filter((item) => selected.includes(item.id)).map((item) => item.issuer);
      const result = await createVaultShare({ ...shareForm, name: shareForm.name.trim() || defaultShareName(accountNick || accountName, issuers), durationSeconds: Math.min(LONG_TERM_SECONDS, Math.max(86400, shareForm.durationSeconds || DEFAULT_SHARE_SECONDS)), credentialIds: selected, maxAccessCount: shareForm.maxAccessCount ? Number(shareForm.maxAccessCount) : null });
	...
	async function submitShareEdit(event: FormEvent) {
		...
		setBusy(true);
		try {
			const result = await updateVaultShare(shareDetail.id, { name: shareForm.name, credentialIds: selected, durationSeconds: Math.min(LONG_TERM_SECONDS, Math.max(86400, shareForm.durationSeconds || 86400)), showAccount: shareForm.showAccount, showPassword: shareForm.showPassword, showOtp: shareForm.showOtp, showLoginUrl: shareForm.showLoginUrl, showNote: shareForm.showNote, allowCopy: shareForm.allowCopy, oneTime: shareForm.oneTime, maxAccessCount: shareForm.maxAccessCount ? Number(shareForm.maxAccessCount) : null });
  ...
  const canCopyCredential = (item: VaultCredential) => !item.shared || item.allowCopy !== false;
  const copyOtp = async (item: VaultCredential) => {
    if (!item.currentOtp || !canCopyCredential(item)) return;
    setLastUsed((current) => {
  ...
  const shareShareInfo = async (share: VaultShare) => {
    let detail = share;
    if (share.accessCodeEnabled && !share.accessCode) {
      try { detail = (await getVaultShare(share.id)).data; }
      catch (error) { notify(error instanceof Error ? error.message : "分享失败", true); return; }
    }
    const text = formatShareText(detail);
    if (typeof navigator.share !== "function") return copy(text, "完整分享信息已复制");
    try { await navigator.share({ title: detail.name?.trim() || "OTP Vault 授权", text }); }
    catch (error) { if (!(error instanceof Error) || error.name !== "AbortError") notify(error instanceof Error ? error.message : "分享失败", true); }
  };
  const manageShareSave = async (saveId: number, action: "kick" | "ban" | "restore") => {
    if (!shareDetail) return;
    setBusy(true);
    try {
      if (action === "kick") await kickVaultShareSave(shareDetail.id, saveId);
      else if (action === "ban") await banVaultShareSave(shareDetail.id, saveId);
      else await restoreVaultShareSave(shareDetail.id, saveId);
      const detail = (await getVaultShare(shareDetail.id)).data;
      setShareDetail(detail);
      setPendingSaveAction(null);
      setModal("shareDetail");
      notify(action === "kick" ? "已移除该转存" : action === "ban" ? "已禁止该用户转存" : "已恢复该转存");
    } catch (error) {
  ...
  const renderCredential = (item: VaultCredential) => {
    ...
    const progress = item.currentOtp && item.periodSeconds ? Math.max(0, Math.min(100, left / item.periodSeconds * 100)) : item.dynamicCode ? inboundTiming.progress : 0;
    ...
    const configuredSourceTypes = Array.from(new Set<DynamicCodeSource>([
      ...(item.dynamicSources || []),
      ...(item.dynamicCodeSources || []).map((source) => source.sourceType),
      ...(item.dynamicCodeSource ? [item.dynamicCodeSource] : []),
    ]));
    const usesInboundSources = Boolean(item.dynamicCodeEnabled && !item.currentOtp);
    return <article className={`vault-card${prefs.compact ? " is-compact" : ""}${item.shared ? " is-shared" : ""}`} key={item.id} role="button" tabIndex={0} onClick={() => void openDetail(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openDetail(item); } }}>
      ...
      <div className="vault-progress"><i style={{ width: `${usesInboundSources ? inboundTiming.progress : progress}%` }} /></div>{usesInboundSources ? <div className="vault-card-foot is-inbound-foot"><span className="vault-card-sources" aria-label={configuredSourceTypes.length ? `已配置${configuredSourceTypes.map(sourceLabel).join("、")}` : "未配置来源"}>{configuredSourceTypes.length ? configuredSourceTypes.map((source) => <span className={`vault-card-source${item.dynamicCode && source === item.dynamicCodeSource ? " is-active" : ""}`} key={source} title={sourceLabel(source)} aria-label={sourceLabel(source)}>{source === "EMAIL" ? <Mail size={13} /> : source === "WEBHOOK" ? <Webhook size={13} /> : <MessageSquareText size={13} />}</span>) : <span className="vault-source-unconfigured">未配置来源</span>}</span>{!prefs.compact && item.dynamicCode ? <span>{formatCodeTime(inboundTiming.left)}</span> : null}</div> : <div className="vault-card-foot"><span>{item.shared ? `来自 ${sharerDisplay(item)}` : item.currentOtp || item.otpConfigured ? `${item.otpType || "TOTP"} · ${item.algorithm} · ${item.digits} 位` : item.passwordConfigured ? "登录密码" : "安全笔记"}</span>{nextOtpLabel && item.otpType !== "HOTP" && !item.requiresStepUp ? <span className="vault-card-next"><small>下一组</small><b>{concealed ? "••• •••" : nextOtpLabel}</b>{copyAllowed ? <button type="button" className="vault-next-copy" onClick={(event) => { event.stopPropagation(); void copy(item.nextOtp || "", "下一组验证码已复制"); }} aria-label="复制下一组验证码"><Copy size={11} /></button> : null}</span> : null}<span>{item.requiresStepUp ? "已锁定" : item.otpType === "HOTP" && (item.currentOtp || item.otpConfigured) ? `计数 ${item.hotpCounter || 0}` : item.currentOtp ? `${left}秒` : shareValidity || (item.shared ? "" : item.passwordConfigured ? "密码" : "笔记")}</span></div>}
      <div className="vault-card-actions"><button type="button" onClick={(event) => { event.stopPropagation(); void openDetail(item); }} aria-label="查看"><Eye size={13} /><span>查看</span></button>{!item.shared ? <button type="button" className={`vault-compact-favorite${item.favorite ? " is-favorite" : ""}`} onClick={(event) => { event.stopPropagation(); void toggleFavorite(item); }} aria-label={item.favorite ? "取消收藏" : "收藏"}><Star size={13} fill={item.favorite ? "currentColor" : "none"} /></button> : null}{item.shared && item.shareId ? <button type="button" onClick={(event) => { event.stopPropagation(); const share = receivedShares.find((row) => row.id === item.shareId); if (share) openReceivedShareDetail(share); }} aria-label="分享详情"><Link2 size={13} /><span>详情</span></button> : null}{!item.shared ? <><button type="button" onClick={(event) => { event.stopPropagation(); openShare(item.id); }} aria-label="分享"><Share2 size={13} /><span>分享</span></button><button type="button" onClick={(event) => { event.stopPropagation(); openCredential(item); }} aria-label="编辑"><Pencil size={13} /><span>编辑</span></button><button type="button" onClick={(event) => { event.stopPropagation(); setPendingDelete(item); setModal("deleteConfirm"); }} aria-label="删除"><Trash2 size={13} /><span>删除</span></button></> : null}</div>
    </article>;
  };
  ...
  const renderReceivedList = () => sourceSections.map((section) => {
    ...
    const bodyItems = prefs.grouped
      ? [...new Set(section.items.map((item) => item.issuer))].map((name) => (
          <section className="vault-group is-nested-batch" key={`${section.key}:${name}`}>
            <header><b>{name}</b><span>{section.items.filter((item) => item.issuer === name).length}</span></header>
            {renderGroupGrid(section.items.filter((item) => item.issuer === name))}
          </section>
        ))
      : [renderGroupGrid(section.items)];
    return (
      <section className={`vault-group is-received-source${collapsed ? " is-collapsed" : ""}`} key={section.key}>
        <header className="vault-received-group-head is-source" role="button" tabIndex={0} aria-expanded={!collapsed} onClick={() => toggleCollapsed(section.key)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggleCollapsed(section.key); } }}>
          <span className={`vault-group-chevron${collapsed ? "" : " is-open"}`} aria-hidden="true"><ChevronDown size={15} /></span>
          <span className="vault-received-avatar" aria-hidden="true">{section.avatar}</span>
          <div className="vault-received-group-copy">
            <b>{sharerDisplay(section)}</b>
            <small>{section.items.length} 项凭据</small>
          </div>
          <span className="vault-received-group-count">{section.items.length}</span>
        </header>
        {collapsed ? null : <div className="vault-received-source-body">{bodyItems}</div>}
      </section>
    );
  });
  ...
  const selectedIssuers = [...new Set(selectedCredentials.map((item) => item.issuer))];
  const shareConfigItems = !shareDetail ? [] : shareDetail.inbound
    ? credentials.filter((item) => item.shared && item.shareId === shareDetail.id).map((item) => ({ id: item.id, issuer: item.issuer, accountName: item.accountName }))
    : shareDetailCredentials(shareDetail.credentialIds, ownCredentials);
  ...
  return <div className="vault-page" onPointerDown={handlePagePointerDown}>
    ...
    <nav className="vault-tabs" aria-label="密钥管理导航">
      {VAULT_NAV.map(([key, Icon, label]) => <button type="button" className={view === key ? "is-active" : ""} onClick={() => changeView(key)} key={key}><Icon size={15} />{label}</button>)}
    </nav>
      ...
      <header className="vault-panel-head">
        ...
        <div className="vault-panel-tools">
          ...
          <div className={`vault-filters${filtersOpen ? " is-open" : ""}`}>
            ...
            {prefs.grouped && credentialTab !== "received" ? <label><span>分组</span><select aria-label="分组方式" value={prefs.groupBy || "system"} onChange={(event) => void updatePrefs({ ...prefs, groupBy: event.target.value as "system" | "tag" })}><option value="system">按系统</option><option value="tag">按标签</option></select></label> : null}
          </div>
        </div>
      </header>
    ...
    {view === "text" ? <section className="vault-panel vault-text-workspace vault-view-enter" key="text">
      <header className="vault-panel-head">
        <div><span className="vault-panel-title-row"><h2>笔记</h2></span><p>安全发送文字、Markdown、代码或 HTML，过期自动失效，也可以随时撤销</p></div>
        {textShares.length ? <div className="vault-panel-tools"><div className="vault-search"><Search size={15} /><input value={textQuery} onChange={(event) => setTextQuery(event.target.value)} placeholder="搜索名称或正文" aria-label="搜索笔记" />{textQuery ? <button type="button" onClick={() => setTextQuery("")} aria-label="清空搜索"><X size={14} /></button> : null}</div></div> : null}
      </header>
      <div className="vault-text-overview">
        <div className="vault-text-overview-copy"><span className="vault-text-overview-icon"><MessageSquareText size={20} /></span><div><small>SECURE TEXT</small><h3>发一段只在需要时存在的文字</h3><p>链接可设访问码、有效期和查看次数；对方无需登录，打开即可阅读。</p></div></div>
        <div className="vault-text-overview-stats" aria-label="笔记概览"><span><b>{activeTextShareCount}</b><small>有效分享</small></span><span><b>{textShares.length}</b><small>全部分享</small></span><span><b>{textShareAccessCount}</b><small>累计查看</small></span></div>
        <button type="button" className="vault-text-overview-create" onClick={openTextShare}>新建笔记</button>
      </div>
      {textShares.length ? <div className="vault-text-folders" role="tablist" aria-label="笔记目录">
        <button type="button" className={textFolderFilter === "all" ? "is-active" : ""} onClick={() => setTextFolderFilter("all")}>全部<b>{textShares.length}</b></button>
        {textShares.some((share) => !share.textFolderId) ? <button type="button" className={textFolderFilter === "none" ? "is-active" : ""} onClick={() => setTextFolderFilter("none")}>未分组<b>{textShares.filter((share) => !share.textFolderId).length}</b></button> : null}
        {textFolders.map((folder) => <button type="button" key={folder.id} className={textFolderFilter === folder.id ? "is-active" : ""} onClick={() => setTextFolderFilter(folder.id)}>{folder.name}<b>{textShares.filter((share) => share.textFolderId === folder.id).length}</b></button>)}
        <button type="button" className="vault-text-folder-add" onClick={() => setFolderModal({ mode: "create", name: "" })}><Plus size={13} />新建目录</button>
        {typeof textFolderFilter === "number" ? <>
          <button type="button" className="vault-text-folder-manage" onClick={() => { const folder = textFolders.find((item) => item.id === textFolderFilter); if (folder) setFolderModal({ mode: "rename", id: folder.id, name: folder.name }); }}><Pencil size={12} />重命名</button>
          <button type="button" className="vault-text-folder-manage is-danger" onClick={() => { const folder = textFolders.find((item) => item.id === textFolderFilter); if (folder) setFolderDeleteTarget(folder); }}><Trash2 size={12} />删除</button>
        </> : null}
      </div> : null}
      {textShares.length ? filteredTextShares.length ? <div className="vault-text-share-list">{filteredTextShares.map((share) => <article className={`vault-text-card is-${share.status.toLowerCase()}`} key={share.id}>
        <header className="vault-text-card-head">
          <span className="vault-text-card-icon"><FileText size={17} /></span>
          <button type="button" className="vault-text-card-title" onClick={() => void openShareDetail(share)}><b>{share.name?.trim() || "临时笔记"}</b><small>{textShareFormatLabel(share.textFormat)}</small></button>
        ...
        <button type="button" className={`vault-text-card-preview${share.textContent ? "" : " is-empty"}`} onClick={() => void openShareDetail(share)}>{share.textContent ? <span>{share.textContent.slice(0, 150).replace(/\s+/g, " ")}{share.textContent.length > 150 ? "…" : ""}</span> : <span>打开详情查看分享内容</span>}<ChevronRight size={15} /></button>
        <div className="vault-text-card-meta"><span><Clock3 size={12} />{share.status === "ACTIVE" ? `剩余 ${formatRemaining(share.expireTime, now)}` : new Date(normalizeDateTime(share.expireTime)).toLocaleDateString("zh-CN")}</span><span><ShieldCheck size={12} />{share.accessCodeEnabled ? "访问码保护" : "免密码"}</span><span><Eye size={12} />{share.accessCount}{share.maxAccessCount ? `/${share.maxAccessCount}` : ""} 次</span></div>
        <footer className="vault-text-card-actions"><button type="button" onClick={() => void openShareDetail(share)}><Eye size={14} />详情</button>{share.status === "ACTIVE" ? <button type="button" onClick={() => void openShareEdit(share)}><Pencil size={14} />编辑</button> : null}{share.status === "ACTIVE" && share.sharePath ? <button type="button" className="is-primary" onClick={() => void copyShareInfo(share)}><Copy size={14} />复制链接</button> : null}{share.status !== "REVOKED" ? <button type="button" className="is-danger" onClick={() => { setPendingRevoke({ share, from: "list" }); setModal("revokeConfirm"); }}><X size={14} />撤销</button> : <button type="button" className="is-danger" onClick={() => { setPendingShareDelete(share); setModal("shareDeleteConfirm"); }}><Trash2 size={14} />删除</button>}</footer>
      </article>)}</div> : <div className="vault-empty vault-text-empty"><Search size={20} /><b>没有匹配的笔记</b><p>换个关键词试试，名称和正文都可以搜索。</p><button type="button" className="vault-ghost" onClick={() => setTextQuery("")}>清空搜索</button></div> : <div className="vault-empty vault-text-empty"><FileText size={20} /><b>第一段分享，从这里开始</b><p>粘贴文字或 Markdown，设置有效期后即可生成安全链接。</p><button type="button" className="vault-primary" onClick={openTextShare}><Plus size={15} />新建笔记</button></div>}
    </section> : null}

    {view === "shares" ? <section className="vault-panel vault-view-enter" key="shares">
      <header className="vault-panel-head"><div><span className="vault-panel-title-row"><h2>授权</h2><div className="vault-fav-switch" role="tablist" aria-label="授权筛选"><button type="button" className={shareTab === "sent" ? "is-active" : ""} onClick={() => setShareTab("sent")}>我发出的</button><button type="button" className={shareTab === "received" ? "is-active" : ""} onClick={() => setShareTab("received")}>我收到的</button></div></span><p>{shareTab === "received" ? `指定给你的授权，以及你转存进来的 · ${receivedShares.length} 项` : `临时凭据授权，均可随时撤销 · ${credentialShares.length} 项`}</p></div></header>
      {shareTab === "received" ? <div className="vault-share-list">{receivedShares.length ? receivedShares.map((share) => <article key={share.id} role="button" tabIndex={0} onClick={openReceivedCredentials} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openReceivedCredentials(); } }}>
        <span className={`vault-status is-${share.status.toLowerCase()}`}>{share.status === "ACTIVE" ? "有效" : share.status === "EXPIRED" ? "已过期" : share.status === "LIMIT_REACHED" ? "次数已用完" : "已撤销"}</span>
        <div><b>{share.name?.trim() || "临时凭据授权"}</b><small><Clock3 size={12} />{share.itemCount} 个凭据 · 来自 {sharerDisplay(share)} · {receivedShareSourceLabel(share.source, share.shareMode)} · {share.status === "ACTIVE" ? `剩余 ${formatRemaining(share.expireTime, now)}` : new Date(normalizeDateTime(share.expireTime)).toLocaleString("zh-CN", { hour12: false })}</small></div>
        <div className="vault-share-actions"><button type="button" onClick={(event) => { event.stopPropagation(); openReceivedShareDetail(share); }}><Eye size={14} />详情</button><button type="button" onClick={(event) => { event.stopPropagation(); openReceivedCredentials(); }}><Eye size={14} />查看凭据</button><button type="button" onClick={(event) => { event.stopPropagation(); setPendingRelease(share); setModal("releaseConfirm"); }}><UserMinus size={14} />解除</button></div>
      </article>) : <div className="vault-empty compact"><Inbox size={20} /><b>还没有收到的授权</b><p>指定给你的授权，或从分享链接转存的内容会出现在这里。</p></div>}</div> : <div className="vault-share-list">{credentialShares.length ? credentialShares.map((share) => <article key={share.id} role="button" tabIndex={0} onClick={() => void openShareDetail(share)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openShareDetail(share); } }}>
        <span className={`vault-status is-${share.status.toLowerCase()}`}>{share.status === "ACTIVE" ? "有效" : share.status === "EXPIRED" ? "已过期" : share.status === "LIMIT_REACHED" ? "次数已用完" : "已撤销"}</span>
      ...
      </article>) : <div className="vault-empty compact"><Share2 size={20} /><b>还没有创建临时授权</b><p>选择自己的凭据，生成限时链接或指定给某个用户。</p>{ownCredentials.length ? <button type="button" className="vault-primary" onClick={() => openShare()}>创建授权</button> : null}</div>}</div>}
    </section> : null}
    ...
    {view === "settings" ? <section className="vault-settings vault-view-enter" key="settings"><header className="vault-panel-head"><div>{settingsSection ? <button type="button" className="vault-settings-back" onClick={() => setSettingsSection(null)}><ArrowLeft size={15} />返回</button> : null}{settingsSection !== "security" ? <><h2>{settingsSection === "account" ? "账号" : settingsSection === "appearance" ? "显示" : settingsSection === "notifications" ? "通知" : settingsSection === "about" ? "关于" : "设置"}</h2><p>{settingsSection === "account" ? "用户名、账号、邮箱和登录" : settingsSection === "appearance" ? "主题、卡片和列表偏好" : settingsSection === "notifications" ? "站内信始终送达；邮件、Bark 和系统通知可按事件选择" : settingsSection === "about" ? "" : "安全、账号和显示偏好会跟随当前账号"}</p></> : null}</div></header>
      ...
      {settingsSection === "account" ? <div className="vault-settings-group vault-account-settings vault-subview-enter" key="settings-account">
      ...
      {settingsSection === "appearance" ? <div className="vault-settings-group vault-settings-appearance vault-subview-enter" key="settings-appearance">
        ...
        {([['masked', EyeOff, '隐藏账号', '在列表中遮住账号主体', 'violet'], ['compact', LayoutGrid, '紧凑卡片', '缩小留白，一屏看到更多内容', 'blue'], ['grouped', Layers3, '分组', '打开后在筛选里选择按系统或按标签分组', 'green'], ['showShared', User, '显示共享', '在全部列表中展示别人分享给我的凭据', 'blue'], ['autoRefresh', Clock3, '自动刷新', '定时同步授权状态和新增共享', 'green'], ['concealOtp', EyeOff, '隐蔽验证码', '列表中先显示掩码，点按后再显示并复制', 'violet'], ['defaultFavorites', Star, '默认显示收藏', '打开后进入凭据页默认只看收藏，关闭则显示全部', 'violet']] as const).map(([key, Icon, title, detail, tone]) => <label className="vault-setting-row" key={key}><span className={`vault-setting-icon is-${tone}`}><Icon size={17} /></span><span className="vault-setting-copy"><b>{title}</b><small>{detail}</small></span><input type="checkbox" checked={Boolean(prefs[key])} onChange={(event) => { if (key === "concealOtp") setRevealedOtp(null); void updatePrefs({ ...prefs, [key]: event.target.checked }); }} /><i /></label>)}
      ...
      {settingsSection === "about" ? <div className="vault-settings-group vault-settings-about vault-subview-enter" key="settings-about">
    ...
    {view === "all" ? <button type="button" className="vault-ghost vault-import-action" onClick={() => setModal("importChoice")} aria-label="添加或导入凭据"><Plus size={18} /><span>添加</span></button> : null}
    {view === "shares" && shareTab === "sent" ? <button type="button" className="vault-create-share" onClick={() => openShare()} disabled={!ownCredentials.length} aria-label="创建授权"><Share2 size={14} /><span>创建授权</span></button> : null}
    {view === "text" ? <button type="button" className="vault-create-share" onClick={openTextShare} aria-label="新建笔记"><FileText size={14} /><span>新建分享</span></button> : null}

    <nav className="vault-mobile-nav" aria-label="密钥管理导航">
      {VAULT_NAV.map(([key, Icon, label]) => <button type="button" className={view === key ? "is-active" : ""} onClick={() => changeView(key)} key={key}><Icon size={18} /><span>{label}</span></button>)}
    </nav>
      ...
      {detailLoading || !liveDetail ? <div className="vault-share-scroll"><div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在安全读取…</div></div> : <>
        <div className="vault-share-scroll">
          ...
          <section className="vault-share-section vault-detail-section"><div className="vault-section-title"><div><span>01</span><h3>登录信息</h3></div><small>{liveDetail.shared ? `${liveDetail.sharedBy} 分享 · ${liveDetail.shareExpireTime ? formatRemaining(liveDetail.shareExpireTime, now) : "临时有效"}` : "敏感信息仅在需要时显示"}</small></div><div className="vault-detail-values">
            ...
            {liveDetail.loginUrl ? <section className="is-login-url"><span>访问地址</span><div><a className="vault-detail-login-url" href={liveDetail.loginUrl} target="_blank" rel="noreferrer">{liveDetail.loginUrl}</a><a className="vault-detail-open-url" href={liveDetail.loginUrl} target="_blank" rel="noreferrer" aria-label="打开访问地址"><ExternalLink size={15} /></a>{canCopyCredential(liveDetail) ? <button type="button" onClick={() => void copy(liveDetail.loginUrl || "", "访问地址已复制")} aria-label="复制访问地址"><Copy size={15} /></button> : null}</div></section> : null}
            {liveDetail.currentOtp ? <section className="is-otp"><span className="vault-otp-label">{liveDetail.otpType === "HOTP" ? `HOTP 验证码 · 计数 ${liveDetail.hotpCounter || 0}` : "验证器验证码"}{liveDetail.otpType !== "HOTP" && liveDetail.otpValidUntil ? <em>{detailOtpLeft}秒</em> : null}</span><div><b>{liveDetail.currentOtp.replace(/(.{3})/, "$1 ")}</b>{canCopyCredential(liveDetail) ? <button type="button" onClick={() => void copyOtp(liveDetail)} aria-label="复制验证码"><Copy size={15} /></button> : null}{!liveDetail.shared && liveDetail.otpType === "HOTP" ? <button type="button" disabled={busy} onClick={() => void advanceHotp(liveDetail.id)}>下一个</button> : null}</div>{liveDetail.nextOtp && liveDetail.otpType !== "HOTP" ? <div className="vault-detail-next"><span>下一组</span><b>{liveDetail.nextOtp.replace(/(.{3})/, "$1 ")}</b><small>{detailOtpLeft}秒后启用</small>{canCopyCredential(liveDetail) ? <button type="button" className="vault-next-copy" onClick={() => void copy(liveDetail.nextOtp || "", "下一组验证码已复制")} aria-label="复制下一组验证码"><Copy size={12} /></button> : null}</div> : null}{liveDetail.otpType !== "HOTP" && liveDetail.otpValidUntil ? <div className="vault-progress"><i style={{ width: `${detailOtpProgress}%` }} /></div> : null}</section> : null}
    ...
    {modal === "credential" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><form className="vault-modal share vault-share-form vault-credential-form" onSubmit={submitCredential}>
      ...
      <label><span>账号</span><input required maxLength={160} value={form.accountName} onChange={(e) => setForm({ ...form, accountName: e.target.value })} placeholder="邮箱或用户名" /></label>
        ...
        <details className="vault-credential-advanced"><summary><span><Layers3 size={15} /><span><b>更多信息</b><small>登录地址、备注、标签与查看保护</small></span></span><em>展开</em></summary><div className="vault-form-grid">
    ...
    {modal === "inboundChannels" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className="vault-modal share vault-share-form vault-inbound-modal">
      <header><div><small>INBOUND CHANNELS</small><h2>验证码接收通道</h2><p>点开通道复制地址，或查看尚未归类的验证码</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header>
      <div className="vault-share-scroll">
    ...
    {modal === "import" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><form className="vault-modal share vault-share-form vault-import-form" onSubmit={submitImport}>
      ...
      <div className="vault-share-scroll">
        {!importPreview.length ? <>
          ...
          <section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>逐项核对</h3></div></div><div className="vault-import-preview">{importPreview.slice(0, 100).map((item, index) => <article key={`${item.issuer}-${item.accountName}-${index}`}><span className={`is-${item.status.toLowerCase()}`}>{item.status === "NEW" ? "新增" : item.status === "DUPLICATE" ? "重复" : "冲突"}</span><div><b>{item.issuer}</b><small>{item.accountName}</small></div></article>)}</div>{importPreview.length > 100 ? <p className="vault-section-help">只展示前 100 项，其余 {importPreview.length - 100} 项会按同一规则处理。</p> : null}</section>
          {importCounts.conflict ? <section className="vault-share-section"><div className="vault-section-title"><div><span>03</span><h3>冲突处理</h3></div></div><label className="vault-check"><input type="checkbox" checked={importReplaceExisting} onChange={(event) => setImportReplaceExisting(event.target.checked)} /><span><b>用导入内容覆盖现有凭据</b><small>关闭时跳过冲突；重复项目始终跳过</small></span></label></section> : null}
        </>}
        {importProgress ? <div className="vault-import-progress" role="status"><span><b>{importProgress.label}</b><small>{importProgress.percent}%</small></span><i><b style={{ width: `${importProgress.percent}%` }} /></i></div> : null}
      </div>
      <footer><span>{importPreview.length ? `将新增 ${importCounts.added} 项${importReplaceExisting ? `、覆盖 ${importCounts.conflict} 项` : ""}` : legacyText.trim() ? "先预览，不会直接写入数据" : "请选择文件或粘贴文件内容"}</span><div>{importPreview.length ? <button type="button" className="vault-ghost" disabled={busy} onClick={() => { setImportItems([]); setImportPreview([]); setImportReplaceExisting(false); }}>返回修改</button> : <button type="button" className="vault-ghost" onClick={closeModal}>取消</button>}<button className="vault-primary" disabled={busy || !legacyText.trim()}>{busy ? <LoaderCircle className="spin" size={15} /> : importPreview.length ? <Check size={15} /> : <Eye size={15} />}{busy ? importProgress?.label || "处理中" : importPreview.length ? "确认导入" : "预览导入"}</button></div></footer>
    </form></div> : null}
    ...
    {modal === "shareDetail" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className="vault-modal share vault-share-form vault-share-detail">
      <header><div><small>AUTHORIZATION DETAIL</small><h2>{shareDetail?.name?.trim() || (shareDetail?.shareType === "TEXT" ? "临时笔记" : "临时授权详情")}</h2><p>{shareDetail ? (shareDetail.shareType === "TEXT" ? `笔记 · ${textShareFormatLabel(shareDetail.textFormat)} · 链接分享` : shareDetail.inbound ? `${shareDetail.itemCount} 个凭据 · 来自 ${sharerDisplay(shareDetail)}` : `${shareDetail.itemCount} 个凭据 · ${shareDetail.shareMode === "DIRECT" ? "指定用户" : "链接分享"}`) : "正在读取授权配置"}</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header>
      {shareDetailLoading || !shareDetail ? <div className="vault-share-scroll"><div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在读取授权配置…</div></div> : <>
        <div className="vault-share-scroll">
          <section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>授权状态</h3></div><span className={`vault-status is-${shareDetail.status.toLowerCase()}`}>{shareDetail.status === "ACTIVE" ? "有效" : shareDetail.status === "EXPIRED" ? "已过期" : shareDetail.status === "LIMIT_REACHED" ? "次数已用完" : "已撤销"}</span></div><div className="vault-share-detail-hero"><div><small>剩余有效时间</small><b>{shareDetail.status === "ACTIVE" ? formatRemaining(shareDetail.expireTime, now) : "授权已结束"}</b></div></div><div className="vault-share-detail-grid"><section><span>授权内容</span><b>{shareDetail.shareType === "TEXT" ? `笔记 · ${textShareFormatLabel(shareDetail.textFormat)}` : `${shareDetail.itemCount} 个凭据`}</b></section><section><span>{shareDetail.inbound ? "分享者" : shareDetail.shareMode === "DIRECT" ? "接收人" : "访问次数"}</span><b>{shareDetail.inbound ? sharerDisplay(shareDetail) : shareDetail.shareMode === "DIRECT" ? shareDetail.recipientUsername : `${shareDetail.accessCount}${shareDetail.maxAccessCount ? ` / ${shareDetail.maxAccessCount}` : " / 不限"}`}</b></section><section><span>访问方式</span><b>{shareDetail.shareMode === "DIRECT" ? "指定用户" : shareDetail.accessCodeEnabled ? "访问码验证" : "链接直达"}</b></section><section><span>复制权限</span><b>{shareDetail.allowCopy ? "允许复制" : "仅查看"}</b></section>{!shareDetail.inbound && shareDetail.shareMode === "LINK" ? <><section><span>转存</span><b>{shareDetail.forbidSave ? "禁止转存" : "允许转存"}</b></section><section><span>访问策略</span><b>{shareDetail.oneTime ? "一次性访问" : shareDetail.maxAccessCount ? `最多 ${shareDetail.maxAccessCount} 次` : "不限次数"}</b></section></> : null}</div></section>
          <section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>分享配置</h3></div></div>{shareDetail.shareType === "TEXT" ? <article className={`vault-text-preview is-${normalizeTextShareFormat(shareDetail.textFormat).toLowerCase()}`}>{shareDetail.textContent ? <div className={`vault-md-body ${normalizeTextShareFormat(shareDetail.textFormat) === "CODE" ? "is-code" : normalizeTextShareFormat(shareDetail.textFormat) === "HTML" ? "is-html" : normalizeTextShareFormat(shareDetail.textFormat) === "MARKDOWN" ? "is-markdown" : "is-text"}`} dangerouslySetInnerHTML={{ __html: renderRichText(shareDetail.textContent.length > 1200 ? `${shareDetail.textContent.slice(0, 1200)}\n\n…（内容过长，仅展示前 1200 字）` : shareDetail.textContent, textShareRenderType(shareDetail.textFormat)) }} /> : <p className="vault-section-help">笔记内容为空</p>}</article> : <>{shareConfigItems.length ? <div className="vault-share-detail-items">{shareConfigItems.map((item) => <article key={item.id}><div><b>{item.issuer}</b><small>{item.accountName}</small></div></article>)}</div> : null}<div className="vault-permission-tags">{([["showAccount", "账号"], ["showPassword", "密码"], ["showOtp", "OTP"], ["showLoginUrl", "登录地址"], ["showNote", "备注"]] as const).map(([key, label]) => <b key={label} className={shareDetail[key] ? undefined : "is-off"}>{label}{shareDetail[key] ? "" : " · 未授权"}</b>)}</div></>}{!shareDetail.inbound ? <>{shareDetail.sharePath ? <label><span>{shareDetail.shareType === "TEXT" ? "笔记链接" : "授权链接"}</span><div className="vault-copy-row"><input readOnly value={`${location.origin}${shareDetail.sharePath}`} /><button type="button" onClick={() => void copyShareInfo(shareDetail)} aria-label="复制完整分享信息"><Copy size={15} /></button></div></label> : null}{shareDetail.shareMode === "DIRECT" ? <p className="vault-modal-note">该授权只会显示在 {shareDetail.recipientUsername} 的 OTP Vault 中，到期或撤销后自动消失。</p> : shareDetail.accessCodeEnabled ? <label><span>访问码</span>{shareDetail.accessCode ? <button type="button" className="vault-access-code vault-access-code-button is-detail" onClick={() => void copy(shareDetail.accessCode || "", "访问码已复制")}>{shareDetail.accessCode}<Copy size={15} /></button> : <p className="vault-modal-note">这是升级前创建的授权，当时只保存了不可逆哈希，无法还原访问码。请重新创建授权。</p>}</label> : <p className="vault-modal-note">此授权未启用访问码，获得链接即可访问。</p>}</> : null}</section>
          {!shareDetail.inbound && shareDetail.shareMode === "LINK" && shareDetail.shareType !== "TEXT" ? <section className="vault-share-section"><div className="vault-section-title"><div><span>03</span><h3>转存列表</h3></div><button type="button">{shareDetail.saveRecords?.length || 0} 条</button></div><p className="vault-section-help">移除后对方可再次转存，也可在这里直接恢复，并会收到通知；禁止后对方不能自行转存，但你可以随时恢复，且不会通知对方，再转存会提示“该链接已被分享者禁止转存”。</p><div className="vault-access-records vault-save-records">{shareDetail.saveRecords?.length ? <div>{shareDetail.saveRecords.map((record) => <article key={record.id} className={record.status === "ACTIVE" ? "is-success" : "is-failed"}><span>{record.status === "BANNED" ? <Ban size={15} /> : record.status === "KICKED" ? <UserMinus size={15} /> : <User size={15} />}</span><div><b>{record.nickname || record.username || "未知用户"}</b><time>{record.createTime ? new Date(normalizeDateTime(record.createTime)).toLocaleString("zh-CN", { hour12: false }) : saveStatusLabel(record.status)}</time></div><div className="vault-save-actions">{record.status === "ACTIVE" ? <button type="button" disabled={busy} onClick={() => { setPendingSaveAction({ saveId: record.id, action: "kick", name: record.nickname || record.username || "未知用户" }); setModal("saveActionConfirm"); }}><UserMinus size={13} />移除</button> : <button type="button" disabled={busy} onClick={() => void manageShareSave(record.id, "restore")}><RotateCcw size={13} />恢复</button>}{record.status !== "BANNED" ? <button type="button" disabled={busy} onClick={() => { setPendingSaveAction({ saveId: record.id, action: "ban", name: record.nickname || record.username || "未知用户" }); setModal("saveActionConfirm"); }}><Ban size={13} />禁止</button> : null}</div></article>)}</div> : <p>还没有人转存</p>}</div></section> : null}
          {!shareDetail.inbound && shareDetail.shareMode === "LINK" ? <section className="vault-share-section"><div className="vault-section-title"><div><span>04</span><h3>访问记录</h3></div><button type="button">{shareDetail.accessRecords?.length || 0} 条</button></div><div className="vault-access-records">{shareDetail.accessRecords?.length ? <div>{shareDetail.accessRecords.map((record, index) => <article key={`${record.createTime}-${index}`} className={record.success ? "is-success" : "is-failed"}><span>{record.success ? <ShieldCheck size={15} /> : <ShieldAlert size={15} />}</span><div><b>{accessActionLabel(record.action, record.success)}</b><small>{record.ipAddress || "未知 IP"} · {ipDetailLabel(record.ipInfo)} · {deviceLabel(record.userAgent)}</small>{record.detail ? <em>{record.detail}</em> : null}</div><time>{new Date(normalizeDateTime(record.createTime)).toLocaleString("zh-CN", { hour12: false })}</time></article>)}</div> : <p>还没有访问记录</p>}</div></section> : null}
        </div>
        <footer><span>{shareDetail.inbound ? `来自 ${sharerDisplay(shareDetail)}` : shareDetail.status === "ACTIVE" ? "授权仍在有效期内" : "授权已结束"}</span><div><button type="button" className="vault-ghost vault-detail-close-action" onClick={closeModal}>关闭</button>{shareDetail.inbound ? <button type="button" className="vault-primary" onClick={() => { closeModal(); openReceivedCredentials(); }}>查看凭据</button> : <>{shareDetail.status === "ACTIVE" && shareDetail.shareMode === "LINK" && shareDetail.sharePath ? <button type="button" className="vault-ghost vault-detail-share-action" onClick={() => void shareShareInfo(shareDetail)}><Share2 size={14} />分享</button> : null}{shareDetail.status === "ACTIVE" ? <button type="button" className="vault-primary" onClick={() => void openShareEdit(shareDetail)}><Pencil size={14} />编辑授权</button> : null}{shareDetail.status !== "REVOKED" ? <button type="button" className="vault-danger" onClick={() => { setPendingRevoke({ share: shareDetail, from: "detail" }); setModal("revokeConfirm"); }}>撤销授权</button> : <button type="button" className="vault-danger" onClick={() => { setPendingShareDelete(shareDetail); setModal("shareDeleteConfirm"); }}>删除记录</button>}</>}</div></footer>
      </>}
    </section></div> : null}

    {modal === "textShare" ? <div className="vault-modal-mask"><form className="vault-modal share vault-share-form vault-text-share-form" onSubmit={submitTextShare}>
      <header><div><small>TEMPORARY NOTE</small><h2>{textShareEditingId ? "编辑笔记" : "分享笔记"}</h2><p>{textShareEditingId ? "内容与权限保存后立即应用，原链接保持不变" : "生成限时链接，对方打开后即可阅读"}</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header>
      <div className="vault-share-scroll">
        <section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>笔记内容</h3></div><span className="vault-count-pill">{textShareForm.textContent.length} / 50000</span></div><div className="vault-form-grid vault-single-field"><label className="wide"><span>分享名称</span><input maxLength={40} value={textShareForm.name} onChange={(event) => setTextShareForm({ ...textShareForm, name: event.target.value })} placeholder="例如 临时说明" /></label></div><p className="vault-section-help">名称会出现在列表、对方打开的页面和复制文案第一行</p><div className="vault-share-mode is-text-formats" role="group" aria-label="分享类型">{([["TEXT", FileText], ["MARKDOWN", MessageSquareText], ["CODE", Code2], ["HTML", FileCode2]] as const).map(([format, Icon]) => <button type="button" className={textShareForm.textFormat === format ? "is-active" : ""} onClick={() => setTextShareForm({ ...textShareForm, textFormat: format })} key={format}><Icon size={16} /><span><b>{textShareFormatLabel(format)}</b><small>{textShareFormatHint(format)}</small></span></button>)}</div><label className={`vault-text-editor${textShareForm.textFormat === "CODE" ? " is-code" : ""}`}><span>要分享的内容</span><textarea required maxLength={50000} value={textShareForm.textContent} onChange={(event) => setTextShareForm({ ...textShareForm, textContent: event.target.value })} placeholder={textSharePlaceholder(textShareForm.textFormat)} spellCheck={textShareForm.textFormat !== "CODE" && textShareForm.textFormat !== "HTML"} /></label>{textShareForm.textContent.trim() ? <div className="vault-text-live-preview"><header><Eye size={13} /><b>实时预览</b><em>{textShareFormatLabel(textShareForm.textFormat)}</em></header><article className={`vault-md-body ${textShareForm.textFormat === "CODE" ? "is-code" : textShareForm.textFormat === "HTML" ? "is-html" : textShareForm.textFormat === "MARKDOWN" ? "is-markdown" : "is-text"}`} dangerouslySetInnerHTML={{ __html: renderRichText(textShareForm.textContent, textShareRenderType(textShareForm.textFormat)) }} /></div> : null}</section>
        <section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>有效期与访问</h3></div></div><div className="vault-duration"><div className="vault-duration-head"><span><Clock3 size={15} /></span><div><small>{textShareEditingId ? "从现在起" : "分享有效时间"}</small><b>{durationHeadline(textShareForm.durationSeconds)}</b></div></div><div className="vault-duration-presets">{DURATION_PRESETS.map((item) => <button type="button" className={textShareForm.durationSeconds === item.days * 86400 ? "is-active" : ""} onClick={() => setTextShareForm({ ...textShareForm, durationSeconds: item.days * 86400 })} key={item.days}>{item.label}</button>)}</div><label><span>自定义</span><input type="number" min={1} max={365} inputMode="numeric" value={isLongTerm(textShareForm.durationSeconds) ? "" : Math.max(1, Math.round(textShareForm.durationSeconds / 86400))} onChange={(event) => { const days = Number(event.target.value); if (days >= 1 && days <= 365) setTextShareForm({ ...textShareForm, durationSeconds: days * 86400 }); }} /><em>天</em></label></div><label className="vault-text-folder-field"><span>所在目录</span><select value={textShareForm.textFolderId ?? ""} onChange={(event) => setTextShareForm({ ...textShareForm, textFolderId: event.target.value ? Number(event.target.value) : null })}><option value="">不分组</option>{textFolders.map((folder) => <option value={folder.id} key={folder.id}>{folder.name}</option>)}</select></label>{textShareEditingId ? <p className="vault-modal-note">原链接和访问码保持不变。</p> : <div className="vault-toggle-row"><label className="vault-check"><input type="checkbox" checked={textShareForm.accessCodeEnabled} onChange={(event) => setTextShareForm({ ...textShareForm, accessCodeEnabled: event.target.checked })} /><span><b>访问码保护</b><small>{textShareForm.accessCodeEnabled ? "打开链接后还需输入访问码" : "获得安全链接即可直接阅读"}</small></span></label>{textShareForm.accessCodeEnabled ? <div className="vault-inline"><label className="vault-check"><input type="radio" checked={textShareForm.accessCodeMode === "AUTO"} onChange={() => setTextShareForm({ ...textShareForm, accessCodeMode: "AUTO" })} /><span>自动生成</span></label><label className="vault-check"><input type="radio" checked={textShareForm.accessCodeMode === "CUSTOM"} onChange={() => setTextShareForm({ ...textShareForm, accessCodeMode: "CUSTOM" })} /><span>自定义</span></label>{textShareForm.accessCodeMode === "CUSTOM" ? <input required minLength={4} maxLength={12} value={textShareForm.accessCode} onChange={(event) => setTextShareForm({ ...textShareForm, accessCode: event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "") })} placeholder="4-12 位" /> : null}</div> : null}</div>}</section>
        <section className="vault-share-section"><div className="vault-section-title"><div><span>03</span><h3>阅读权限</h3></div></div><div className="vault-form-grid vault-access-options"><label className="vault-check"><input type="checkbox" checked={textShareForm.allowCopy} onChange={(event) => setTextShareForm({ ...textShareForm, allowCopy: event.target.checked })} /><span><b>允许复制</b><small>关闭后阅读页不显示复制按钮</small></span></label><label className="vault-check"><input type="checkbox" disabled={Boolean(textShareEditingId && shareDetail && shareDetail.accessCount > 1)} checked={textShareForm.oneTime} onChange={(event) => setTextShareForm({ ...textShareForm, oneTime: event.target.checked })} /><span><b>一次性阅读</b><small>{textShareEditingId && shareDetail && shareDetail.accessCount > 1 ? "已访问多次，不能改为一次性" : "成功打开一次后立即失效"}</small></span></label><label><span>最大访问次数（可选）</span><input type="number" min={Math.max(1, shareDetail?.accessCount || 1)} max={1000} disabled={textShareForm.oneTime} value={textShareForm.oneTime ? "1" : textShareForm.maxAccessCount} onChange={(event) => setTextShareForm({ ...textShareForm, maxAccessCount: event.target.value })} placeholder="不填则不限" /></label></div><p className="vault-section-help">笔记只允许在线阅读，不支持转存到保险库；正文在数据库中加密保存。</p></section>
      </div>
      <footer><span>{textShareForm.textContent.trim() ? `${textShareFormatLabel(textShareForm.textFormat)} · ${durationHeadline(textShareForm.durationSeconds)} · ${textShareForm.allowCopy ? "允许复制" : "仅阅读"}${textShareEditingId ? "" : textShareForm.accessCodeEnabled ? " · 访问码保护" : " · 免密码"}` : "请输入笔记内容"}</span><div><button type="button" className="vault-ghost" onClick={closeModal}>取消</button><button className="vault-primary" disabled={busy || !textShareForm.textContent.trim()}>{busy ? "保存中" : textShareEditingId ? "保存修改" : "生成分享"}</button></div></footer>
    </form></div> : null}

    {modal === "shareCreateConfirm" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setModal("share"); }}><section className="vault-modal share vault-share-form vault-delete-modal"><header><div><small>CONFIRM ACCESS</small><h2>确认生成授权</h2><p>核对有效期、内容和平台后再生成</p></div><button type="button" onClick={() => { if (!busy) setModal("share"); }} aria-label="关闭"><X size={18} /></button></header><div className="vault-share-scroll"><section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>有效期</h3></div></div><div className="vault-share-detail-hero vault-share-confirm-duration"><div><small>授权时间</small><b>{durationHeadline(shareForm.durationSeconds)}</b></div></div></section><section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>分享内容</h3></div></div><div className="vault-share-detail-items">{selectedCredentials.map((item) => <article key={item.id}><div><b>{item.issuer}</b><small>{item.accountName}</small></div></article>)}</div><div className="vault-permission-tags">{([["showAccount", "账号"], ["showPassword", "密码"], ["showOtp", "OTP"], ["showLoginUrl", "登录地址"], ["showNote", "备注"]] as const).map(([key, label]) => <b key={label} className={shareForm[key] ? undefined : "is-off"}>{label}{shareForm[key] ? "" : " · 未授权"}</b>)}</div></section><section className="vault-share-section"><div className="vault-section-title"><div><span>03</span><h3>平台</h3></div></div><div className="vault-share-detail-items">{selectedIssuers.length ? selectedIssuers.map((name) => <article key={name}><div><b>{name}</b></div></article>) : <p className="vault-section-help">还没有选择凭据</p>}</div></section></div><footer><span>确认后才会生成授权链接</span><div><button type="button" className="vault-ghost" disabled={busy} onClick={() => setModal("share")}>返回修改</button><button type="button" className="vault-primary" disabled={busy || !selected.length} onClick={() => void confirmShareCreate()}>{busy ? "生成中" : "确认生成"}</button></div></footer></section></div> : null}

    {modal === "share" ? <div className="vault-modal-mask"><form className="vault-modal share vault-share-form vault-share-compose" onSubmit={requestShareCreate}>
	...
	{modal === "shareEdit" && shareDetail ? <div className="vault-modal-mask"><form className="vault-modal share vault-share-form" onSubmit={submitShareEdit}>
		...
		<div className="vault-share-scroll">
			<section className="vault-share-section vault-share-picker-section"><div className="vault-section-title"><div><span>01</span><h3>授权凭据</h3></div><button type="button" onClick={toggleSelectAllShareItems}>{selected.length && selected.length === Math.min(ownCredentials.length, SHARE_ITEM_LIMIT) ? "取消全选" : "全部选择"}</button></div><details className="vault-credential-picker" open={sharePickerOpen} onToggle={(event) => setSharePickerOpen(event.currentTarget.open)}><summary><span><b>{selected.length ? `已选择 ${selected.length} / ${SHARE_ITEM_LIMIT} 个凭据` : "请选择要授权的凭据"}</b><small>{selected.length ? `${selectedCredentials.slice(0, 2).map((item) => item.issuer).join("、")}${selected.length > 2 ? ` 等 ${selected.length} 项` : ""}` : `展开后可多选，单次最多 ${SHARE_ITEM_LIMIT} 个`}</small></span><em>{sharePickerOpen ? "收起" : "调整"}</em></summary><div className="vault-select-list">{ownCredentials.map((item) => <label className={selected.includes(item.id) ? "is-selected" : ""} key={item.id}><input type="checkbox" checked={selected.includes(item.id)} onChange={(event) => applyShareSelection(item.id, event.target.checked)} /><span><b>{item.issuer}</b><small>{item.accountName}</small></span><Check size={15} /></label>)}</div></details></section>
			<section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>名称与剩余有效期</h3></div></div><div className="vault-form-grid vault-single-field"><label className="wide"><span>授权名称</span><input maxLength={40} value={shareForm.name} onChange={(event) => setShareForm({ ...shareForm, name: event.target.value })} placeholder="例如 给同事的临时访问，不填则为临时凭据授权" /></label></div><p className="vault-section-help">保存后从当前时间重新计算，原链接不会变化</p><div className="vault-duration"><div className="vault-duration-head"><span><Clock3 size={15} /></span><div><small>从现在起</small><b>{durationHeadline(shareForm.durationSeconds)}</b></div></div><div className="vault-duration-presets">{DURATION_PRESETS.map((item) => <button type="button" className={shareForm.durationSeconds === item.days * 86400 ? "is-active" : ""} onClick={() => applyDurationDays(item.days)} key={item.days}>{item.label}</button>)}</div><label><span>自定义</span><input type="number" min={1} max={365} inputMode="numeric" value={durationDraft ?? (isLongTerm(shareForm.durationSeconds) ? "" : String(Math.max(1, Math.round(shareForm.durationSeconds / 86400))))} onChange={(event) => onDurationDraft(event.target.value)} onBlur={commitDurationDraft} /><em>天</em></label></div></section>
			<section className="vault-share-section"><div className="vault-section-title"><div><span>03</span><h3>访问权限</h3></div></div><p className="vault-section-help">保存后立即应用到现有链接和已登录接收人</p><div className="vault-option-grid">{([['showAccount','账号'],['showPassword','密码'],['showOtp','动态验证码'],['showLoginUrl','登录地址'],['showNote','备注']] as const).map(([key, label]) => <label className={shareForm[key] ? "is-selected" : "is-off"} key={key}><input type="checkbox" checked={Boolean(shareForm[key])} onChange={(event) => setShareForm({ ...shareForm, [key]: event.target.checked })} /><Check size={14} /><span>{label}</span><small>{shareForm[key] ? "已授权" : "未授权"}</small></label>)}</div><div className="vault-form-grid vault-access-options"><label className="vault-check"><input type="checkbox" checked={shareForm.allowCopy} onChange={(event) => setShareForm({ ...shareForm, allowCopy: event.target.checked })} /><span><b>允许复制</b><small>关闭后只能在页面中查看</small></span></label>{shareDetail.shareMode === "LINK" ? <><label className="vault-check"><input type="checkbox" checked={shareForm.forbidSave} onChange={(event) => setShareForm({ ...shareForm, forbidSave: event.target.checked })} /><span><b>禁止转存</b><small>开启后所有人都不能转存此链接</small></span></label><label className="vault-check"><input type="checkbox" disabled={shareDetail.accessCount > 1} checked={shareForm.oneTime} onChange={(event) => setShareForm({ ...shareForm, oneTime: event.target.checked })} /><span><b>一次性访问</b><small>{shareDetail.accessCount > 1 ? "已访问多次，不能改为一次性" : "成功访问一次后立即失效"}</small></span></label><label><span>最大访问次数（已访问 {shareDetail.accessCount} 次）</span><input type="number" min={Math.max(1, shareDetail.accessCount)} max={1000} disabled={shareForm.oneTime} value={shareForm.oneTime ? "1" : shareForm.maxAccessCount} onChange={(event) => setShareForm({ ...shareForm, maxAccessCount: event.target.value })} placeholder="不填则不限" /></label></> : null}</div></section>
		</div>
    ...
    {modal === "saveActionConfirm" && pendingSaveAction ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) cancelSaveAction(); }}><section className="vault-modal share vault-share-form vault-delete-modal"><header><div><small>{pendingSaveAction.action === "kick" ? "REMOVE SAVE" : "BAN SAVE"}</small><h2>{pendingSaveAction.action === "kick" ? "移除转存" : "禁止转存"}</h2><p>{pendingSaveAction.action === "kick" ? "移除后对方会收到通知，你也可以随时恢复" : "禁止后对方不能自行转存，你可以随时恢复，且不会通知对方"}</p></div><button type="button" onClick={cancelSaveAction} aria-label="关闭"><X size={18} /></button></header><div className="vault-share-scroll"><section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>将要处理</h3></div></div><div className="vault-delete-summary"><span className="vault-delete-icon">{pendingSaveAction.action === "kick" ? <UserMinus size={21} /> : <Ban size={21} />}</span><div><b>{pendingSaveAction.name}</b><small>{pendingSaveAction.action === "kick" ? "从转存列表中移除" : "禁止该用户转存此链接"}</small></div></div></section><section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>影响范围</h3></div></div><p className="vault-section-help">{pendingSaveAction.action === "kick" ? "对方会从「我收到的」中暂时失去这份授权，并收到移除通知。之后仍可再次转存，你也可以随时点恢复。" : "对方不能自行转存此链接。再次尝试时会提示“该链接已被分享者禁止转存”。你可以随时在列表中恢复。"}</p></section></div><footer><span>确认后立即生效</span><div><button type="button" className="vault-ghost" disabled={busy} onClick={cancelSaveAction}>取消</button><button type="button" className="vault-danger" disabled={busy} onClick={() => void manageShareSave(pendingSaveAction.saveId, pendingSaveAction.action)}>{busy ? "处理中" : pendingSaveAction.action === "kick" ? "确认移除" : "确认禁止"}</button></div></footer></section></div> : null}
    ...
    {folderModal ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) setFolderModal(null); }}><form className="vault-modal small" onSubmit={async (event) => { event.preventDefault(); const name = folderModal.name.trim(); if (!name) return notify("请输入目录名称", true); setBusy(true); try { if (folderModal.mode === "create") { const result = await createVaultTextFolder(name); setTextFolders((current) => [...current, result.data]); setTextFolderFilter(result.data.id); notify("目录已创建"); } else if (folderModal.id) { await renameVaultTextFolder(folderModal.id, name); setTextFolders((current) => current.map((folder) => folder.id === folderModal.id ? { ...folder, name } : folder)); notify("目录已重命名"); } setFolderModal(null); } catch (error) { notify(error instanceof Error ? error.message : "目录保存失败", true); } finally { setBusy(false); } }}>
      <header><div><small>NOTES FOLDER</small><h2>{folderModal.mode === "create" ? "新建目录" : "重命名目录"}</h2><p>目录用来给笔记归类，只自己可见，随时可重命名或删除</p></div><button type="button" onClick={() => setFolderModal(null)} aria-label="关闭"><X size={18} /></button></header>
      <label><span>目录名称</span><input required autoFocus maxLength={40} value={folderModal.name} onChange={(event) => setFolderModal({ ...folderModal, name: event.target.value })} placeholder="例如 工作" /></label>
      <footer><span>最多 50 个目录</span><div><button type="button" className="vault-ghost" onClick={() => setFolderModal(null)}>取消</button><button className="vault-primary" disabled={busy || !folderModal.name.trim()}>{busy ? "保存中" : folderModal.mode === "create" ? "创建" : "保存"}</button></div></footer>
    </form></div> : null}
    {folderDeleteTarget ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setFolderDeleteTarget(null); }}><section className="vault-modal small vault-delete-modal"><header><div><small>DELETE FOLDER</small><h2>删除目录「{folderDeleteTarget.name}」？</h2><p>目录内的笔记不会被删除，会移回「未分组」。</p></div><button type="button" onClick={() => setFolderDeleteTarget(null)} aria-label="关闭"><X size={18} /></button></header><footer><span>删除后不可恢复</span><div><button type="button" className="vault-ghost" onClick={() => setFolderDeleteTarget(null)}>取消</button><button type="button" className="vault-danger" disabled={busy} onClick={async () => { setBusy(true); try { await deleteVaultTextFolder(folderDeleteTarget.id); setTextFolders((current) => current.filter((folder) => folder.id !== folderDeleteTarget.id)); if (textFolderFilter === folderDeleteTarget.id) setTextFolderFilter("all"); setFolderDeleteTarget(null); notify("目录已删除"); await load(true); } catch (error) { notify(error instanceof Error ? error.message : "目录删除失败", true); } finally { setBusy(false); } }}>{busy ? "删除中" : "确认删除"}</button></div></footer></section></div> : null}
    {modal === "created" && created ? <div className="vault-modal-mask"><section className="vault-modal small vault-created"><header><div><small>READY</small><h2>{created.name?.trim() || (created.shareType === "TEXT" ? "笔记已创建" : "授权已创建")}</h2></div><button type="button" onClick={closeModal}><X size={18} /></button></header><span className="vault-created-icon"><Check size={24} /></span><div className="vault-created-countdown"><Clock3 size={15} /><span><small>剩余有效时间</small><b>{formatRemaining(created.expireTime, now)}</b></span></div>{created.shareMode === "DIRECT" ? <div className="vault-direct-created"><User size={20} /><span><small>已授权给</small><b>{created.recipientUsername}</b><p>对方登录后可在「全部」和「我收到的」中查看，到期或撤销后自动移除。</p></span></div> : <><label><span>{created.shareType === "TEXT" ? "笔记链接" : "授权链接"}</span><div className="vault-copy-row"><input readOnly value={created.shareUrl || ""} /><button type="button" onClick={() => void copy(created.shareUrl || "")}><Copy size={15} /></button></div></label>{created.accessCode ? <label><span>访问码</span><button type="button" className="vault-access-code vault-access-code-button" onClick={() => void copy(created.accessCode || "", "访问码已复制")}>{created.accessCode}<Copy size={15} /></button></label> : null}<div className="vault-created-actions">{canUseSystemShare() ? <button type="button" className="vault-primary" onClick={() => void shareOrCopy(fullShareText)}>系统分享</button> : created.autoFillAllowed ? <button type="button" className="vault-ghost" onClick={() => void copy(autoFillUrl, "自动填充链接已复制")}>复制自动填充链接</button> : null}<button type="button" className="vault-primary" onClick={() => void copy(fullShareText, "完整分享信息已复制")}>{created.accessCode ? "复制链接和访问码" : "复制分享链接"}</button></div><p>{created.shareType === "TEXT" ? (created.accessCode ? "对方打开链接并输入访问码即可在线阅读；访问结果会记录在分享详情中。" : "对方打开链接即可在线阅读，无需访问码；访问结果会记录在分享详情中。") : "访问码以后仍可从临时授权详情查看，访问结果也会记录在详情中。"}</p></>}</section></div> : null}
    ...
    {modal === "username" ? <div className="vault-modal-mask"><section className="vault-modal share vault-share-form vault-account-modal"><header><div><small>ACCOUNT</small><h2>账号</h2><p>当前：{accountName || "未设置"}</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header><div className="vault-share-scroll"><VaultAccountSetup initialUsername={accountName} email={accountEmail} only="username" cancellable onCancel={closeModal} onFinish={(result) => { setModal(null); if (result.username) { onAccountNameChange(result.username); localStorage.setItem("otp-vault-username", result.username); } notify("账号已更新"); }} /></div></section></div> : null}

    {modal === "nickname" ? <div className="vault-modal-mask"><section className="vault-modal share vault-share-form vault-account-modal"><header><div><small>DISPLAY NAME</small><h2>用户名</h2><p>当前：{accountNick || accountName || "未设置"}</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header><div className="vault-share-scroll"><VaultAccountSetup initialUsername={accountName} initialNickname={accountNick || accountName} email={accountEmail} only="nickname" cancellable onCancel={closeModal} onFinish={(result) => { setModal(null); if (result.nickName) onAccountNickChange(result.nickName); notify("用户名已更新"); }} /></div></section></div> : null}
    ...
    <VaultScreenLock
      prefs={prefs}
      locked={screenLocked && Boolean(prefs.screenLockSet)}
      setupOpen={screenLockSetup}
      onUnlocked={unlockScreen}
      onSetupClose={() => setScreenLockSetup(false)}
      onSetupDone={lockScreen}
      onSaved={async (patch) => {
        const next = { ...prefs, ...patch };
        if (patch.screenLockPasskeyEnabled != null || patch.autoScreenLockMinutes != null) await updatePrefs(next);
        else setPrefs(next);
      }}
    />
    <VaultStepUpDialog email={accountEmail} />
    {clipboardOffer ? <div className="share-save-dock vault-clipboard-dock" role="status">
      <span><b>{clipboardOffer.name}</b><small>检测到授权，可以转存到「我收到的」</small></span>
...
function formatShareText(share: { name?: string; shareType?: string; sharePath?: string; shareUrl?: string; accessCode?: string; expireTime: string }) {
  const title = share.name?.trim() || (share.shareType === "TEXT" ? "临时笔记" : "临时凭据授权");
...
function saveStatusLabel(status: string) {
  if (status === "BANNED") return "已禁止转存";
  if (status === "KICKED") return "已移除";
  return "已转存";
}
...
function formatShareValidity(expireTime?: string, now?: number) {
  if (!expireTime) return "";
  const seconds = Math.max(0, Math.ceil((new Date(normalizeDateTime(expireTime)).getTime() - (now || Date.now())) / 1000));
  if (!Number.isFinite(seconds) || seconds <= 0) return "已到期";
  if (seconds >= 10 * 365 * 86400) return "";
