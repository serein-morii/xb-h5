...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const filteredTextShares = useMemo(() => {
    ...
    return textShares.filter((share) => `${share.name || ""} ${share.textContent || ""} ${textShareSearchLabel(share.textFormat)} ${share.accessCodeEnabled ? "访问码" : "免密码"}`.toLowerCase().includes(value));
  ...
  const [legacyText, setLegacyText] = useState("");
	...
	const [textShareForm, setTextShareForm] = useState({ name: "", textContent: "", textFormat: "TEXT" as TextShareFormat, durationSeconds: DEFAULT_SHARE_SECONDS, accessCodeEnabled: true, accessCodeMode: "AUTO", accessCode: "", allowCopy: true, oneTime: false, maxAccessCount: "" });
  ...
  const openShareDetail = async (share: VaultShare) => {
	...
	const openShareEdit = async (share: VaultShare) => {
		...
		try {
			...
			if (detail.shareType === "TEXT") {
				...
				setTextShareForm({ name: detail.name || "", textContent: detail.textContent || "", textFormat: normalizeTextShareFormat(detail.textFormat), durationSeconds: durationDays * 86400, accessCodeEnabled: detail.accessCodeEnabled, accessCodeMode: "CUSTOM", accessCode: detail.accessCode || "", allowCopy: detail.allowCopy, oneTime: detail.oneTime, maxAccessCount: detail.maxAccessCount ? String(detail.maxAccessCount) : "" });
				setModal("textShare");
				return;
			}
	...
	const openTextShare = () => {
		...
		setShareDetail(null);
		setTextShareForm({ name: "", textContent: "", textFormat: "TEXT", durationSeconds: DEFAULT_SHARE_SECONDS, accessCodeEnabled: false, accessCodeMode: "AUTO", accessCode: "", allowCopy: true, oneTime: false, maxAccessCount: "" });
		setModal("textShare");
	};
	async function submitTextShare(event: FormEvent) {
		...
		setBusy(true);
		try {
			const payload = { ...textShareForm, shareType: "TEXT", shareMode: "LINK", textContent, name: textShareForm.name.trim() || "临时笔记", durationSeconds: Math.min(LONG_TERM_SECONDS, Math.max(86400, textShareForm.durationSeconds || DEFAULT_SHARE_SECONDS)), maxAccessCount: textShareForm.maxAccessCount ? Number(textShareForm.maxAccessCount) : null };
