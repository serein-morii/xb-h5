...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  return <div className="vault-page">
    ...
    {view === "shares" ? <section className="vault-panel">
      ...
      <div className="vault-share-list">{shares.length ? shares.map((share) => <article className="vault-share-card" key={share.id} role="button" tabIndex={0} onClick={() => void openShareDetail(share)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openShareDetail(share); } }}>
        <header className="vault-share-card-head"><span className="vault-share-card-icon">{share.shareMode === "DIRECT" ? <User size={16} /> : <Link2 size={16} />}</span><div><b>{share.name?.trim() || "临时凭据授权"}</b><small>{share.shareMode === "DIRECT" ? "指定用户授权" : share.accessCodeEnabled ? "访问码保护" : "链接访问"}</small></div><span className={`vault-status is-${share.status.toLowerCase()}`}>{share.status === "ACTIVE" ? "有效" : share.status === "EXPIRED" ? "已过期" : share.status === "LIMIT_REACHED" ? "次数已用完" : "已撤销"}</span></header>
        <div className="vault-share-card-meta"><span><small>授权内容</small><b>{share.itemCount} 个凭据</b></span><span><small>{share.status === "ACTIVE" ? "剩余有效期" : "结束时间"}</small><b>{share.status === "ACTIVE" ? formatRemaining(share.expireTime, now) : new Date(normalizeDateTime(share.expireTime)).toLocaleDateString("zh-CN")}</b></span><span><small>{share.shareMode === "DIRECT" ? "接收人" : "访问次数"}</small><b>{share.shareMode === "DIRECT" ? share.recipientUsername : `${share.accessCount}${share.maxAccessCount ? ` / ${share.maxAccessCount}` : " / 不限"}`}</b></span></div>
