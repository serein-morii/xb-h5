...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  return <div className="vault-page" onPointerDown={handlePagePointerDown}>
    ...
    {modal === "textShare" ? <div className="vault-modal-mask"><form className="vault-modal share vault-share-form vault-text-share-form" onSubmit={submitTextShare}>
      ...
      <div className="vault-share-scroll">
        <section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>文本内容</h3></div><button type="button">{textShareForm.textContent.length} / 50000</button></div><div className="vault-form-grid vault-single-field"><label className="wide"><span>分享名称</span><input maxLength={40} value={textShareForm.name} onChange={(event) => setTextShareForm({ ...textShareForm, name: event.target.value })} placeholder="例如 临时说明" /></label></div><div className="vault-text-format" role="group" aria-label="文本格式"><button type="button" className={textShareForm.textFormat === "TEXT" ? "is-active" : ""} onClick={() => setTextShareForm({ ...textShareForm, textFormat: "TEXT" })}><FileText size={15} /><span><b>普通文本</b><small>按原始换行显示</small></span></button><button type="button" className={textShareForm.textFormat === "MARKDOWN" ? "is-active" : ""} onClick={() => setTextShareForm({ ...textShareForm, textFormat: "MARKDOWN" })}><MessageSquareText size={15} /><span><b>Markdown</b><small>支持标题、列表与代码</small></span></button></div><label className="vault-text-editor"><span>要分享的内容</span><textarea required maxLength={50000} value={textShareForm.textContent} onChange={(event) => setTextShareForm({ ...textShareForm, textContent: event.target.value })} placeholder={textShareForm.textFormat === "MARKDOWN" ? "# 标题\n\n输入 Markdown 内容…" : "输入要临时分享的文本…"} /></label></section>
      ...
      <footer><span>{textShareForm.textContent.trim() ? `${textShareForm.textFormat === "MARKDOWN" ? "Markdown" : "普通文本"} · ${textShareForm.allowCopy ? "允许复制" : "仅阅读"}` : "请输入要分享的文本"}</span><div><button type="button" className="vault-ghost" onClick={closeModal}>取消</button><button className="vault-primary" disabled={busy || !textShareForm.textContent.trim()}>{busy ? "保存中" : textShareEditingId ? "保存修改" : "生成分享"}</button></div></footer>
    </form></div> : null}
