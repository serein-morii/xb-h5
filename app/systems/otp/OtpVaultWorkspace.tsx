    ...
    {view === "text" ? <section className="vault-panel vault-view-enter" key="text">
      ...
      {textShares.length ? filteredTextShares.length ? <div className="vault-share-list vault-text-share-list">{filteredTextShares.map((share) => <article className="is-text" key={share.id} role="button" tabIndex={0} onClick={() => void openShareDetail(share)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openShareDetail(share); } }}>
        ...
        <div className="vault-text-card-main">
          ...
          <small><em className="vault-text-format-pill">{share.textFormat === "MARKDOWN" ? "Markdown" : "普通文本"}</em><span className={`vault-status is-${share.status.toLowerCase()}`}>{share.status === "ACTIVE" ? "有效" : share.status === "EXPIRED" ? "已过期" : share.status === "LIMIT_REACHED" ? "次数已用完" : "已撤销"}</span>· {share.accessCodeEnabled ? "访问码保护" : "免密码"} · {share.status === "ACTIVE" ? `剩余 ${formatRemaining(share.expireTime, now)}` : new Date(normalizeDateTime(share.expireTime)).toLocaleString("zh-CN", { hour12: false })} · 已访问 {share.accessCount}{share.maxAccessCount ? `/${share.maxAccessCount}` : ""}</small>
