...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const renderChannelForm = (afterCreate?: () => void) => <form className="vault-channel-create" onSubmit={async (event) => { await submitInboundChannel(event); afterCreate?.(); }}>
    ...
    <div className="vault-channel-form-actions">{inboundChannels.length ? <button type="button" className="vault-ghost" onClick={() => setChannelFormOpen(false)}>取消</button> : null}<button className="vault-primary" disabled={busy}><Plus size={14} />创建接收通道</button></div>
  </form>;
  ...
  const renderCredential = (item: VaultCredential) => {
    ...
    return <article className={`vault-card${prefs.compact ? " is-compact" : ""}${item.shared ? " is-shared" : ""}`} key={item.id} role="button" tabIndex={0} onClick={() => void openDetail(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openDetail(item); } }}>
      ...
      <div className="vault-progress"><i style={{ width: `${progress}%` }} /></div><div className="vault-card-foot"><span>{item.shared ? `来自 ${sharerDisplay(item)}` : item.currentOtp || item.otpConfigured ? `${item.otpType || "TOTP"} · ${item.algorithm} · ${item.digits} 位` : item.dynamicCodeEnabled ? `${dynamicSources}接收` : item.passwordConfigured ? "登录密码" : "安全笔记"}</span>{nextOtpLabel && item.otpType !== "HOTP" && !item.requiresStepUp ? <span className="vault-card-next"><small>下一组</small><b>{concealed ? "••• •••" : nextOtpLabel}</b>{copyAllowed ? <button type="button" className="vault-next-copy" onClick={(event) => { event.stopPropagation(); void copy(item.nextOtp || "", "下一组验证码已复制"); }} aria-label="复制下一组验证码"><Copy size={11} /></button> : null}</span> : item.dynamicCode && !item.currentOtp ? <span className="vault-card-next"><small>{item.dynamicCodeSource === "SMS" ? <MessageSquareText size={11} /> : item.dynamicCodeSource === "EMAIL" ? <Mail size={11} /> : <Webhook size={11} />}</small><b>{sourceLabel(item.dynamicCodeSource)}</b></span> : null}<span>{item.requiresStepUp ? "已锁定" : item.otpType === "HOTP" && (item.currentOtp || item.otpConfigured) ? `计数 ${item.hotpCounter || 0}` : item.currentOtp ? `${left}s` : item.dynamicCodeEnabled ? (item.dynamicCode ? dynamicCodeAge(item.dynamicCodeReceivedTime, now) : "等待验证码") : shareValidity || (item.shared ? "" : item.passwordConfigured ? "密码" : "笔记")}</span></div>
