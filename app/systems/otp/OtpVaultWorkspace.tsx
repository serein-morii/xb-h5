...
type Modal = "credential" | "scanner" | "detail" | "import" | "share" | "shareDetail" | "deleteConfirm" | "revokeConfirm" | "created" | "account" | null;
...
const defaultPrefs: VaultPrefs = { masked: false, compact: false, grouped: true, showShared: true, autoRefresh: true, autoLockMinutes: 5, securityAlerts: true };
...
export default function OtpVaultWorkspace({ onLogout, accountName, accountEmail, onAccountNameChange }: { onLogout: () => void; accountName: string; accountEmail: string; onAccountNameChange: (name: string) => void }) {
  ...
  async function submitCredential(event: FormEvent) {
    event.preventDefault(); setBusy(true);
	    try { await saveVaultCredential(editingId, await protectCredential(form)); notify(editingId ? "凭据已更新" : "凭据已添加"); setModal(null); await load(true); }
  ...
  return <div className="vault-page">
    ...
    {view === "settings" ? <section className="vault-settings"><header className="vault-panel-head"><div><h2>设置</h2><p>账号资料和显示偏好会跟随当前账号</p></div></header><div className="vault-settings-group"><header><div><b>账号与登录</b><small>用户名、邮箱和登录密码</small></div><span>{accountName || "当前账号"}</span></header><button type="button" className="vault-account-entry" onClick={() => setModal("account")}>修改用户名 / 设置密码<small>管理当前 OTP Vault 账号的登录方式</small></button></div><div className="vault-settings-group"><header><div><b>界面显示</b><small>验证码与账号列表</small></div><span>5 项</span></header>{([['masked', EyeOff, '隐藏账号', '在列表中遮住账号主体', 'violet'], ['compact', LayoutGrid, '紧凑卡片', '缩小留白，一屏看到更多内容', 'blue'], ['grouped', Layers3, '按系统分组', '将同一系统的凭据排列在一起', 'green'], ['showShared', User, '显示共享', '在全部列表中展示别人分享给我的凭据', 'blue'], ['autoRefresh', Clock3, '自动刷新', '定时同步授权状态和新增共享', 'green']] as const).map(([key, Icon, title, detail, tone]) => <label className="vault-setting-row" key={key}><span className={`vault-setting-icon is-${tone}`}><Icon size={17} /></span><span className="vault-setting-copy"><b>{title}</b><small>{detail}</small></span><input type="checkbox" checked={prefs[key]} onChange={(event) => updatePrefs({ ...prefs, [key]: event.target.checked })} /><i /></label>)}</div></section> : null}
      ...
      {detailLoading || !liveDetail ? <div className="vault-share-scroll"><div className="vault-detail-loading"><LoaderCircle className="spin" size={22} />正在安全读取…</div></div> : <>
        <div className="vault-share-scroll">
          ...
          <section className="vault-share-section"><div className="vault-section-title"><div><span>02</span><h3>账号与验证码</h3></div></div><div className="vault-detail-values">
            ...
            {liveDetail.currentOtp ? <section className="is-otp"><span>{liveDetail.otpType === "HOTP" ? `HOTP 验证码 · 计数 ${liveDetail.hotpCounter || 0}` : "动态验证码"}</span><div><b>{liveDetail.currentOtp.replace(/(.{3})/, "$1 ")}</b>{!liveDetail.shared || liveDetail.allowCopy ? <button type="button" onClick={() => void copy(liveDetail.currentOtp || "", "验证码已复制")} aria-label="复制验证码"><Copy size={15} /></button> : null}{!liveDetail.shared && liveDetail.otpType === "HOTP" ? <button type="button" disabled={busy} onClick={() => void advanceHotp(liveDetail.id)}>下一个</button> : null}</div></section> : null}
          </div></section>
    ...
    {modal === "credential" ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><form className="vault-modal share vault-share-form" onSubmit={submitCredential}><header><div><small>CREDENTIAL</small><h2>{editingId ? "编辑凭据" : "添加凭据"}</h2><p>{editingId ? "已保存的敏感值不会回填，留空即可保留" : "扫码或手动录入一条新的验证器数据"}</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header><div className="vault-share-scroll">{!editingId ? <section className="vault-share-section"><div className="vault-section-title"><div><span>01</span><h3>快捷录入</h3></div></div><button type="button" className="vault-scan-entry" onClick={() => void openScanner()}><ScanLine size={18} /><span><b>扫描验证器二维码</b><small>自动识别 Google Authenticator 和标准 OTP 数据</small></span></button></section> : null}<section className="vault-share-section"><div className="vault-section-title"><div><span>{editingId ? "01" : "02"}</span><h3>凭据信息</h3></div></div>{editingCredential ? <div className="vault-maintained-status"><span><ShieldCheck size={15} /><b>敏感信息状态</b></span><div><em className={editingCredential.otpConfigured ? "is-ready" : ""}>{editingCredential.otpConfigured ? "OTP 密钥已保存" : "未保存 OTP 密钥"}</em><em className={editingCredential.passwordConfigured ? "is-ready" : ""}>{editingCredential.passwordConfigured ? "密码已保存" : "未保存密码"}</em></div><small>出于安全考虑不会显示原值；对应输入框留空时保持不变。</small></div> : null}<div className="vault-form-grid">
      ...
      <label className="wide"><span>OTP Secret</span><input value={form.otpSecret} onChange={(e) => setForm({ ...form, otpSecret: e.target.value.toUpperCase().replace(/[^A-Z2-7=\s-]/g, "") })} autoComplete="off" placeholder={editingCredential?.otpConfigured ? "已保存；留空保持不变，填写则替换" : "Base32 Secret"} /></label>
      ...
      <label><span>密码 {editingId ? "（可选替换）" : "（可选）"}</span><input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} autoComplete="new-password" placeholder={editingCredential?.passwordConfigured ? "已保存；留空保持不变" : "可保存登录密码"} /></label>
    ...
    {modal === "account" ? <div className="vault-modal-mask"><section className="vault-modal share vault-share-form">
      <header><div><small>ACCOUNT SECURITY</small><h2>账号安全</h2><p>修改用户名或设置登录密码</p></div><button type="button" onClick={closeModal} aria-label="关闭"><X size={18} /></button></header>
      <div className="vault-share-scroll">
        <VaultAccountSetup
          ...
          onFinish={(result) => {
            setModal(null);
            if (result.username) {
              onAccountNameChange(result.username);
              localStorage.setItem("otp-vault-username", result.username);
            }
            notify(result.passwordSet ? "密码已设置，下次可用账号密码登录" : result.passwordSkipped ? "已跳过密码设置，仍可稍后再补设" : "用户名已更新");
          }} />
      </div>
    </section></div> : null}
