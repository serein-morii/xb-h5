...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  const renderChannelForm = (afterCreate?: (channel: VaultInboundChannel) => void) => <form className="vault-channel-create" onSubmit={async (event) => { const created = await submitInboundChannel(event); if (created) afterCreate?.(created); }}>
    ...
    <div className="vault-channel-form-actions">{inboundChannels.length ? <button type="button" className="vault-ghost" onClick={() => setChannelFormOpen(false)}>取消</button> : null}<button className="vault-primary" disabled={busy}><Plus size={14} />创建通道</button></div>
  </form>;
        ...
        <div className="vault-share-scroll">
          ...
          <section className="vault-share-section vault-detail-section"><div className="vault-section-title"><div><span>01</span><h3>登录信息</h3></div><small>{liveDetail.shared ? `${liveDetail.sharedBy} 分享 · ${liveDetail.shareExpireTime ? formatRemaining(liveDetail.shareExpireTime, now) : "临时有效"}` : "敏感信息仅在需要时显示"}</small></div><div className="vault-detail-values">
            ...
            {liveDetail.passwordConfigured ? <section><span>密码</span><div><b className={passwordVisible ? "" : "is-secret"}>{passwordVisible ? liveDetail.password : "••••••••••••"}</b><button type="button" onClick={() => setPasswordVisible(!passwordVisible)} aria-label={passwordVisible ? "隐藏密码" : "显示密码"}>{passwordVisible ? <EyeOff size={15} /> : <Eye size={15} />}</button>{liveDetail.password && canCopyCredential(liveDetail) ? <button type="button" onClick={() => void copy(liveDetail.password || "", "密码已复制")} aria-label="复制密码"><Copy size={15} /></button> : null}</div></section> : null}
				{liveDetail.dynamicCode ? <section className="is-inbound"><span className="vault-otp-label">{sourceLabel(liveDetail.dynamicCodeSource)}验证码<em>{formatCodeTime(detailInboundTiming.left)}</em></span><div><b>{liveDetail.dynamicCode.replace(/(.{3})(?=.)/, "$1 ")}</b>{canCopyCredential(liveDetail) ? <button type="button" onClick={() => void copyDynamicCode(liveDetail)} aria-label={`复制${sourceLabel(liveDetail.dynamicCodeSource)}验证码`}><Copy size={15} /></button> : null}</div><span className="vault-card-sources" aria-label={`已配置${(liveDetail.dynamicSources || []).map(sourceLabel).join("、")}`}>{(liveDetail.dynamicSources || []).map((source) => <span className={`vault-card-source${source === liveDetail.dynamicCodeSource ? " is-active" : ""}`} key={source} title={sourceLabel(source)} aria-label={sourceLabel(source)}>{source === "EMAIL" ? <Mail size={13} /> : source === "WEBHOOK" ? <Webhook size={13} /> : <MessageSquareText size={13} />}</span>)}</span><small>{liveDetail.dynamicCodeSender || "通过已配置的接收通道送达"}{liveDetail.dynamicCodeUsed ? " · 已使用" : " · 新验证码"}</small><div className="vault-progress"><i style={{ width: `${detailInboundTiming.progress}%` }} /></div></section> : liveDetail.dynamicCodeEnabled ? <section className="is-inbound is-waiting"><span className="vault-otp-label">接收验证码</span><div><b>等待验证码</b><button type="button" disabled aria-label="暂无验证码可复制"><Copy size={15} /></button></div><span className="vault-card-sources" aria-label={liveDetail.dynamicSources?.length ? `已配置${liveDetail.dynamicSources.map(sourceLabel).join("、")}` : "未配置来源"}>{liveDetail.dynamicSources?.length ? liveDetail.dynamicSources.map((source) => <span className="vault-card-source" key={source} title={sourceLabel(source)} aria-label={sourceLabel(source)}>{source === "EMAIL" ? <Mail size={13} /> : source === "WEBHOOK" ? <Webhook size={13} /> : <MessageSquareText size={13} />}</span>) : <span className="vault-source-unconfigured">未配置来源</span>}</span></section> : null}
          ...
          {liveDetail.dynamicCodeEnabled || detailCodeHistory.length || detailCodeHistoryLoading ? <InboundCodeHistory rows={detailCodeHistory} total={detailCodeHistoryTotal} loading={detailCodeHistoryLoading} allowCopy={canCopyCredential(liveDetail)} now={now} onCopy={(value) => void copy(value, "历史验证码已复制")} onLoadMore={() => void loadDetailCodeHistory(liveDetail.id, detailCodeHistoryPage + 1)} /> : null}
          {liveDetail.loginUrl || liveDetail.note || liveDetail.tags ? <section className="vault-share-section vault-detail-section"><div className="vault-section-title"><div><span>{liveDetail.dynamicCodeEnabled ? "03" : "02"}</span><h3>补充信息</h3></div></div>{liveDetail.loginUrl ? <a className="vault-detail-link" href={liveDetail.loginUrl} target="_blank" rel="noreferrer"><ExternalLink size={14} /><span>{liveDetail.loginUrl}</span></a> : null}{liveDetail.note ? <p className="vault-detail-note">{liveDetail.note}</p> : null}{liveDetail.tags ? <div className="vault-detail-tags">{liveDetail.tags.split(/[\s,，]+/).filter(Boolean).map((tag) => <b key={tag}>{tag}</b>)}</div> : null}</section> : null}
        </div>
