...
export default function OtpVaultWorkspace({ onLogout, accountName, accountNick, accountEmail, onAccountNameChange, onAccountNickChange, onAccountEmailChange }: { onLogout: () => void; accountName: string; accountNick: string; accountEmail: string; onAccountNameChange: (name: string) => void; onAccountNickChange: (name: string) => void; onAccountEmailChange: (email: string) => void }) {
  ...
  return <div className="vault-page" onPointerDown={handlePagePointerDown}>
    ...
    <nav className="vault-tabs" aria-label="密钥管理导航">
      {([['all', KeyRound, '凭据'], ['shares', Link2, '授权'], ['settings', Settings2, '我的']] as const).map(([key, Icon, label]) => <button type="button" className={view === key ? "is-active" : ""} onClick={() => changeView(key)} key={key}><Icon size={15} />{label}</button>)}
    </nav>
    ...
    {view === "shares" ? <section className="vault-panel vault-view-enter" key="shares">
      <header className="vault-panel-head"><div><span className="vault-panel-title-row"><h2>授权</h2><div className="vault-fav-switch" role="tablist" aria-label="授权筛选"><button type="button" className={shareTab === "sent" ? "is-active" : ""} onClick={() => setShareTab("sent")}>我发出的</button><button type="button" className={shareTab === "received" ? "is-active" : ""} onClick={() => setShareTab("received")}>我收到的</button></div></span><p>{shareTab === "received" ? `指定给你的授权，以及你转存进来的 · ${receivedShares.length} 项` : `临时分享凭据或文本，均可随时撤销 · ${shares.length} 项`}</p></div>{shareTab === "sent" ? <div className="vault-panel-tools"><button type="button" className="vault-primary vault-text-share-trigger" onClick={openTextShare}><FileText size={15} />分享文本</button></div> : null}</header>
      ...
      </article>) : <div className="vault-empty compact"><Share2 size={20} /><b>还没有创建临时分享</b><p>可以分享一段文本，或选择凭据生成限时授权。</p><div className="vault-empty-actions"><button type="button" className="vault-primary" onClick={openTextShare}><FileText size={15} />分享文本</button>{ownCredentials.length ? <button type="button" className="vault-ghost" onClick={() => openShare()}>分享凭据</button> : null}</div></div>}</div>}
    </section> : null}
    ...
    <nav className="vault-mobile-nav" aria-label="密钥管理导航">
      {([['all', KeyRound, '凭据'], ['shares', Link2, '授权'], ['settings', Settings2, '我的']] as const).map(([key, Icon, label]) => <button type="button" className={view === key ? "is-active" : ""} onClick={() => changeView(key)} key={key}><Icon size={18} /><span>{label}</span></button>)}
    </nav>
