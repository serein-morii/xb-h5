...
type Props = {
  ...
  onEditScreenLock: () => void;
};

/** 安全页只展示可核验状态；浏览器密钥始终只保留在当前页面内存中。 */
export default function VaultSecurityCenter({ prefs, updatePrefs, zeroKnowledgeKey, onZeroKnowledgeKey, onEditScreenLock }: Props) {
  ...
  const load = async () => {
    ...
    setSessions(sessionResult.data || []); setTrash(trashResult.data || []); setActivities(activityResult.rows || []);
    setActivityPage(1); setActivityTotal(activityResult.total || 0);
  ...
  const loadMoreActivities = () => run("activity-more", async () => {
    const nextPage = activityPage + 1;
    const result = await listVaultActivities(nextPage);
    setActivities((current) => [...current, ...(result.rows || [])]);
    setActivityPage(nextPage); setActivityTotal(result.total || 0);
  });
  ...
  const exportQr = () => run("qr", async () => {
    const result = await exportVaultMigration(); setQrs(await buildMigrationQrs(await materializeItems(result.data.items)));
  ...
  return <div className="vault-security-center">
    ...
    {section === "protect" ? <div className="vault-security-layout is-protect">
      <section className="vault-settings-group vault-security-facts"><header><div><b>实时保护状态</b><small>数据来自当前密钥、会话与安全记录</small></div><span>{status?.encryption || "读取中"}</span></header><div className="vault-security-grid">
        ...
        <article><LockKeyhole size={17} /><div><b>{!prefs.stepUpEnabled ? "敏感操作免验证" : status?.unlocked ? "敏感操作已验证" : "敏感操作已保护"}</b><small>{!prefs.stepUpEnabled ? "可在下方开启二次身份验证" : status?.unlocked ? `${status.stepUpExpiresIn} 秒后自动结束` : "查看、编辑、分享和导出时再次验证"}</small></div></article>
      ...
      <section className="vault-settings-group"><header><div><b>锁屏</b><small>手动锁定保险库，也可设置无操作后自动锁定</small></div><span>{prefs.screenLockSet ? screenLockTypeLabel(prefs.screenLockType) : "未设置"}</span></header>
        <p className="vault-transparency-note">顶栏皮肤切换右侧的小锁可随时锁定。未设置密码时会先进入设置。以后也在「我的 → 安全」修改。</p>
        <div className="vault-security-actions"><button type="button" onClick={onEditScreenLock}><LockKeyhole size={14} />{prefs.screenLockSet ? "修改锁屏密码" : "设置锁屏密码"}</button></div>
        <label className="vault-security-field"><span>无操作自动锁屏</span><select disabled={!prefs.screenLockSet} value={prefs.autoScreenLockMinutes || 0} onChange={(event) => void updatePrefs({ ...prefs, autoScreenLockMinutes: Number(event.target.value) })}><option value={0}>关闭（仍可手动锁屏）</option><option value={1}>1 分钟</option><option value={5}>5 分钟</option><option value={15}>15 分钟</option><option value={30}>30 分钟</option></select></label>
    ...
    {section === "activity" ? <div className="vault-security-layout is-activity">
      ...
      <section className="vault-settings-group"><header><div><b>最近安全活动</b><small>关键操作、设备和来源 IP</small></div><span>{activityTotal} 条</span></header><div className="vault-activity-list">{activities.map((item) => <article key={item.id}><span><KeyRound size={14} /></span><div><b>{activityLabel(item.action)}</b><small>{item.detail || item.targetType || "保险库"} · {item.ipAddress || "未知 IP"} · {ipDetailLabel(item.ipInfo)}</small></div><time>{formatTime(item.createTime)}</time></article>)}</div>{activities.length < activityTotal ? <button type="button" className="vault-security-more" disabled={busy !== ""} onClick={() => void loadMoreActivities()}>{busy === "activity-more" ? <><LoaderCircle className="spin" size={13} />加载中</> : `加载更多（剩余 ${activityTotal - activities.length} 条）`}</button> : null}</section>
    </div> : null}
...
function screenLockTypeLabel(type?: string) {
  if (type === "pin4") return "4 位数字";
  if (type === "complex") return "复杂密码";
  if (type === "pin6") return "6 位数字";
  return "已设置";
}
