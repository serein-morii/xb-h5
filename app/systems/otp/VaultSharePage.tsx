...
function SharedItemCard({ item, index, groupName, compact, allowCopy, copied, now, onCopy, onOpenDetail, defaultOpen = false }: { item: SharedItem; index: number; groupName: string; compact: boolean; allowCopy: boolean; copied: string; now: number; onCopy: (value: string, key: string, message?: string) => void; onOpenDetail: () => void; defaultOpen?: boolean }) {
  ...
  return <article className={`vault-card share-credential-card${item.otp || item.dynamicCode || hasInbound ? " has-otp" : " is-login"}${compact ? " is-compact" : ""}`}>
    ...
    {hasInbound && !item.otp ? <div className="vault-card-foot is-inbound-foot"><SourceBadges sources={configuredSources} active={item.dynamicCode ? item.dynamicCodeSource : undefined} />{open && item.dynamicCode ? <span>{formatCodeTime(inboundTiming.left)}</span> : null}</div> : <div className="vault-card-foot"><span>{item.otp ? "动态验证码" : "账号密码"}</span>{item.nextOtp ? <span className="vault-card-next"><small>下一组</small><b>{item.nextOtp.replace(/(.{3})/, "$1 ")}</b>{allowCopy ? <button type="button" className="vault-next-copy" onClick={() => void onCopy(item.nextOtp || "", nextOtpKey, "下一组验证码已复制")} aria-label="复制下一组验证码">{copied === nextOtpKey ? <Check size={11} /> : <Copy size={11} />}</button> : null}</span> : null}<span>{item.otp ? `${left}秒` : ""}</span></div>}
