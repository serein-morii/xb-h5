...
export default function VaultSharePage({ token }: { token: string }) {
  ...
  const [usageTipOpen, setUsageTipOpen] = useState(() => {
    try { return !localStorage.getItem(`otp-vault-share-tip:${token}`); } catch { return true; }
  });
  ...
  const saveToInbox = useCallback(async () => {
    ...
    try {
      ...
      setToast(result.data.alreadySaved ? "已经转存过了，无需再次转存" : "已转存");
    } catch (saveError) {
  ...
  const hasInboundShare = items.some((item) => item.dynamicCode || (item.dynamicSources || []).length);
  const dismissUsageTip = () => {
    setUsageTipOpen(false);
    try { localStorage.setItem(`otp-vault-share-tip:${token}`, "1"); } catch { /* 无痕模式下仅本次关闭 */ }
  };
  return <main className={`share-page ${gateVisible ? "is-gate" : "is-open"}`}>
    <header className="share-brand"><span className="share-vault-mark">OTP</span><div><b>OTP Vault</b><small>{status?.name || "临时凭据授权"}</small></div>{hasInboundShare ? <button type="button" className="vault-ghost vault-theme-action share-help-action" onClick={() => setUsageTipOpen(true)} aria-label="验证码使用说明"><CircleHelp size={18} /><span>说明</span></button> : null}<button type="button" className="vault-ghost vault-theme-action" onClick={toggleHeaderTheme} aria-label={`切换显示模式，当前${themeMode === "system" ? "跟随系统" : themeMode === "dark" ? "暗黑" : "亮色"}`}>{themeMode === "system" ? <SunMoon size={18} /> : themeMode === "dark" ? <Moon size={18} /> : <Sun size={18} />}<span>{themeMode === "system" ? "系统" : themeMode === "dark" ? "暗黑" : "亮色"}</span></button></header>
    {gateVisible ? <section className="share-access-card">
      ...
      {usageTipOpen && hasInboundShare ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) dismissUsageTip(); }}><section className="vault-modal small share-usage-modal" role="dialog" aria-label="验证码使用说明">
        ...
        <div className="vault-share-scroll"><ul className="share-usage-list">
          ...
          <li><ShieldCheck size={14} />本页无需登录，请在授权有效期内使用。</li>
        </ul></div>
        <footer><span>点顶栏的 ? 可再次查看</span><div><button type="button" className="vault-primary" onClick={dismissUsageTip}>知道了</button></div></footer>
      </section></div> : null}
      <div className="share-toolbar vault-panel-tools"><div className="vault-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索" aria-label="搜索分享凭据" />{query ? <button type="button" onClick={() => setQuery("")} aria-label="清空搜索"><X size={14} /></button> : null}</div><div className="vault-view-toggles"><label className={`vault-view-toggle${displayPrefs.grouped ? " is-active" : ""}`}><Layers3 size={14} /><span>分组</span><input type="checkbox" checked={displayPrefs.grouped} onChange={(event) => setDisplayPrefs({ ...displayPrefs, grouped: event.target.checked })} /><i /></label><label className={`vault-view-toggle${displayPrefs.compact ? " is-active" : ""}`}><LayoutGrid size={14} /><span>紧凑</span><input type="checkbox" checked={displayPrefs.compact} onChange={(event) => setDisplayPrefs({ ...displayPrefs, compact: event.target.checked })} /><i /></label></div></div>
      <div className="share-groups">{groups.map(([name, groupItems]) => <section className="share-group" key={name || "all"}>{name ? <header><b>{name}</b><span>{groupItems.length}</span></header> : null}<div className={`share-item-list${displayPrefs.compact ? " is-compact" : ""}`}>{groupItems.map((item, index) => <SharedItemCard key={`${item.issuer}-${item.accountName || ""}-${index}`} item={item} index={index} groupName={name} compact={displayPrefs.compact} allowCopy={allowCopy} copied={copied} now={syncedNow} defaultOpen={items.length === 1} onCopy={copy} onOpenDetail={() => { setDetailPasswordVisible(false); setDetailItem(item); }} />)}</div></section>)}{!filteredItems.length ? <p className="share-empty">没有匹配的凭据</p> : null}</div>
      ...
      {liveShareDetail ? <div className="vault-modal-mask" onMouseDown={(event) => { if (event.target === event.currentTarget) setDetailItem(null); }}><section className="vault-modal share vault-share-form vault-detail">
        ...
        <div className="vault-share-scroll">
          ...
          <section className="vault-share-section vault-detail-section"><div className="vault-section-title"><div><span>01</span><h3>登录信息</h3></div><small>授权内容仅在有效期内可查看</small></div><div className="vault-detail-values">
            ...
            {liveShareDetail.loginUrl ? <section className="is-login-url"><span>访问地址</span><div><a className="vault-detail-login-url" href={liveShareDetail.loginUrl} target="_blank" rel="noreferrer">{liveShareDetail.loginUrl}</a><a className="vault-detail-open-url" href={liveShareDetail.loginUrl} target="_blank" rel="noreferrer" aria-label="打开访问地址"><ExternalLink size={15} /></a>{allowCopy ? <button type="button" onClick={() => void copy(liveShareDetail.loginUrl || "", "detail-login-url", "访问地址已复制")} aria-label="复制访问地址"><Copy size={15} /></button> : null}</div></section> : null}
            {liveShareDetail.otp ? <section className="is-otp"><span className="vault-otp-label">动态验证码{liveShareDetail.otpValidUntil ? <em>{shareDetailLeft}秒</em> : null}</span><div><b>{liveShareDetail.otp.replace(/(.{3})/, "$1 ")}</b>{allowCopy ? <button type="button" onClick={() => void copy(liveShareDetail.otp || "", "detail-otp", "验证码已复制")} aria-label="复制验证码"><Copy size={15} /></button> : null}</div>{liveShareDetail.nextOtp ? <div className="vault-detail-next"><span>下一组</span><b>{liveShareDetail.nextOtp.replace(/(.{3})/, "$1 ")}</b><small>{shareDetailLeft}秒后启用</small>{allowCopy ? <button type="button" className="vault-next-copy" onClick={() => void copy(liveShareDetail.nextOtp || "", "detail-next-otp", "下一组验证码已复制")} aria-label="复制下一组验证码"><Copy size={12} /></button> : null}</div> : null}{liveShareDetail.otpValidUntil ? <div className="vault-progress"><i style={{ width: `${shareDetailProgress}%` }} /></div> : null}</section> : null}
            {liveShareDetail.dynamicCode ? <section className="is-otp is-inbound"><span className="vault-otp-label">{sourceLabel(liveShareDetail.dynamicCodeSource)}验证码{liveShareDetail.dynamicCodeUsed ? " · 已使用" : ""}<em>{formatCodeTime(shareDetailInboundTiming.left)}</em></span><div><b>{liveShareDetail.dynamicCode.replace(/(.{3})(?=.)/, "$1 ")}</b>{allowCopy ? <button type="button" onClick={() => void copy(liveShareDetail.dynamicCode || "", "detail-inbound", "接收验证码已复制")} aria-label="复制接收验证码"><Copy size={15} /></button> : null}</div><div className="vault-detail-next"><span>来源</span><SourceBadges sources={liveShareDetail.dynamicSources} active={liveShareDetail.dynamicCodeSource} /><small>{liveShareDetail.dynamicCodeSender || "接收通道"}</small></div><div className="vault-progress"><i style={{ width: `${shareDetailInboundTiming.progress}%` }} /></div></section> : liveShareDetail.dynamicSources?.length ? <section className="is-otp is-inbound is-waiting"><span className="vault-otp-label">接收验证码</span><div><b>等待验证码</b><button type="button" disabled aria-label="暂无验证码可复制"><Copy size={15} /></button></div><div className="vault-detail-next"><span>来源</span><SourceBadges sources={liveShareDetail.dynamicSources} /></div></section> : null}
          </div></section>
          {liveShareDetail.note ? <section className="vault-share-section vault-detail-section"><div className="vault-section-title"><div><span>02</span><h3>补充信息</h3></div></div><p className="vault-detail-note">{liveShareDetail.note}</p></section> : null}
          {liveShareDetail.dynamicSources?.length || detailCodeHistory.length || detailCodeHistoryLoading ? <InboundCodeHistory rows={detailCodeHistory} total={detailCodeHistoryTotal} loading={detailCodeHistoryLoading} allowCopy={allowCopy} now={syncedNow} step={liveShareDetail.note ? "03" : "02"} onCopy={(value) => void copy(value, "detail-history", "历史验证码已复制")} onLoadMore={() => void loadMoreDetailCodes()} /> : null}
        </div>
    ...
    {!gateVisible && !ownShare ? <div className={`share-save-dock${saveCollapsed ? " is-collapsed" : ""}`}>
      {saveCollapsed ? <button type="button" className="share-save-fab" aria-label={saved ? "已转存" : "转存"} onClick={() => setSaveCollapsed(false)}><FolderDown size={18} /></button> : <>
        <button type="button" className="share-save-collapse" aria-label="收起" onClick={() => setSaveCollapsed(true)}><ChevronDown size={15} /></button>
        <span><b>{status?.name?.trim() || (saved ? "已转存" : "转存")}</b><small>{saved ? "已保存到「我收到的」" : "保存到「我收到的」，内容与时效仍由分享者控制"}</small></span>
        {saved ? <a href={APP_ROUTES.otp}>打开保险库</a> : <button type="button" disabled={saving} onClick={() => void saveToInbox()}>{saving ? <LoaderCircle className="spin" size={15} /> : <FolderDown size={15} />}{getOtpToken() ? (saving ? "转存中" : "转存") : "登录后转存"}</button>}
      </>}
    </div> : null}
...
function SharedItemCard({ item, index, groupName, compact, allowCopy, copied, now, onCopy, onOpenDetail, defaultOpen = false }: { item: SharedItem; index: number; groupName: string; compact: boolean; allowCopy: boolean; copied: string; now: number; onCopy: (value: string, key: string, message?: string) => void; onOpenDetail: () => void; defaultOpen?: boolean }) {
  ...
  const inboundKey = `${groupName}-${index}-inbound`;
  ...
  const configuredSources = Array.from(new Set<DynamicCodeSource>([...(item.dynamicSources || []), ...(item.dynamicCodeSource ? [item.dynamicCodeSource] : [])]));
  const hasInbound = configuredSources.length > 0;
