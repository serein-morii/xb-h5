...
import { finishVaultStepUpPasskey, getVaultStepUpPasskeyOptions, listVaultPasskeys, setOtpStepUpToken, verifyVaultSecurity } from "./vaultApi";
...
type VerifyMode = "passkey" | "email" | "password";
...
export default function VaultStepUpDialog({ email }: { email: string }) {
  ...
  useEffect(() => {
    const open = (event: Event) => {
      setWaiter((event as CustomEvent<Waiter>).detail);
      setValue(""); setMessage("");
      // Passkey 优先：已注册 Passkey 时默认用它解锁，密码和邮箱码作为备选
      listVaultPasskeys().then((result) => {
        const registered = (result.data || []).length > 0;
        setHasPasskey(registered);
        setMode(registered ? "passkey" : "email");
      }).catch(() => { setHasPasskey(false); setMode("email"); });
    };
  ...
  const send = async () => {
    if (!email) return setMessage("当前账号没有绑定邮箱，请改用密码或 Passkey 验证");
    setSending(true); setMessage("");
    try {
      const result = await sendEmailCode(email, "otp-security") as { resendAfter?: number };
      setCountdown(Number(result.resendAfter || 60));
      setMessage("验证码已发送到绑定邮箱");
    }
    catch (error) { setMessage(error instanceof Error ? error.message : "验证码发送失败"); }
    finally { setSending(false); }
  };
  const submit = async (event: FormEvent) => {
    event.preventDefault(); if (mode !== "passkey" && !value.trim()) return;
    setBusy(true); setMessage("");
    try {
      let token = "";
      if (mode === "passkey") {
        const options = await getVaultStepUpPasskeyOptions();
        const result = await finishVaultStepUpPasskey(options.data.requestId, await getPasskey(options.data.publicKey));
        ...
        token = (await verifyVaultSecurity(body)).data.token;
      }
      setOtpStepUpToken(token);
  ...
  return <div className="vault-modal-mask vault-step-up-mask"><form className="vault-modal small vault-step-up" onSubmit={submit}>
    ...
    <div className="vault-share-mode vault-step-up-modes">{(hasPasskey ? [["passkey", "email", "password"] as const] : [["email", "password", "passkey"] as const])[0].map((key) => key === "passkey"
      ? <button type="button" className={mode === "passkey" ? "is-active" : ""} onClick={() => { setMode("passkey"); setValue(""); setMessage(""); }}><Fingerprint size={15} /><span><b>Passkey</b><small>生物识别或设备 PIN</small></span></button>
      : key === "email"
      ? <button type="button" className={mode === "email" ? "is-active" : ""} onClick={() => { setMode("email"); setValue(""); setMessage(""); }}><ShieldCheck size={15} /><span><b>邮箱验证码</b><small>{email || "未绑定邮箱"}</small></span></button>
      : <button type="button" className={mode === "password" ? "is-active" : ""} onClick={() => { setMode("password"); setValue(""); setMessage(""); }}><KeyRound size={15} /><span><b>登录密码</b><small>使用当前账号密码</small></span></button>)}</div>
