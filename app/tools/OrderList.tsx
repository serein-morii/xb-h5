...
export default function OrderList({ orders, contact, onEdit, onDelete, onView }: { orders: PublicOrderRecord[]; contact?: string; onEdit?: (order: PublicOrderRecord) => void; onDelete?: (order: PublicOrderRecord) => void; onView?: (order: PublicOrderRecord) => void }) {
  ...
  const visible = orders.filter((order) => (active === "ALL" || order.orderStatus === active) && (!normalizedKeyword || [order.orderCode, order.customer, order.phone, order.address, order.expCode, order.store, order.storeName, order.purchaser, order.createBy].some((value) => String(value || "").toLowerCase().includes(normalizedKeyword))));
