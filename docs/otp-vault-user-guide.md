...
展开“更多信息”后，还可以保存登录密码、登录网址、备注和标签，并设置收藏和敏感等级。输入标签后按回车即可生成标签块，最多 8 个；列表可按标签筛选。

同一系统和账号可以重复添加，例如两台设备各自的验证码。保存时如果已经有相同系统和账号，会先确认，确认后仍可保存。

### 导入文本

如果已有包含 `otpauth://` 地址的文本，或未加密的 Aegis / andOTP JSON，可以点击“导入”，选择文件或粘贴内容。系统会先列出新增、重复和冲突项目，不会直接写入；重复项目自动跳过，冲突项目可选择跳过或覆盖。读取和准备较大的文件时会显示进度。导入前请确认文件来源可信。开启零知识保护时，明文密钥只会在浏览器里加密后再上传。
...
- 批量导入会先预览新增、重复和冲突，再选择跳过或覆盖；大文件会显示进度
