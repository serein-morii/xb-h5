...
test("keeps received codes with credentials and exposes source filters", async () => {
  ...
  assert.match(workspace, /vault-code is-waiting"><span>等待验证码<\/span><button[^>]*vault-wait-copy[^>]*disabled/);
  assert.match(workspace, /一个来源组怎么工作/);
  assert.match(workspace, /来源组 = 一个接收通道 \+ 一套匹配规则/);
  assert.match(workspace, /只负责接收 · 可以复用/);
  assert.match(workspace, /选通道 \+ 定匹配规则/);
  assert.match(workspace, /复用已有规则（可选）/);
  assert.match(workspace, /包含任意一个关键词/);
  assert.match(workspace, /必须包含全部关键词/);
  assert.match(workspace, /只负责接收验证码，不决定验证码显示在哪张卡片/);
  ...
  assert.match(api, /dynamicCodeSources\?: VaultCredentialCodeSource\[\]/);
  ...
  assert.match(styles, /\.vault-code\.is-waiting span \{[^}]*font-size: 20px/);
