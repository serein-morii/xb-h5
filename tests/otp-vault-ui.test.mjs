...
test("keeps received codes with credentials and exposes source filters", async () => {
  ...
  assert.match(workspace, /vault-code is-waiting"><Radio size=\{11\} \/><span>等待验证码/);
  assert.match(workspace, /验证码怎么到这里/);
  assert.match(workspace, /把验证码收进来 · 全账号共用/);
  assert.match(workspace, /分到当前账号 · 仅此凭据/);
  ...
  assert.match(styles, /\.vault-code\.is-waiting span \{[^}]*font-size: 9px/);
  assert.match(styles, /\.vault-card\.is-compact \.vault-code\.is-waiting span \{ font-size: 8px/);
