...
test("keeps OTP display preferences accessible and compact layouts dense", async () => {
  ...
  assert.match(workspace, /setModal\("logoutConfirm"\)/);
  assert.match(workspace, /确认退出/);
  assert.doesNotMatch(workspace, /vault-logout" onClick=\{onLogout\}/);
  assert.match(workspace, /vault-otp-label/);
  assert.match(workspace, /detailOtpProgress/);
  ...
  assert.match(accountSetup, /only === "nickname"/);
  assert.match(accountSetup, /setVaultNickname/);
  assert.match(accountSetup, /setVaultEmail/);
  assert.match(accountSetup, /prepareIdentity/);
  assert.match(workspace, /only="email"/);
  assert.match(workspace, /<b>用户名<\/b>/);
  assert.match(workspace, /<b>账号<\/b>/);
  assert.match(workspace, /<b>邮箱<\/b>/);
  assert.match(accountSetup, /确认修改账号？/);
  assert.match(accountSetup, /确认修改用户名？/);
  ...
  assert.match(workspace, /长期/);
  assert.match(workspace, /durationDraft/);
  ...
  assert.match(workspace, /vault-detail-share-action/);
  assert.match(workspace, /await copy\(text, "完整分享信息已复制"\)/);
  ...
  assert.match(share, /readThemePreference/);
  assert.match(share, /验证码已复制/);
  assert.match(share, /aria-label="查看"/);
  assert.match(share, /ChevronDown/);
